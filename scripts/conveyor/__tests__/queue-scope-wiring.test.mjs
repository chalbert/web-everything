/**
 * @file queue-scope-wiring.test.mjs — epic #3383. Proves the queue scope is actually WIRED into every repo-wide
 * mechanical pass the conveyor tick runs, not merely available as a module.
 *
...
import { queueScopeEnvPatch } from '../../../skills-src/conveyor/runner.mjs';
import { QUEUE_SCOPE_ENV } from '../queue-scope.mjs';
...
const pending = (pr) => ({ ...pr, labels: [{ name: 'review:pending' }], comments: [], statusCheckRollup: [], mergeStateStatus: 'CLEAN', body: '' });

describe('reconcile-pass — the pass that drove the incident (review-dispatch + both tag scripts + the fix dispatch)', () => {
  const OPEN = [...MINE, ...THEIRS].map(pending);
  const run = (queueScope) => runReconcilePass({
    readPrs: () => OPEN,
    readAgents: () => [],
    enrich: (a) => a,
  ...
  it('UNSCOPED (the default): reconciles every open PR in the repo — today\'s behavior, pinned', () => {
    const plan = run({ enabled: false, log: silent });
    expect(plan.prs).toBe(4);
    const touched = new Set([...plan.dispatch, ...plan.refusals].map((e) => e.prNumber));
    expect(touched.has(2127)).toBe(true);
    expect(touched.has(2131)).toBe(true);
  });

  it('SCOPED: neither unrelated PR is even a candidate, so no review can be dispatched for it', () => {
    const plan = run({ enabled: true, ids: QUEUE, log: silent });
    expect(plan.prs).toBe(2);
    const touched = new Set([...plan.dispatch, ...plan.refusals].map((e) => e.prNumber));
    expect(touched.has(2127)).toBe(false);
    expect(touched.has(2131)).toBe(false);
    for (const n of [...touched]) expect([3001, 3002]).toContain(n);
  });

  it('SCOPED still reviews the instance\'s OWN queued PRs — scoping narrows, it does not disable the pass', () => {
    const plan = run({ enabled: true, ids: QUEUE, log: silent });
    const reviews = plan.dispatch.filter((d) => d.kind === 'review').map((d) => d.prNumber);
    expect(reviews.sort()).toEqual([3001, 3002]);
  });

  it('an EMPTY queue reconciles nothing at all rather than falling back to repo-wide', () => {
    const plan = run({ enabled: true, ids: [], log: silent });
    expect(plan.prs).toBe(0);
...
describe('runner --scope-to-queue — the flag that reaches every child process', () => {
  it('absent ⇒ patches NOTHING (so a marker the operator set still wins, and production is untouched)', () => {
    expect(queueScopeEnvPatch({})).toEqual({});
    expect(queueScopeEnvPatch({ once: true, json: true })).toEqual({});
  });

  it('present ⇒ exports the env override the shelled passes read', () => {
    expect(queueScopeEnvPatch({ 'scope-to-queue': true })).toEqual({ [QUEUE_SCOPE_ENV]: '1' });
  });
});
