 ...
 *   Two pieces are intentionally left for the #3443 reaper slice (#3862, which ports `session-reaper.mjs`
 *   itself): the file's last describe block ('the reaper plan, through the classifier', needs
 *   `classifySessionReapWithVerdict`) and one test inside the #3721 grace-period block ('through the reaper
 *   plan: ...', needs `sessionReapPlan`'s not-yet-landed `evidenceFor`/`graceMinutes` rewrite) — see the
 *   comments at each site. Everything else runs unchanged (backlog/3855).
 */
...
import { sessionTarget } from '../session-reaper.mjs';
...
describe('#3721 finished-but-alive: a result file or completion record reaps only a session at its prompt, quiet past the grace period', () => {
  ...
  // 'through the reaper plan: ...' (five-row planner case) intentionally omitted here, same reason as the
  // file's last describe block above it: it drives `session-reaper.mjs`'s `sessionReapPlan` through
  // `evidenceFor`/`graceMinutes`, which is the branch's REWRITTEN reaper (#3721/#3862), not `main`'s current
  // `sessionReapPlan` (still the older `groundTruthFor`/`completionFor`/`idleThresholdMs` shape — confirmed
  // live: passing `evidenceFor`+`graceMinutes` to it is silently ignored, so it reaps nothing where the
  // branch's version reaps two). Belongs to the #3443 reaper slice (#3862) alongside the rest of that wiring.
});
...
describe('grammar pins (mirrored, so tested against the originals)', () => {
  it.each(['conveyor-3451', 'conveyor-3441b', 'prepare-3438', 'prepare-decision-3402', 'review-148', 'fix-2347', 'ci-heal-2107', 'proto-note', 'lane-salvage', '', undefined])(
    ...
    (name) => {
      // `main`'s `sessionTarget` grew a `repo` field for multi-repo support after this branch's grammar was
      // frozen (#3855 port note); this pin only proves `dispatchGrammar` agrees on `kind`/`id`, which is all
      // it has an opinion on. Making `dispatchGrammar` repo-aware belongs to the #3443 slice that ports
      // `session-reaper.mjs` itself, not this one.
