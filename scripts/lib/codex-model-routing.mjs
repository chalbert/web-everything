 ...
 *
 * Both of those notes are discharged here. Three literal copies of a RATIFIED constant is exactly the shape
 * that lets a re-ratification (a new pinned model, a re-derived effort ladder) land on one copy and silently
 * miss the other two — the decision would then be true of the repo's documentation and false of its behaviour.
 * So the constants live HERE, in `scripts/lib/`, and every call site imports them.
 *
