...
describe('the #2771/#2785 tier table — real repo paths, pinned route', () => {
  ...
  it('supplying diffText never LOOSENS a non-statute route (it only ever proves the codify shape)', () => {
    const junk = 'diff --git a/x b/x\n+whatever';
    for (const [, changedFiles, humanRequired] of TABLE) {
      expect(scoreEscalation({ changedFiles, diffText: junk }).humanRequired).toBe(humanRequired);
    }
  });
});
