 ...
 * fail); the auto-ff only ever runs on a CLEAN tree and is `--ff-only --autostash` (advance main, preserve
 * dirty edits, never force/rebase); a dirty/diverged checkout is only WARNED about, never touched.
 *
 ...
 *   { action:'auto-ff' }           — behind, clean tree, NOT diverged → safe to fast-forward.
 *   { action:'warn', warning }     — behind AND (dirty OR diverged) → read may be stale; warn, don't touch.
...
export function classifyStaleness({ behind, ahead, dirty, autoFf, base = 'main' }) {
  ...
  if (autoFf && !dirty && (!ahead || ahead === 0)) return { action: 'auto-ff', behind, ahead: 0 };
  const why = dirty ? 'dirty tree' : ahead ? `diverged (${ahead} local commit(s))` : 'not auto-syncing';
  return {
