...
// `module.exports` surfaces as the default import; the alias map rides as a property on it.
const weSiteTheme = weSiteThemeModule;
const { LEGACY_ALIASES } = weSiteThemeModule;
 ...
 * `--token-*`. Generated from {@link LEGACY_ALIASES}, so renaming/removing a token at the source
 * updates the site's var via this projection (the #1813 acceptance), never a hand edit.
 */
function emitAliasBlock() {
  const decls = LEGACY_ALIASES.map(
...
export async function tokenCss() {
  ...
  return [
    ...
    '/* Legacy --<family>-* alias bridge → the emitted --token-* (#1824 Fork 2a sub-fork). */',
    emitAliasBlock(),
  ].join('\n');
}
