 ...
 *  - an explicit `utc-day-slice-ok: <reason>` comment on the offending line or the two lines above it,
 *    for a date that is UTC-anchored ARITHMETIC rather than a wall-clock read (re-projecting such a date
 *    into a local zone would shift it). The reason is required — the marker alone does not exempt.
 *
...
function sources(dir, out = []) {
  ...
  for (const name of entries) {
    ...
    if (name === 'utc-day-slice-scan.mjs') continue; // this file DEFINES the pattern; its docblock quotes it
...
export function findUtcDaySlices(scriptsDir, root = join(scriptsDir, '..')) {
  ...
  for (const abs of sources(scriptsDir)) {
    ...
    for (const match of text.matchAll(DAY_SLICE)) {
      ...
      // The marker may sit on the offending line itself or in the comment above it (a 3-line window, so a
      // marker plus a wrapped reason still counts).
      const window = lines.slice(Math.max(0, i - 2), i + 1).join('\n');
      if (OK_MARKER.test(window)) continue;
