 ...
 * @returns {{launch: object|null, suppressed: object|null, resolvedNum: string, item: object|null, briefTemplate: string, nextState: object, statusLine: string, notes: object[], bookkeepingSource: string}}
 */
export function readTick({
  num,
  bookkeepingFile = '',
  root = REPO_ROOT,
  runNode = (argv, opts) => execFileSync(process.execPath, argv, {
 ...
 * operation), and the count of skipped records rides the result so a caller can see the guard was partial.
 *
 * @param {string} key - the NORMALIZED item id.
 * @param {{store?: {list: Function, read: Function}}} [o]
 * @returns {{runs: Array<{runId: string, key: string, handle: (string|null), startedAt: (string|null)}>, unreadable: number}}
 */
export function inFlightDispatchesFor(key, { store = createFileRunStore() } = {}) {
  ...
  for (const id of Array.isArray(ids) ? ids : []) {
    ...
    for (const e of (run && Array.isArray(run.effects) ? run.effects : [])) {
      ...
      runs.push({ runId: String(run.id), key: String(e.key), handle: e.handle ?? null, startedAt: e.startedAt ?? null });
    }
  }
 ...
 * scope and eventual PR all belong to a lane nobody assigned it. `REPO_ROOT` is resolved by script location, so
 * this fires exactly when the operation itself was invoked from a lane clone.
 */
...
export function createDispatchSinks({
  root = REPO_ROOT,
  spawnAgent = (argv, opts) => execFileSync('claude', argv, {
 ...
 * closes the entry out with `resolveInFlight`. That is a standing cost per completed build, not a one-off line.
 * The genuine completion signal for a delivery agent is its PR, which the conveyor's own watcher
 * (`we:scripts/conveyor/pr-watch.mjs`) already watches to a terminal state; folding that in is #x9ylkp7.
 *
 ...
 */
export function createDispatchObservers({ listAgents = defaultListAgents, now = () => new Date() } = {}) {
 ...
 */
export function defaultListAgents() {
