 ...
 *   |---------|-----------|--------------------------------------------------------------------------------------|
 *   | `read`  | `compute` | ONE injected `readFacts` call: the checkout, whether it is a primary, the state paths, the clock, the tracked artifact.json |
 ...
 *      `<state root>/docket/state.json` — UNLESS `--force` was passed, in which case the comparison is skipped
 *      and the run is always treated as changed (a session that explicitly asked for a refresh gets a page and
 *      an owed record even when nothing in the ranked data actually moved — #3383's docket-tooling follow-up);
 *   d. only when the hash changed (or `--force`): `render` to the page beside the data, `state.json` rewritten,
 *      and ONE hand-off record, `publish-owed.json` ({@link buildPublishOwed}) — carrying the Artifact URL to
 *      republish (read from the tracked `skills-src/decision-docket/artifact.json`, so no session ever has to
 *      list its own artifacts to find the page) and the parseOk:false count + item numbers, so a session sees
 *      at a glance whether hand-fill is needed before it publishes. An unchanged hash (no `--force`) writes
 *      nothing more.
 *
...
/**
 * DID THE DATA CHANGE. `changed` is false only when a hash was recorded, equals this one, AND `force` was not
 * passed — `--force` always hands off (a session that explicitly asked for a refresh gets a page and an owed
 * record even when the ranked data is byte-identical to the last run, #3383's docket-tooling follow-up). PURE.
 * @param {{hash: string, state: (object|null), force?: boolean}} o
 */
export function decideHandoff({ hash, state, force = false } = {}) {
  if (!hash) throw new Error('docket-refresh: no content hash to compare');
  if (force) return { changed: true, reason: '--force requested a refresh regardless of whether the docket data changed' };
...
/**
 * THE HAND-OFF RECORD: what a publish session needs, and nothing it must decide. Carries the Artifact URL to
 * republish (read from the tracked `skills-src/decision-docket/artifact.json` by the IO shell — `null` when
 * that file is absent or unparseable, never fabricated) and the parseOk:false count + item numbers, so a
 * session sees at a glance whether hand-fill is needed before it publishes. PURE.
 * @param {{hash: string, ref: string, headSha: string, dataPath: string, htmlPath: string, now: string,
 *   counts: object, artifact: ({url: string, title: string}|null)}} o
 */
export function buildPublishOwed({ hash, ref, headSha, dataPath, htmlPath, now, counts, artifact = null } = {}) {
  return {
    owed: 'publish',
    hash,
    ref,
...
/** Counts shown on a run and carried in the hand-off: decisions, prepared, prepared with a parse warning, and
 * WHICH item numbers those are (never just the count — a session deciding whether hand-fill is needed needs
 * the numbers to go look at, not just a tally). PURE. */
export function countItems(data) {
  const items = Array.isArray(data?.items) ? data.items : [];
  const prepared = items.filter((i) => i && i.prepared);
  const parseWarningItems = prepared.filter((i) => !i.parseOk);
  return {
    decisions: items.length,
    prepared: prepared.length,
    parseWarnings: parseWarningItems.length,
    parseWarningNums: parseWarningItems.map((i) => String(i.num)),
  };
}
 ...
 * @param {{apply?: boolean, ref?: string, fetch?: boolean, force?: boolean}} input
 */
export function planDocketRefresh(read, { apply = false, ref = 'origin/main', fetch = true, force = false } = {}) {
  ...
  return {
    ...
    steps: [
      ...
      force
        ? 'skip the content-hash comparison (--force): always render + record + hand off'
        : 'compare the content hash (clock fields removed) with the state file',
      'when it changed (or --force): render the page, record the hash, write the publish-owed record',
    ],
...
export function finishDocketOutcome({ run, code, lines, json = false } = {}) {
  ...
  const nums = r.counts.parseWarningNums ?? [];
  const out = [
    ...
    // Surfaced at a glance, never buried in the JSON: a session reading only stdout must see immediately
    // whether hand-fill is owed before it publishes.
    nums.length ? `docket-refresh: parseOk:false — ${nums.length} item(s): #${nums.join(', #')}` : 'docket-refresh: parseOk:false — 0 items (no hand-fill needed)',
    `docket-refresh: ${r.reason}`,
    r.owedPath
      ? `docket-refresh: publish owed, recorded at ${r.owedPath} (page ${r.htmlPath}); artifact ${r.artifactUrl || '(no skills-src/decision-docket/artifact.json found — publish target unknown)'}; publish via ${PUBLISH_VIA}`
      : 'docket-refresh: no publish owed',
  ];
...
export function docketRefreshOperation({ readFacts } = {}) {
  ...
  return op(DOCKET_REFRESH_OP, {
    declaresOver: DECLARED_HOMES['docket-refresh'],
    input: {
      ...
      // Skip the unchanged-hash short-circuit: a session asked EXPLICITLY to refresh still gets a rendered
      // page + a publish-owed record even when the ranked data hasn't moved since the last run.
    ...
    plan: compute({
      reads: ['findings.read', 'input.apply', 'input.ref', 'input.fetch', 'input.force'],
      fn: (view) => planDocketRefresh(view.findings.read, {
        apply: view.input.apply, ref: view.input.ref, fetch: view.input.fetch, force: view.input.force,
      }),
    }),
    ...
    apply: effectStep({
      ...
      effects: (view) => {
        ...
        return [{
          ...
          // IDEMPOTENT: a second run regenerates the same data, and an unchanged hash (without `--force`)
          // writes no hand-off.
          idempotent: true,
          payload: {
            root: r.root, ref: view.verdict.ref, fetch: view.verdict.fetch, force: view.verdict.force,
            dir: r.dir, dataPath: r.dataPath, htmlPath: r.htmlPath, statePath: r.statePath, owedPath: r.owedPath,
            state: r.state, artifact: r.artifact, now: r.now,
          },
        }];
      },
    }),
