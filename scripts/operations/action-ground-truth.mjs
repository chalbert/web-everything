...
// #3901 Wave B finding — at snapshot 600acc14f, `isDispatchHandleLive` lived on `./dispatch-lane-io.mjs`
// (re-exported there from `./detached-dispatch.mjs`, the real home post-#3645). `detached-dispatch.mjs` is
// #3902's file (blockedBy THIS item, so it cannot land first) and main's own `dispatch-lane-io.mjs` has never
// carried this symbol, so the original import silently resolved to `undefined` and crashed under real Node
// ESM the instant this module was loaded outside a mock — proven with `node --input-type=module -e
// "import('./action-ground-truth.mjs')"` (SyntaxError: does not provide an export named 'isDispatchHandleLive').
// Inlined here VERBATIM from the snapshot (`isDispatchHandleLive`/`isHandleListed`/`normalizeHandle` from
// `we:scripts/operations/dispatch-lane-io.mjs`, `detachedHandlePid`/`DETACHED_HANDLE_PREFIX` from
// `we:scripts/operations/detached-dispatch.mjs`) — no behaviour change, just relocated so this leaf is
// actually loadable before #3902 lands. #3902 should delete this block and restore the import from
// `./detached-dispatch.mjs` once that file exists on main.
const DETACHED_HANDLE_PREFIX = 'pid:';
function detachedHandlePid(handle) {
...
function isHandleListed(handle, sessions) {
  const h = normalizeHandle(handle);
  if (!h) return false;
  return (Array.isArray(sessions) ? sessions : []).some((s) => normalizeHandle(s?.sessionId).startsWith(h));
}
function isDispatchHandleLive(handle, sessions, { isPidAlive } = {}) {
