...
export function createExploreObservers({
  ...
  return {
    [INVESTIGATE_EFFECT]: async (entry, ctx) => {
      ...
      // #3331: matched by NAME FIRST, not by `handle` — the CLI ignores `--session-id` on a `--bg` spawn, so the
      ...
      // in it but no usable `sessionId` AND no `name` on any of them is a shape this reader does not understand.
      // Sitting ABOVE the report tail below (not merely above the `unresolved` return) matters here
      // specifically: with a report on disk, falling through would answer `resolved` with a TRUNCATED result on
      // the strength of a listing that told us nothing — the exact failure this exists to stop. A listing that
      // carries a `name` on at least one element (the #3331 shape checked above, just none matching THIS
      // panelist) is a valid, understood read and must not trip this guard — it correctly falls through to the
      // id-based check and then liveness-grace below.
      const listedIds = new Set(sessions.map((s) => normalizeHandle(s?.sessionId)).filter(Boolean));
      const anyName = sessions.some((s) => s && String(s?.name ?? '').trim() !== '');
      if (sessions.length && !listedIds.size && !anyName) {
        throw new TypeError(
          'explore-io: `claude agents --json` returned '
          + `${sessions.length} element(s) but not one carried a usable \`sessionId\` or \`name\` — the listing was `
          + 'read and not understood, which is not evidence that any session ended',
        );
      }
