...
// createLandAdvanceApplier lives in land-advance-io.mjs, which is #3856's scope (this item is blockedBy-order
// before it, and #3856 hasn't landed yet). The one case below that exercised it is deferred to #3856 rather
// than dropped silently — see this slice's PR notes and #3856's card for the restore.
...
// 'applies items by queueing them into the canonical conveyor sidecar, idempotently, only when opted in' deferred
// to #3856 (needs createLandAdvanceApplier from land-advance-io.mjs, #3856's scope).
