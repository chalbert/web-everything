...
it('reports owed unsupported work in text and JSON and uses the constellation slugs', () => {
  ...
  try {
    ...
    expect(JSON.parse(log.mock.calls[0][0])).toEqual({ ready: [], pending: [], notReady: [], stoodDown: [], stuck: [], errors: [], unsupported: [{ ...row, repo: 'plateau-app', at: expect.any(String) }], laneDecisions: [], backpressure: [] });
    log.mockClear();
