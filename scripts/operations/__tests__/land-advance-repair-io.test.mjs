...
const ports = (over = {}) => ({ fs: emptyFs, now: () => now, home: '/nonexistent/home', drainDir: '/nonexistent/drain', jobsDir: '/nonexistent/jobs', trialLog: '/nonexistent/trials.jsonl', readSessions: () => [], store: createMemoryRunStore(), machineLoad: () => 0, findItemFn: () => null, loadItems: () => [],
  resolveFallbackScope: vi.fn((pr) => [`we:scripts/x-${pr}.mjs`]), readPrComments: () => [],
...
describe('reader: repair evidence', () => {
  it('plans #2349 and #2344 through the same item-number-free planner, marking the conflict', () => {
    ...
    expect(inputs.fixPlans['we#2349'].planned).toMatchObject({ pr: 2349, itemNum: null, attributionKind: 'PR', attributionNum: '2349', scope: ['we:scripts/x-2349.mjs'], scopeSource: 'pr-diff', isConflict: false });
    expect(inputs.fixPlans['we#2344'].planned).toMatchObject({ pr: 2344, itemNum: null, attributionKind: 'PR', attributionNum: '2344', scopeSource: 'pr-diff', isConflict: true });
    expect(p.resolveFallbackScope).toHaveBeenCalledWith(2349, null);
  ...
  it('reads the PR\'s own CI-heal and stand-down comments, and only for PRs that owe a repair', () => {
    const readPrComments = vi.fn((pr) => pr.number === 2349 ? [{ body: `${CI_HEAL_COMMENT_MARKER}\n\nx` }, { body: `${CI_HEAL_COMMENT_MARKER}\n\nx` }, { body: 'quoting: ' + CI_HEAL_COMMENT_MARKER }] : [{ body: `${STAND_DOWN_MARKER}\n\nx` }]);
...
describe('apply: the repair dispatches', () => {
  ...
  it('routes ci-heal to the ci-heal dispatch and a conflict to the reconcile fix, each with its own tool grant and ledger kind', async () => {
    ...
    expect(p.dispatchCiHeal).toHaveBeenCalledWith(expect.objectContaining({ pr: 2349, reason: 'red-ci', lane: 7, attributionKind: 'PR' }), expect.objectContaining({ extraArgs: [allowedToolsArg('ci-heal')], repo: 'chalbert/web-everything' }));
...
describe('dispatchCiHeal (the tick sink, entered for a PR by number)', () => {
  ...
  it('fills the real ci-heal brief and hands one ci-heal payload to the sink', async () => {
    ...
    const out = await dispatchCiHeal(planned, { sinks, repo: 'chalbert/web-everything' });
    ...
    expect(payload).toMatchObject({ launchKind: 'ci-heal', sessionSlug: 'ci-heal-2349', pr: 2349, reason: 'red-ci', lane: 7, scope: planned.scope, repo: 'chalbert/web-everything' });
