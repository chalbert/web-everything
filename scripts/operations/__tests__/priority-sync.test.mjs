...
describe('the section parser', () => {
  ...
  // #3892 (this operation's own graduation) — faithful port from the prototype branch, where this test is
  // green because the branch's OWN copy of the #3383 card has been dogfooding a live `## Priority order`
  // section for weeks. `main`'s independently-evolved #3383 card has NO such section at all, and this
  // operation is a SYNC tool, not a section-creator: `priority-sync-io.mjs`'s read step refuses outright when
  // the heading is entirely absent, because `place()` above requires a pre-existing Band A/B/C/Claimed/
  // Off-path skeleton to insert into. Seeding that skeleton needs one real operator judgment call this port
  // must not invent unaided (which cards belong in the rule-3-pinned Health-chain/Delegation lists, never
  // auto-populated) — filed as `we:backlog/xa478ud-*`. Skipped here, not deleted or reworded, exactly like the
  // sibling pair in `we:scripts/__tests__/priority-order.test.mjs` (landed by #3891, written to wait for this
  // same operation): it starts proving the real checkout the moment `xa478ud` seeds the skeleton.
  it.skip('agrees with the gate\'s own parser over the REAL tracker card of this checkout', () => {
