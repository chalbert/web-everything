...
describe('the section parser', () => {
  ...
  // #3892 — HERMETIC BY FIXTURE, not the live checkout's #3383 card. The section this test needs (100+ ordered
  // entries, several bands, an off-path list) exists only on the prototype's own copy of the card — `main`'s
  // #3383 card never carries a `## Priority order` section, so reading it live made this test fail on every
  // checkout of `main` for a reason that has nothing to do with the parser. `3383-priority-order.md` is a
  // byte-for-byte trim of that section from the `origin/lane/mechanical-dispatcher` snapshot this operation was
  // ported from (`600acc14f`) — real prose, real entries, just not re-read from a moving target. The assertion
  // is unchanged: `parseSection` (this operation's own parser) must still agree with `parsePriorityOrder` (the
  // `check-priority` gate's parser) on every line, over real >100-entry data.
  it('agrees with the gate\'s own parser over a real, fixed #3383 tracker section (#3892)', () => {
    const path = resolve(HERE, '..', '__fixtures__', 'priority-sync', '3383-priority-order.md');
