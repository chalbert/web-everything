#!/usr/bin/env node
/**
 * review-core.mjs — a thin impure CLI over the PURE review-verdict core in
 ...
 *
 * Usage:
 *   node scripts/review-core.mjs reduce --file=findings.json          # human: verdict/disposition/outcome + table
 *   cat round.json | node scripts/review-core.mjs reduce --json       # machine object on stdout
 *   node scripts/review-core.mjs reduce --file=r.json --round=2        # also derive the negotiation outcome
 *   node scripts/review-core.mjs reduce --file=r.json --round=2 --phase=plan   # ...the plan-handshake outcome
 *   node scripts/review-core.mjs mandate --lens=correctness           # a panel reviewer mandate for one lens
 *   node scripts/review-core.mjs mandate --validator --lens=security  # the independent-validator mandate
 *   cat findings.json | node scripts/review-core.mjs mandate --editor --round=2   # the editor-round mandate
 *
...
function main(argv) {
  ...
  return fail(
    'usage: review-core.mjs <reduce|mandate> [flags] — see the header for options',
    2,
  );
}
