...
export function projectVerdictCommentLength({ body = '', actor = '', reason = '', clearerId = '' } = {}) {
  // The not-established outcomes, worst case. `unknown-clearer` fires on an empty clearer id; `unknown-author`
  // on a present clearer with no author stamp; `self-clear` on two equal ids — which `--to=clear-human` now
  // RENDERS (the PR #1100 human-ceremony exemption: it is no longer refused, so its note reaches the comment and
  // must be counted, exactly the under-count M2 documents). All three messages embed the clearer id, so each is
  // projected with the REAL one rather than a fixed-width stand-in.
  const outcomes = [
    null,
