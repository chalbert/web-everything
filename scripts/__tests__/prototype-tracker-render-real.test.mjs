...
describe('prototype-tracker render (real command line)', () => {
  it('renders the compact page by default: sections in order, under 60 KB, NEEDS YOU verbatim, rows matching the list', async () => {
  ...
  it('--out writes the file and reports its size in bytes', async () => {
  ...
  it('reads a card only a ref has (the prototype branch lags main), and can be told not to', async () => {
  ...
  it('links card numbers only when given a base URL', async () => {
    await scenario(({ render }) => {
  ...
  it('says NEEDS YOU is unavailable, never "none", when no operator-queue is found', async () => {
    await scenario(({ render }) => {
      const { out } = render();
  ...
  it('prints "none" when the queue has nothing for the operator', async () => {
    await scenario(({ render }) => {
