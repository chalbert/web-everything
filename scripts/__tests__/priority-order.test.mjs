...
describe("this checkout's real tracker card", () => {
  ...
  // #3891 lands only the priority-order LIBRARY (this file's fixture-driven tests above are the faithful,
  // unmodified port and are fully green). The two tests below assert that the real backlog/3383 tracker card
  // ALREADY carries a maintained `## Priority order` section — that section is written by the `priority-sync`
  // operation, which is #3892 (blockedBy: ["3891"], not yet landed on main). Skipped here, not deleted or
  // reworded, so they start proving the real checkout the moment #3892 lands and runs priority-sync for real;
  // hand-authoring the section here would mean re-deriving #3892's own band/ranking logic against ~300 live
  // cards, which is that item's job, not this one's.
  it.skip('has a parseable Priority order section with no duplicate numbers', () => {
  ...
  it.skip('lists no card that is resolved in this checkout, and orders no blocker below its blocked card', () => {
