/**
 * @file review-core-cli.test.mjs — proof of the PURE glue layer of the `scripts/review-core.mjs` CLI (#2435):
 *   `parseFlags`, `reduceReview` (the reduction that collapses the drain's 5× inline `node -e` calls into one
 *   testable fn), and `buildMandateText`. The stdin/--file/print I/O is the CLI's boundary; the derivations are
 *   decided in these pure helpers and unit-tested here against fixtures — no spawning, no shelling out.
 *
...
import { parseFlags, reduceReview, buildMandateText } from '../review-core.mjs';
import {
  VERDICTS,
