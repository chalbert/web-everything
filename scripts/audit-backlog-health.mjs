...
// ---- run checks ----------------------------------------------------------
const flags = { G1: [], G2: [], G3: [], G4: [], G5: [], D1: [], D2: [], D3: [] };
...
const desc = { G1: 'prose prereq not lifted into `blockedBy` (INFO = both ends resolved)', G2: 'resolved build gated on a decision that was open/later (git-dated)', G3: 'graduated entity with no governing decision (transitive lineage + prose)', G4: 'prepared decision with prioritization/effort tells in a `## Fork` (fork-existence test skipped)', G5: 'prepared decision with a `## Fork` lacking a fork-existence justification line (#819)', D1: 'backticked code path that no longer exists', D2: 'referenced #N item that does not exist', D3: 'project missing from projects.json, or `concept` despite shipped work' };
...
console.log(`audit: ${items.size} items, ${tot} flags (G1=${flags.G1.length} [${HI.length} HIGH, ${G1_INFO} INFO], G2=${flags.G2.length}, G3=${flags.G3.length}, G4=${flags.G4.length}, G5=${flags.G5.length}, D1=${flags.D1.length}, D2=${flags.D2.length}, D3=${flags.D3.length})`);
