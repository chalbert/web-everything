...
function cmdAcquire(repo) {
  ...
  if (flags.lane !== undefined) {
    ...
    if (!flags.force && !targetWasReserved && !flags['no-reset']) {
      const { dirty, uncommitted, ahead } = laneDirtyOrAhead(dir, repo.branch);
      // #3383 (found live 2026-09-24) — commits ahead of origin/<branch> are only at risk when NO remote head holds
      // them: a lane sitting on a pushed POC tip (hundreds ahead of main, all on origin) was refused, the same
      // containment proof auto-pick and the #2924 re-check below already accept.
      const atRisk = dirty || (ahead > 0 && !aheadIsProvablyPushed(dir, liveRemoteShas(dir)));
      if (atRisk) {
        // …and a refusal hands the lane back: the claim above already wrote OUR lease, and leaving it would hold a
        // lane nobody is using until its TTL (the lane stayed leased for 4 hours after a refused acquire).
