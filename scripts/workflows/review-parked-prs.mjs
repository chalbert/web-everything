...
//
// NOTE — every agent here runs on the session model; none is cheap-tiered. `schema` VALIDATES shape, never
// judgment: `VERDICT_SCHEMA.verdict`/`outcome` are plain strings with no enum, and `FETCH_SCHEMA.diff` requires
// only non-empty — so a wrong answer from a weaker model would pass validation and be consumed verbatim. The
// right fix is not a cheaper model but NO model (these shell fixed commands and exercise no judgment); that
// refactor is #xpr5p67.
// ─────────────────────────────────────────────────────────────────────────────
