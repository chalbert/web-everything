...
export function buildManifest(input) {
  ...
  const repos = (input.repos ?? [])
    .map((r) => ({
      ...
      // omitted for a plain sibling off origin/main). Optional; never written as an empty string.
      ...(r.base != null && String(r.base) !== '' ? { base: String(r.base) } : {}),
    }))
...
export function validateManifest(m) {
  ...
  for (const r of repos) {
    ...
    // #2387 F3 — `base` is optional, but when present it must be a non-empty string (a commit SHA), never
    // e.g. a bare boolean/number that would silently fail every downstream `git` call.
    else if (r.base != null && (typeof r.base !== 'string' || r.base === '')) errors.push(`repo "${r.repo}" has an invalid \`base\` (must be a non-empty string)`);
  }
