...
*correct* (the loser drops the now-`active`/dirty item), but the loser already burned the up-front
...
|---|---|
| `dirty` | `node scripts/backlog.mjs claim <NNN>` **refused** it (a concurrent session owns it / uncommitted edits). The tool decided, not you. |
