...
- **One `## Fork N` section per open fork** — crux-with-refs → options **A/B/… named with their tradeoffs** → the **recommended default in bold** → rejected branches marked *Rejected* with the reason → any sub-decision. (At resolution each gains a dated ruling, and the item **graduates to spin-off builds via a `blockedBy` chain** in composition order — #64 → #295 hierarchy → #296 block (disclosure pre-existed, from #008) — a prepared decision yields agent-ready builds, not code itself.)
...
- **Commit each finished piece of work — never push.** When an item closes out (`resolved`/`released`, after the capture pass + green gate), **commit that item's changes** with a clear message, then continue. One commit per closed item (the natural "piece of work"); a batch produces one commit per resolved item, not one lump at the end. **Stage only the files that piece touched** (explicit paths — never `git add -A`), so a pre-existing dirty/untracked tree from other work is left untouched and never swept into your commit. **Never `git push`** — the user owns pushing and the remote. Don't audit or comment on *unrelated* changes already in the tree — commit your piece and move on. End commit messages with the standard `Co-Authored-By` trailer.
