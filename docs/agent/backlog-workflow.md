...
**Fork-readiness pass — before presenting a decision, make the item explain its fork concretely; fix it on the spot if it doesn't.** You've just claimed it and are reading it to present it — the cheapest moment to improve it (same fix-on-the-spot rule as *The digest*). A decision item earns a good ruling only if the fork is legible, so check four things and *edit the item* (not just your chat message) where it falls short:
...
3. **No eligible next item — *after a seam re-pack*.** The planned pack is exhausted **and** a fresh `--select --budget=<remaining>` surfaces nothing new (the cascade freed nothing eligible; what's left is a decision/review, a `story·≥13`/`epic`, blocked, or the pool is empty after the concurrency re-read). This is the budget-left-on-the-table stop: it's correct, not a failure — don't pad the batch with unvetted close-out leftovers to reach the number.
...
5. **Context seam — only on a countable signal, never a feeling (earlier stop only).** The **points budget (rule 2) is the driver**: absent a hard stop (1/3/4), the batch runs *to the budget*. You **cannot read your own context meter**, so a *felt* sense of "heavy context" (long transcript, many large reads, repeated test runs) is **not a measurement and not a valid stop** — it reliably mis-fires (a batch that *felt* full has measured at ~22% used, i.e. ~3× the budget still in hand). Do **not** stop on a vibe; "I've read a lot" is not a seam. This rule fires **only** on a concrete proxy: **(a)** a *planned* repo/subsystem boundary you set at plan time (the next item loads a fresh codebase — the cheapest handoff, stop there), or **(b)** the user reports an actual context-meter reading that's genuinely near the limit (don't estimate it — *ask*, per *Calibrating the budget* and the "ask, don't estimate" rule). On (a) or (b), finish the current item cleanly and stop; otherwise **keep claiming the next eligible item until the budget (or rule 1/3/4) fires.** **This seam is also the calibration signal:** when you do stop, record the points resolved and — once the user reports it — the context %, which *Calibrating the budget* (below) folds back in. (Premature stops were the failure this rule was rewritten to kill: trust the budget, not the gut.)
