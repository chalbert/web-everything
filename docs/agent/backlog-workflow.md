...
| `blocked-in-fact` | A dependency or artifact the item *needs* is **verified absent** — you checked and it isn't there (a function/file/served URL the body names doesn't exist), not "I assume it's blocked." Often means a `blockedBy` edge is missing → add it (*Keep the blocker DAG honest*). |
...
| `out-of-locus` | It lives in a repo whose gate this batch can't run — see *Repo-locus* below. |
...
#### Repo-locus — a batch can only honestly close items its own gate validates

A batch's close-out gate is **the repo it runs in** — for a `webeverything` batch that's `npm run check:standards` + WE vitest (+ the WE dev servers for a render check). An item is **in-locus** when that gate can *honestly* validate it, and **out-of-locus** when its acceptance can only be confirmed by **another repo's gate or dev server**: a plateau-app configurator view (needs `:4000` + plateau-app checks), a Frontier UI primitive that publishes `@frontierui` (gated in `frontierui`), or a flagship exercise-app phase (owned by the `/exercise-app` loop's conformance discipline). **Resolving an out-of-locus item on *this* repo's green gate is dishonest** — the gate never exercised it. Locus is about *which gate validates the work*, **not** where the backlog item is filed (the WE backlog tracks the whole constellation): an item filed as "FUI slice" whose code legitimately lands in WE and gates here (e.g. a generator that generalizes WE's own loader) is **in-locus** — judge by the gate, not the label.

**How it works (BUILT — locus-filter the packer).** Every item carries a `locus` — its **gate home** — derived in the loader (`src/_data/backlog.js`): an explicit `locus:` frontmatter field when authored, else inferred (an `exercise-app` *descendant* of the flagship epic #314 — structural, never the noisy `exercise-app` tag; or a precise build-tag like `frontier-ui` / `technical-configurator`), else `webeverything` (the default). Valid values are `LOCI` in `check-standards-rules.mjs` (`webeverything` / `frontierui` / `plateau-app` / `exercise-app`). `npm run check:readiness -- --select` packs **only** items whose locus matches the batch's (`--locus=`, default `webeverything`) and shunts the rest to an **"Other locus"** list — so the pool the recovery refills from is honest **by construction**: a WE batch can't pack, and so can't under-run on, a plateau-app/FUI/exercise-app item it was never going to gate. `check:standards` errors on an invalid authored locus and warns on a *batchable* item whose cross-repo locus was only inferred (a nudge to declare it; inference biases toward exclusion — wrongly deferring a workable item is the cheap failure, wrongly packing an out-of-locus one resolves on the wrong gate). An explicit `locus:` always overrides the inference (e.g. an item tagged `frontier-ui` but built and gated in WE declares `locus: webeverything`).

**Future capability (a) — repo-aware close-out (not built; only if cross-repo batches become common).** Teach the batch to run **each item against its own locus's gate** (WE item → WE gate; plateau-app item → plateau-app checks + `:4000`), so cross-repo items become genuinely batchable in one run. More machinery (per-locus gate config, multi-repo dev-server awareness); layered on top of the locus signal above. Until/unless it lands, an `out-of-locus` item is a hard drop-reason — surfaced, never silently resolved on the wrong gate.
