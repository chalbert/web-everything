...
4. **Plan, then design-first.** Write a plan naming **which layer(s)** to create (see cheat-sheet below) *and* the plugs/implementations that follow. When a standard exists so **many libraries/vendors can interoperate** (e.g. positioning engines, keyboard handlers), prefer a **registry + provider-contract plug** design ("one engine, all consumers delegate via the injector chain") with thin adapters — **do not reimplement** the third-party lib. Draft the spec **with open questions**, and capture every unresolved fork as a backlog **decision work item** (`type: decision`) — the fork, the options, the ruling, and the implementation outline live *in that item* (the plan of record), with a recommended default in bold (not rapid-fire popups). **Do NOT use Claude plan mode / `ExitPlanMode` / a `~/.claude/plans` file to stage a design decision** — the backlog item is where the plan lives; iterate there before writing anything. See [backlog-workflow.md](backlog-workflow.md). Then implement and run the **Definition of Done** (AGENTS.md): `gen:inventory` if you added a block/plug/intent, `check:standards`, affected tests.
