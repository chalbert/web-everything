...
**Lineage:** ratified by #2851 (operator, 2026-08-02). Composes with — does not alter — [#review-human-declarative-leash-only](#review-human-declarative-leash-only) (#2771, resolved — the leash-vs-derivation split this loop clears the derivation branch of; its implementation #2785 is `status: open`, so the split is ruled but not yet live). **Composes with — applies (does not alter)** [#agent-convergence-independent-validation](#agent-convergence-independent-validation) (#2398, resolved — independence rests on a distinct fresh validator, in-process role-separated subagents allowed): invariant 1 applies that bar to the conveyor — the orchestrator wearing a reviewer hat is #2398's own non-fresh-context reject case — and cites #2398's actual sentence rather than restating a stricter one. Cites #2439 (resolved — the distinct fresh-context validator + `redteam:accepted` interim rail). Invariant 2 applies the prevention-introspection review discipline (#2823, still `status: active` — cited as the discipline this loop adopts, not settled precedent). The independence-enforcement and reviewer-id checks are owed on the OPEN items #2840/#2785, tracked as outstanding preventions on #2851.

---

### The converge editor is gated by `humanRequired`, never by the care band — care stays advisory and never routes {#converge-editor-gated-by-human-required}

**Ratified 2026-08-08 by the operator (Nicolas Gilbert) (#2908).** The convergence loop's **editor** — the subagent that rewrites the diff and pushes to the author's branch — is enabled on **every** parked PR **except** one whose diff is `humanRequired`: a **statute-layer** edit (`isStatutePath`) or a **declarative-leash** edit (`isDeclarativeLeashPath`). That set is the whole hazard and nothing more — a machine editing *its own constraints* is the conflict of interest; a machine editing code that merely *obeys* the constraints is not, per the #2445 two-tier flip and the #2771/#2785 policy-tier split. It is a strict **subset** of blast-radius, so the exclusion is the narrowest one that covers the hazard. **The care band is NOT the editor's switch, and this is the load-bearing half of the rule.** A care level is advisory review-*rigor* — how hard to look — and must **never** decide *who* looks (`we:scripts/lib/review-escalation.mjs:244`). Gating the editor by band would silently convert that dial into a routing dial, and would re-admit **diff size** as grounds for reaching a person: size (`CARE_WEIGHTS.size` 2) plus blast-radius (3) reaches `high`, and a review-only band routes to the operator. Diff size is deliberately barred from human routing — the scored rubric caps it at `review:pending`, never `review:human` (#2563), because **a human reviews a big diff worse than the panel does** (PR #1095 closed the last self-park loophole on exactly that ground). Blast-radius, size, cross-repo and dismissed-findings therefore raise **rigor only**, never route. **Rider — enablement and round budget are one decision:** the editor is reachable only at a band with **≥ 2 rounds** (one to push, one for a fresh panel to judge the push), because at `roundCap: 1` the loop forces `escalate` before the editor step is reached (`we:scripts/workflows/review-parked-prs.mjs:997-1003`, editor at `:1050`) — and a push no panel re-read would break the loop's invariant that a `land` means a non-author panel signed off the final diff. Carry that minimum on a **dedicated editor knob**, never by raising `panelRigorForCareLevel`'s `low` entry, which `/jury` and `/review` also read (`we:scripts/lib/jury-core.mjs:1113`). **Exhaustively, four things then reach the operator:** a statute edit; a declarative-leash edit; loop **deadlock** (rounds spent, panel still at `changes`); loop **breakage** (the editor could not push, a mandatory correctness/security lens did not run, or the diff could not be fetched — a dead reviewer never reads as an accept). Prevention strategies for the last two are **out of scope of this rule** and filed separately.

**Lineage:** ratified by #2908 (operator, 2026-08-08). The prepared default ("editor at `low`, review-only above") was **withdrawn as unimplementable** — `low` carries a 1-round cap, so it selects the one band where the editor cannot run, making it behaviourally identical to review-only-everywhere; its repair (bind to the band, raise `low` to 2 rounds) was then withdrawn on the diff-size ground above. Grounded in the loop's first real run, PR #1018 (`care: elevated`): the editor pushed a 15-file fix, round 2 faulted that repair three ways including a fail-open in the gate the fix had just written, and the loop escalated having cleared nothing — 16 agents, 1.08M tokens, 56 minutes. The ruling **accepts** that band anyway: #1018's safety property held (the panel caught the bad repair; nothing defective landed), so it evidences an *efficiency* failure, and `humanRequired` is the only axis with a stated mechanism rather than a correlation. Composes with [#fix-review-convergence-independent-root-cause](#fix-review-convergence-independent-root-cause) (#2851 — escalate only on judgment or deadlock), [#review-human-declarative-leash-only](#review-human-declarative-leash-only) (#2771 — the leash/derivation split this rule's exclusion set is taken from), [#human-required-is-judgment-only](#human-required-is-judgment-only) (#2851) and [#blast-radius-advisory-care-not-a-gate](#blast-radius-advisory-care-not-a-gate) (which this rule extends from park-gating to editor-gating). Mechanisms: `we:scripts/workflows/review-parked-prs.mjs`, `we:scripts/lib/review-escalation.mjs` (`scoreEscalation` / `deriveCareLevel`), `we:scripts/lib/jury-core.mjs` (`panelRigorForCareLevel`).

---

### A slice with a deterministic oracle is cleared by the oracle, not a human {#deterministic-oracle-clears-slice}

**Ratified 2026-08-02 by the operator (Nicolas Gilbert) (#2851).** When a slice carries a **deterministic acceptance oracle** — a test whose green/red is unambiguous — a **GREEN oracle mechanically clears the slice**. No human verification is owed once such an oracle exists and is green: verifying a deterministic pass/fail is not judgment ([#human-required-is-judgment-only](#human-required-is-judgment-only)), it is reading the result. The `human-verify` tag on such a slice applies **only until that oracle exists** — its purpose is to hold the slice while the acceptance criterion is still a matter of human eyeballing, and it retires the moment the criterion becomes a deterministic check. (`human-verify` is a documented convention today with **no code reader** — the retirement is a prose discipline; a gate that observes it is an outstanding prevention on #2851, owed on the open #2840/#2785 line.)
...
**Lineage:** ratified by #2840 (operator, 2026-08-02). Mechanizes [#human-required-is-judgment-only](#human-required-is-judgment-only) (#2851) on the review gate and **extends** [#review-human-declarative-leash-only](#review-human-declarative-leash-only) (#2771 — narrowed the gate by *path* to the declarative leash; this adds the *edit / guarantee* axis on top), composing [#fix-review-convergence-independent-root-cause](#fix-review-convergence-independent-root-cause) (#2851 — the mechanical clearer the shed behaviour-preserving work routes to). Depends on impl #2785 (the base `POLICY_SPEC` path narrowing, `status: open`) landing first, and on the shared producer-side `diffHunks` plumbing. Current mechanism it replaces: `we:scripts/lib/review-escalation.mjs#isGateSelfPath`, `we:scripts/lib/gate-config.mjs#isPolicyCorePath`. Composes with [#principle-and-impl-two-pr](#principle-and-impl-two-pr) (#2839) and [#enforce-flip-triple-gated](#enforce-flip-triple-gated) (#2838).
