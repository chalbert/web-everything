...
- `principleTouch` reuses #2840's `isPrincipleSurface` at its **pre-existing-guarantee grain** (statute-anchor
  edit, or edit/removal of a `@principle`/`@invariant` marker present in base). `implTouch` = executable/impl
  code OR a hunk that **adds** a new marker+assertion. REFUSE iff `principleTouch && implTouch`.
