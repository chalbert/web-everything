...
Photopea's live capture lands on its marketing splash, not the editor; the inclusion gate currently quarantines it (parked in design-refs/needs-review.json). Tune its targets.json enterAction (the 'Start using Photopea' click-through) and readySelector so the editor surface is captured and confirmed, then re-run design-refs collect --refresh --only=photopea and verify the shot is the app, not the landing page.
...
**Fix:** `design-refs/targets.json` — `readySelector: "canvas"` → `readySelector: "text=New Project"` (the editor Home screen's primary button — visible ~0.4s after the click-through, editor-only, never on the marketing splash which says "Start using Photopea"). Bumped `readyTimeout` 15000 → 20000 for headroom. Verified with the **exact** `design-refs.mjs` launch (headless, `--disable-blink-features=AutomationControlled`, no GPU flags) — no script change needed.
