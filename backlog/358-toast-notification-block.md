...
- **`fui:blocks.json`** — `notification` entry: `implementsIntent: feedback` (it realizes the existing Feedback
...
- **`we:block-descriptions/notification.njk`** — overview, Web Standards Alignment + Framework Research tables,
...
- **`we:semantics.json`** — terms *Notification*, *Notification Region*.
