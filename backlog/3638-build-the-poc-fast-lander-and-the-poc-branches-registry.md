...
This is the follow-on build `#2260`'s ruling named and deliberately did not build. That decision settled the
design (transport **A′**, the `deliveryTarget:` field, and the registry built now rather than deferred); this
item is the implementation, including the blockers `#2260`'s own survey found.
