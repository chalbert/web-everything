...
- **Tests:** `scripts/autofix/__tests__/engine.test.mjs` extended — cost-bound budget/deferral, reference-free-of-budget, decide accept+revert, lineDiff/formatDiff (17 engine + 8 modelFixer = 25 pass). `check:standards` green (0 errors). CLI loads and runs (green path + flag validation).
- **Note:** an unrelated concurrent change to `scripts/readiness/engine.mjs` leaves one readiness test red (missing `tierBPrepared` in an expected object) — external, not from this item.
