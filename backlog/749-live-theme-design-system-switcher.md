...
Earlier batch pre-flights re-blocked this twice on "no concrete manifest registry/presets to switch between" — both **now cleared**: #775 (the creator/assembler open-core decision) is resolved, #747 ratified the bundle shape, and #871 built the `designSystems.json` registry + `/design-systems/` catalog + validator. The switcher itself is FUI-resident (see Progress) so it never reads WE's registry at runtime — the #809/#815 boundary forbids a WE↔FUI channel — it consumes the #747 manifest *shape* with FUI-owned preset data.
