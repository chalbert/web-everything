...
families as CLASSES (`loan-origination/app.ts`, `auto-insurance/app.ts`), so they couldn't be deleted until
those apps moved to FUI (**#823**, resolved 2026-06-17 — unblocked). When unblocked: delete the vendored
runtime impls (upstream in `@frontierui/blocks`); their `blocks.json` contracts already point to FUI (#657)
so leave them, and the generated `custom-elements.json` follows. #697 took the 3 genuinely-free families
(`type-ahead`, `background-task-surface`, `data-grid`).

## Grounding corrections (2026-06-17, at claim — verified against both trees)

The original body said "**6 families**, delete byte-identical, leave only `blocks.json`." Two corrections:

1. **`stepper` STAYS — it is NOT deletable (6 → 5 families).** `blocks/wizard/WizardElement.ts:132` does
...
2. **Delete the runtime impl ONLY; KEEP each `blocks/<f>/contract.ts`.** Post-#875/#878/#879 (all resolved
   2026-06-17), FUI deleted its own `contract.ts` copies and imports `@webeverything/contracts/<f>` — so
   **WE's `blocks/<f>/contract.ts` are now the canonical single source FUI depends on** (the FUI→WE arrow).
   ...
   impl files are safe to delete. (`custom-elements.json` is a pure projection of `blocks.json` via
...
`demos/auto-insurance/` (hosted in FUI by #835/#836; their `demos.json` entries already became `fuiDemoFile`
iframe embeds in #837, which stay). One STAY→delete edge to repoint first:
`blocks/renderers/audit-timeline/renderAuditTimeline.ts` type-imports `AuditEvent` from `audit/AuditProvider`
→ repoint to `audit/contract` (+ its unit test). No `plugs/bootstrap.ts` registrations exist for these
...
Done:
- **Deleted 5 runtime impl files** (kept each `blocks/<f>/contract.ts` — the canonical `@webeverything/contracts/<f>`
  surface FUI imports post-#875): `audit/AuditProvider.ts`, `lifecycle/LifecycleProvider.ts`,
  `master-detail/MasterDetailBehavior.ts`, `selection/SelectionBehavior.ts`, `tree-select/TreeSelectBehavior.ts`.
  Each `blocks/<f>/` dir now holds only `contract.ts`.
- **`stepper` retained** (grounding correction §1) — `wizard/WizardElement.ts` composes `StepperBehavior`
...
- **Repointed** `renderers/audit-timeline/renderAuditTimeline.ts` + its test: `AuditEvent` type import
...
- **Deleted the 2 WE-local exercise app dirs** `demos/loan-origination/` + `demos/auto-insurance/` (FUI-hosted
  by #835/#836; their `demos.json` entries are `fuiDemoFile` iframe embeds from #837 — left intact).
- **Removed** the two stale `routerDemoFallback` base-path rewrites from `vite.config.mts`.
- **Left untouched** (correct): `blocks.json` family contracts (point to FUI, #657), `custom-elements.json`
  (pure projection of `blocks.json` via `gen:cem` — unaffected), `contracts/*.ts`, `demos.json` embeds.
- **Doc-rot fix:** `docs/agent/demo-workflow.md` note that the two apps moved to FUI.
