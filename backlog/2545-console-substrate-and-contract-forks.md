...
real build — distinct from the visual grammar ([#2544]). Each changes the runner, the spec data model, or the
...
- **F3 — structured spec fields vs prose.** The proof rows (spec-proven R1..Rn) need a data model. Decide
  ...
  schema the proof backend ([#2553]) and the review modal consume.
...
F4→agent-context injection); [#2553] (proof backend) and [#2550] (board) can then build against settled
contracts.
