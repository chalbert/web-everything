...
**Fix:** in the integrator's closeout, reconcile each `carried` ledger entry back to `status: open` (drop `dateStarted`, or keep it as an attempt record), so unlanded items re-enter the next pack honestly. Distinct from the concurrent-id / merge self-healing work (that heals *colliding new items* at merge; this is *status reconciliation* for items that never merged at all). Surfaced by [[parallel-orchestrator-first-real-multilane-run]] / #1153; sibling to the orchestrator story #1147 and pre-lock slice #1945.
