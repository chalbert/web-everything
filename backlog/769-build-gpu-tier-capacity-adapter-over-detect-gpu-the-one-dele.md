...
- **Source/adapter:** [capabilities/capacity-gpu.ts](../capabilities/capacity-gpu.ts) — `GPU_CAPACITY_ADAPTER` (id `gpu`, dimensions `['gpuTier']`, non-native), `createGpuCapacitySource(probe?)` and `createGpuCapacityProvider(probe?)`. Registers as a **capacity resolver impl** (impl-is-not-a-standard) answering only `gpuTier` of the #767 contract.
...
- **Bucketing reconciled (#729 Fork 2):** the source emits the **raw tier 0–3** as scalar; `deriveBucket('gpuTier', …)` in capacity.ts now maps it centrally (3→high, 2→mid, 1/0→low), replacing the placeholder bucket-string branch (kept as back-compat). Scalar = tier, bucket = derived — consistent with every other dimension.
- **Tests:** [capabilities/__tests__/capacity-gpu.test.ts](../capabilities/__tests__/capacity-gpu.test.ts) — 7 cases (tier→bucket map, undefined-before-ready, gpuTier-only, probe-failure-stays-unknown, provider/adapter shape). Existing capacity suite still 15/15.
