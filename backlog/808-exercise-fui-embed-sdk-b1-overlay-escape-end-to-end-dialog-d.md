...
- **(2) Production build target** — `vite.config.mts` adds three rollup entries with stable unhashed
  names; `vite build` now emits `we:dist/embed/embed-host.js` / `we:embed-guest.js` / `we:in-document.js` for the
  published demos host (`FUI_DEMO_BASE`). Verified emitted.
- **(3) Mode C (#786) demo + CI proof** — `fui:demos/embed-dialog-in-document.ts` exports `mountInDocument(root)`;
  `fui:demos/embed-c-host.html` mounts it in-document (no iframe). `fui:embed/__tests__/e2e/embed-c-in-document.spec.ts`
  asserts the dialog escapes natively to the host top layer with zero coordination. **Passing.**

`we:playwright.config.ts` testMatch extended to `embed/**/__tests__/**`. `fui:embed/README.md` updated (the three
