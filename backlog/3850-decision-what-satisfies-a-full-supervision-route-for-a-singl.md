...
- **A fix or ci-heal pushes onto an existing PR.** `we:scripts/operations/fix-dispatch-wrapper.mjs` updates the bounced PR in place (`pushLaneRef`, `:579`); it does not open a new one. A `fix` already re-arms the review after its push (`rearmReview`, `:832`, `review:changes` to `review:pending`). A `ci-heal` is forbidden to touch any review label (`we:scripts/operations/ci-heal-dispatch-wrapper.mjs:34-35`, `:566-574`). An accept is stamped with the head SHA, so a head move makes it stale and re-parks the PR (the `restamp` target's note, `we:scripts/review-set-label.mjs:154-158`).
- **`executed` does not say who ran today.** It is the constant `EXECUTABLE_PROVIDER = 'claude'` (`we:scripts/lib/dispatch-contracts.mjs:759`, written at `:856`), while the delivery wrapper can really spawn Codex (`we:scripts/operations/deliver-item-wrapper.mjs:831`, `:922`). The #3717 child `3848` fixes this (#3801 *Settled by statute*), and #3784 is `blockedBy` it.
...
- **(a) The review panel on the lane's own PR, as a merge hold — recommended.** The dispatch records the supervisor as that PR review (so `supervisionHold` no longer holds it), and the obligation moves to land: the PR of a `full` route opens parked `review:pending` whatever its escalation score, and cannot land until the review panel records `review:accepted`. The reviewer is the `review-dispatch` panel (a fresh session with its own jurors), not converge's panel. For a `fix` or `ci-heal`, which pushes onto an existing PR, the hold is enforced at the land seam, not by the wrapper writing a label: a push by a bound route makes any earlier `review:accepted` stale (the existing head-SHA re-park), and a bound route's PR may not land on `ready-to-merge` alone. `ci-heal`'s rule never to touch a review label is unchanged. This is the "full independent review at `full`" rule 7 names: fresh validators who did not write the diff, as `#agent-convergence-independent-validation` requires, and it already exists as the parked path of today's review flow. Any provider may sit in the seat, a different one from the builder preferred. **Delegation trials:** the panel's verdict is the independent result the trial row is scored against. **Supervision:** at `spot-check` the PR takes the advisory floor instead, per rule 7 — the lowest level the ratified statute defines (`SUPERVISION_LEVELS` in `we:scripts/lib/provider-routing.mjs` has exactly two values, `full` and `spot-check`); rule 7's floor "gets shallower but never goes away," so no triple, however proven, ever reaches zero independent review under the ruling as it stands today. **Scope:** this fork governs `dispatch-lane` launches whose output is a standard PR to `main`; it has no bearing on epic #3383's own prototype delivery, which commits straight to `lane/mechanical-dispatcher` with no PR and no review pass of any shape (`we:docs/agent/backlog-workflow.md#delivery-target`). **Not branch or trust graduation:** the hold is about when #3784 switches this enforcement on for the mechanical dispatch path — not #3443's branch-to-main graduation or #3690's per-triple trust promotion; once enforcement is on, how long any one `{provider, model, taskType}` triple stays at `full` is governed entirely by #3690's rule 6 (`we:docs/agent/platform-decisions.md#delegation-trial-record-graduation`), not by this fork.
...
**Skeptic:** SURVIVES-WITH-AMENDMENT → applied (an independent skeptic pass re-read every citation on `5ab89f87b`; all accurate). Attack: "the review is post-hoc, so it is not supervision". Answer: rule 7 defines the independent pass by depth, not by timing, and the merge hold is what makes it binding; without it a `label-on-green` delegated PR lands unreviewed (FOUND). Attack: the first draft had the hold "apply to the existing PR after the push", which a `ci-heal` cannot do, because it may never touch a review label. Fixed: for `fix` and `ci-heal` the hold is enforced at the land seam through the existing head-SHA re-park, and the reviewer is named (the `review-dispatch` panel, not converge's).
...
- **(a) Every route whose executed vendor is not Claude — recommended.** The statute governs "a delegated agent's trial record" (its title and scope): the trust unit is a delegated triple, and a Claude-executed run is the baseline that record is compared with, not a delegated trial. Native Claude work keeps today's review policy (the escalation rubric and `#every-pr-gets-a-look-advisory-floor`). The binding reads the vendor the wrapper actually spawned: `executed`, once the child `3848` stamps it from the provider port instead of the constant (FOUND). Until then, a route is bound when either `routed` or the spawned provider is not Claude, so a marker-forced Codex run is bound even when the criteria routed Claude. "Vendor" is the agent vendor that ran the worker (its `#agent-vendor-registry` descriptor), not the model family, so a Claude model run through Antigravity counts as delegated: the trust unit's provider is Antigravity. The dispatch-time `supervisionHold` cannot know who will run, so it passes a Claude-executed `full` route itself and the land seam does the rest; otherwise native work stays held. **Delegation trials:** every delegated trial gets a full review until its triple is promoted. **Supervision:** unchanged for Claude routes.
