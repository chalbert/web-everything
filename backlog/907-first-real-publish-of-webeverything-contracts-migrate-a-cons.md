...
- **Registry/org absent** — `npm view @webeverything/contracts version` → `E404 Not Found`; the
  ...
  this environment).
- **Publish is a human/CI tag-push step** — the #877 pipeline (`.github/workflows/publish-contracts.yml`)
...
- **The consumer migration cascades from the publish** — FUI consumes the contract via vite alias +
  ...
  `frontierui/vite.config.mts:222`, `frontierui/tsconfig.json:51`). Pinning it to
