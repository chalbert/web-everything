...
- **Fork 2 (source vs bundle) is FUI-internal packaging.** WE's consumption is identical either way
  ...
  discriminator.
- **The pin resolves for free — including the transitive deps.** A pinned SHA/tag (D) or a locked dep (A) in
  `we:package-lock.json` *is* the declared-version pin `npm ci` reproduces, and it locks the **whole tree** —
  `happy-dom` **and** its CJS transitives (`iconv-lite`/`safer-buffer`) that the #1946 externality leaves
  unbundled — so render-skew from a drifting transitive can't recur. Closes the
  `we:scripts/lib/data-table-build-hook.cjs:32` "no version probe" gap regardless of channel. (Under the
  concurrent-session's vendor default the pin had to be re-invented to capture those transitives by hand;
  under A/D the lockfile captures them for free.)
