...
`we:backlog/xlv5507-a-card-resolve-pr-can-land-before-the-impl-it-names-in-gradu.md` is on `main` with a hash
...
1. **Executable** — a check that fails on a landed (on `main`) `we:backlog/` file whose id is still the
   `xNNNNNN` hash form. Red against the current tree while `xlv5507` remains hash-keyed, green once numbered.
