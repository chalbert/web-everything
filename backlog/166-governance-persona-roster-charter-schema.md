...
**Crux.** The item asks whether personas are "purely product-defaults, or can an org publish its own ('release-manager', 'support')." Today the roster is a fixed seven-element array ([plateau:profiles.ts:941](../../plateau-app/src/profiles/profiles.ts#L941)) with no extension seam. The SaaS RBAC literature gives a precise answer to this exact question.
...
**Crux.** The item asks whether the roster "lives in WE as a standard entity (like intents/protocols), or stays a plateau-app data file with the dev-browser importing it." Today it's a static TS array compiled into plateau-app ([plateau:profiles.ts:941](../../plateau-app/src/profiles/profiles.ts#L941)) — which the dev-browser **cannot import without coupling** to the SaaS app, yet the "charter drives the toggle-map" relationship requires both lenses to read one roster.
...
All seven are implemented as full charters in `plateau:plateau-app/src/profiles/profiles.ts` and render at `/profiles`. The charter schema (pure data):
...
- ✅ Four open questions reshaped into bold-defaulted forks (2026-06-11, prepared) — see report `we:reports/2026-06-11-governance-persona-charter-schema.md`.
