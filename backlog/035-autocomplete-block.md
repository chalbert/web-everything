...
Seed content already exists in the dropdown-trait-composition report under "Autocomplete in detail" (focus host vs collection, the controller param, type-ahead → filter, the typing trace) and in the trait-language report under "Autocomplete — swap one trait, move the focus host". Depends on landing the `controller` prototype on FocusDelegation (see [focus-delegation-controller](/backlog/029-focus-delegation-controller/)) before the runnable spec can be claimed.
