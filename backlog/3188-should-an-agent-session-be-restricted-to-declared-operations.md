...
ADDED 2026-09-21 (operator ruling). The words below were typed by the operator in the orchestrating session; they are quoted, not paraphrased. Asked: "Are declared operations exempt from 'the main session never runs shell commands itself'? My pick is yes." The operator answered: "Ok for operation". **Ruling:** a DECLARED OPERATION run through the operation runner (`node we:scripts/operations/run.mjs <op>`) is exempt from the rule that the main session never runs Bash itself. Ad hoc shell, investigation and edits still go to a subagent. Two standing statements from the same session, also typed by the operator and quoted: "very important to only use operation" (about dispatch: a worker is launched through a declared operation and no other way) and "note to change to only operation" (about the commands a dispatched worker may run: the end state is that workers may run declared operations only, under a committed per-dispatch-kind allow-list). **Consequence already built:** the `dispatch-task` operation (#3730), on the prototype branch `lane/mechanical-dispatcher`, commit 6869cdc0f (verified: that commit exists there, and `we:scripts/operations/dispatch-task.mjs` is not on `main` yet). **Scope of this entry:** it is a ruling on the question this card asks, for the operator's main session. It does NOT ratify the card, resolve it or set `codifiedIn`. The card stays open for its remaining scope: Fork 1 (the channel for the missing-operation escape), the egress validation gate, and the *Done when* items below. The workers' end state is a statement of direction; the per-kind allow-list is not built.
