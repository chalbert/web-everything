...
| 4 — when the janitor runs | **(a) on demand, when `list --acquirable` comes back thin** | (b) standing, every tick: more destructive passes than the pool needs |
...
- **(a) Exactly `isLeaseStale` returning false — recommended** (the least invasive: one definition, already used by the pool reaper). A lease younger than the stale window (default 240 minutes) blocks every destructive operation, whatever its session's name.
- **(b) A stricter signal for purpose-named sessions** (the `claude agents` listing, or a heartbeat). Rejected as the default: the signal does not exist yet (it is part of #3669's fix), and (a) already errs on the safe side: a live lease blocks the janitor for at most the stale window.
...
- **(a) On demand, when `list --acquirable` comes back thin — recommended** (the least invasive: it runs only when the pool needs lanes, so the fewest destructive passes).
- **(b) Standing, on every runner tick.** Rejected as the default: it recovers lanes before anyone needs them, but runs a destructive pass many more times than needed; it can be switched on later if thin pools recur.
