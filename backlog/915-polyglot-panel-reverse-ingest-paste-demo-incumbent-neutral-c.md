...
- **Vendored the #851 ingest adapter** FUI-side at `fui:tools/ingest-adapter/ingestComponent.mjs` —
  **byte-identical** copy of `we:scripts/ingest-adapter/ingestComponent.mjs` (the #892-analog
  vendoring, contracts-only precedent #855/#892), plus `fui:ingestComponent.d.ts` for a typed import
  (mirrors the gen-wrapper vendoring used by #753).
- **`fui:workbench/ingestPanel.ts`** — `mountIngestPanel(host)` (paste textarea seeded with a MUI-ish
...
- **`fui:vitest.config.ts`** — added a `workbench/**/__tests__/**/*.test.ts` unit glob (e2e specs are
  `*.spec.ts`, untouched).
