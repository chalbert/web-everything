...
**Grounding (✓ ready to ratify):** no gate existed yet for rendered-site a11y/UX. `check:standards` validates spec/data/backlog invariants and **skips the 11ty build** (runs no browser — `scripts/check-standards.mjs`), so rendered regressions stay green — [#762](/backlog/762-site-navs-lack-a-current-page-active-state-fui-header-we-doc/) (neither site nav carries `aria-current`, shipped silently) is the worked example. Surveyed the a11y-gating prior art (axe-core / pa11y-ci / Lighthouse CI, route-list patterns, WCAG-tag scoping) and published [/research/rendered-site-a11y-gating/](/research/rendered-site-a11y-gating/) (report: `reports/2026-06-16-rendered-site-a11y-gating.md`). **The survey reshaped the fork:** the repo *already* has a Playwright integration lane that reuses the running dev servers, so "axe over the built sites" is not new infra — that collapses the original A/B/C three-way (A static-lint and C DoD-checklist **coexist** with axe, they don't rival it) into **one genuine tool call + two sub-decisions** (posture, route set), each with a **bold** default below.
...
- **The gate today** has no a11y rule and skips the build: `scripts/check-standards.mjs` — its closest "route-list" precedent is the §9 Vite-proxy-allowlist cross-check at `scripts/check-standards.mjs:716`.
- **A Playwright lane already exists and reuses the running servers:** `playwright.config.ts` `webServer` boots `npm run dev` (Vite `:3000` proxying `/backlog/` → 11ty `:8080`) with `reuseExistingServer`, and `@playwright/test` is already a devDep (`playwright.config.ts:21-41`). `test:integration` = `playwright test`. **No `@axe-core/playwright` dep yet** — adding it is the whole new surface.
- **The two rendered sites live in different repos:** WE-docs `:8080` (this repo, `src/_layouts/base.njk`) and FUI `:3001` (`frontierui`, its own server) — so a gate in this repo natively covers only WE-docs; FUI needs a mirrored copy (the Spec-Explorer-dev-panel-duplicated pattern).
...
**Crux:** which engine observes the rendered pages for the real gate (contrast, focus order, names, computed current-state) — the coverage `check:standards` structurally can't provide (`scripts/check-standards.mjs` skips the build + runs no browser).

- **A — `@axe-core/playwright` into the existing Playwright project.** axe-core is the embeddable WCAG rule engine; `AxeBuilder` scopes by `.include()/.exclude()` and filters by `.withTags(['wcag2a','wcag2aa','wcag21a','wcag21aa'])`, failing on any violation. Reuses the already-wired harness (`playwright.config.ts` reuses the running dev server) — one new devDep + spec files + a route list, no new server/runner.
...
- **A — hand-maintained allowlist, mirrored per-repo.** An explicit, reviewed list of gated URLs, one copy per repo. Matches the existing §9 Vite-proxy-allowlist precedent (`scripts/check-standards.mjs:716`) and the duplicated-dev-panel pattern; a new page is gated only when added (a known, reviewable seam).
...
- **Fork 1 → A — `@axe-core/playwright` in the existing lane.** Confirmed. axe-core *is* the WCAG engine; the Playwright harness already reuses the running dev servers (`playwright.config.ts:24-41`, verified) and `@playwright/test` is already a devDep — so this is one new devDep + spec files + a route list, no new runner. pa11y (same axe engine, second harness) and Lighthouse (coarse aggregate a11y score, not per-rule WCAG) both lose on merit, not as live forks.
