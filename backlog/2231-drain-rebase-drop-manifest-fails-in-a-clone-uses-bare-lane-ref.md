...
isolated clone (the #2197 model): it passes the **bare** head-ref name to git — `git merge-tree --write-tree <base> <laneRef>` and `git commit-tree … -p <laneRef>` — but in a fresh clone the lane branch only exists as the
