...
**Resolved 2026-06-12** → extended the Windowed Collection intent's **Set-Size Semantics** section in `we:src/_data/intents.json`. Added the *Indeterminate totals (progressive load)* paragraph: while the count is unknown (`infinite` data source / cursor pagination / before the last page lands) every rendered item carries `aria-setsize="-1"` (the platform size-unknown sentinel), switching to the real count once known; the container sets `aria-busy="true"` during batch row insertion, flipped to `false` on completion — following the WAI-ARIA APG Feed pattern rather than fabricating an estimated total. Thin spec touch-up as ratified (#018 Fork 4); `check:standards` green.
