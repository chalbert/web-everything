...
- **Tests:** 6 new cases in `blocks/__tests__/unit/renderers/jsxToHtml.test.ts` (named round-trip, member-path, react-dialect lowercasing, closure→lossy, verbatim string attr, string-API delegation). Full JSX renderer suite green: 122 tests across 5 files; `check:standards` 0 errors.
