...
- **AST library picked: the TypeScript Compiler API** (`typescript`, already in `frontierui` deps) for the imperative/class side — the genuine AST need; `parseImperative` walks the `SourceFile`. The declarative/HTML side uses a minimal single-element tag reader (the documented swap seam: replace with parse5/an HTML AST when nested-element fidelity is needed). The **IR** (`ir.ts`: `{ name, shadow, templateHTML }`) is the neutral seam both directions route through — grow the contract by adding an IR field + its paired parse/emit rule.
- **Files (frontierui/compiler/src/component-transform/):** `ir.ts` (IR + `pascal`), `declarative.ts` (`parseDeclarative`/`emitDeclarative`), `imperative.ts` (`parseImperative`/`emitImperative` via TS AST), `index.ts` (wires `transform()`; failures return structured `errors`, not throws).
- **Tests:** `transform.test.ts` refactored into a reusable `contract(slug, …)` helper (4 assertions: both lowerings + both round-trips). Two fixture pairs green: `x-empty` (default-shadow, empty template) and `user-card` (light DOM `shadow="none"`, non-empty `<slot>` template, multi-segment pascal name) — **8/8 passing**. Also added `compiler/**/__tests__/**` to `vitest.config.ts` include globs: the red tests (and pre-existing dormant `compiler/__tests__/*` suites) weren't being discovered at all before. Full frontierui suite 1254 passing; `tsc -p compiler/tsconfig.json` clean; `check:standards` 0/0.
- **Note (no overlap with #076):** the actual `<component>` ↔ class lowering that shipped lives in *webeverything* (`blocks/renderers/component/declarativeComponent.ts`, exercised by #076). This item is the separate *frontierui AST-library, byte-identical round-trip* compiler. The frontierui canonical class shape differs from webeverything's (`const root = this.shadowRoot ?? …` rather than a `#root` field) — the fixture defines frontierui's canon.
