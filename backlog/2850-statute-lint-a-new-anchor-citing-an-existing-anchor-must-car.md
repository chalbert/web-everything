...
`anchorSubstance` (`:143`) already knows how to slice an anchor's body — *from its definition line to the next
heading* — so "the new anchor's body" needs no new parsing; reuse it rather than re-deriving a span.
