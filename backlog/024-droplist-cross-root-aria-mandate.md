...
Settling A vs B is the upstream fork for the droplist family: [#020 base-select first-class adapter](../backlog/020-base-select-first-class-adapter.md) (base-select becomes a sanctioned enhancement path), [#064 tree-select block](../backlog/064-tree-select-block.md) (same a11y model), [#018 dropdown async pagination](../backlog/018-dropdown-async-pagination-paradigm.md), and the droplist-native-substrate-fork resolver tiebreak.
...
This ratifies what `frontierui/blocks/droplist/` already ships (light-DOM trigger↔listbox wiring in a shared root). **Unblocks** [#020](../backlog/020-base-select-first-class-adapter.md) (base-select is a sanctioned co-equal substrate — promotion to a first-class adapter is now purely an ergonomics call), [#064](../backlog/064-tree-select-block.md) (inherits the outcome-not-mechanism model), [#018](../backlog/018-dropdown-async-pagination-paradigm.md). Enforcement (does the conformance checker actually test the trigger↔listbox a11y relationship) is a separate tooling concern, not part of this contract ruling.
