...
| --- | --- | --- | --- |
| Fork 1 — surface: what tools may every reviewer hold | **Declared operations only**, as MCP tools; built-ins removed; four missing operations declared. Raw tools stay the measured escalation | Raw shell and writes for every reviewer | Medium (regrounded on merit after the red team) |
| Fork 2 — containment: where the model and its operation server run | **Container** for every tool-bearing seat, as a backend-neutral isolation provider; never a pooled lane | Host throwaway full clone, keyed to enforceability (host only where the tool list is preventively verified) | Medium-low |
...
**Forks 1 and 2 are two independent choices that stack, not rivals.** Fork 1 is the *surface* (which tools the model holds). Fork 2 is the *containment* (what boundary the model and its operation server sit inside). The recommended combination is **operations-only inside a container**: the model's only tools are typed calls, and the container bounds whatever a provider's tool removal cannot prove. Fork 2 constrains Fork 1 in one direction only: a raw shell cannot be confined on the host, so a raw surface forces a container. The reverse does not hold.

| Surface (Fork 1) | Host throwaway clone | Container |
| --- | --- | --- |
| **Operations only** | Feasible; acceptable only where the tool list is preventively verified (Claude). Detective-only for Codex, unreducible for Antigravity | **Recommended.** Typed, unforgeable results and an audit log, plus a boundary independent of the provider |
| **Raw shell and writes** | **Not feasible** — a shell cannot be confined on the host | The measured escalation, not the default (network exfiltration, in-guest token, unverifiable claims; see Fork 1 (c)) |
...
## Fork 1 — Surface: what tools may every reviewer hold?
...
## Fork 2 — Containment: where do the model and its operation server run?

**Fork-existence.** The model and the operation server it calls sit inside exactly one boundary per review, and a pooled lane, a host throwaway clone and a container are different trust placements. Where the PR's own suite executes is a separate question, not decided here (see "Supported by default"). The excluded branch is the pooled lane: `mutation-check` restores in a `finally` but a crash can leave a mutant behind, and a dirtied pooled lane is later leased to unrelated work.
...
- **Folded (2026-09-21, follow-up commit):** Fork 1 is now the *surface* (operations versus raw) and Fork 2 the *containment* of the model and its operation server (host versus container), with the cell table under "Recommended path at a glance". Placement of the suite's own execution moved out of the forks into "Supported by default".
...
- **The PR's own suite executes in a container wherever one is available**, through `we:scripts/readiness/heavy-admission.mjs run --container`, independent of where the model and operation server sit. Both placements coexist, so this is not a fork; the `run-suite` operation routes through that path.
