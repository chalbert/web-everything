...
preparedAgainstSha: "bafcda8f3b8eab9865f5421e562b6dd608940748"
...
| --- | --- | --- | --- |
| Fork 1 — tool ceiling for every reviewer | **Declared operations only**, as MCP tools; built-ins removed; four missing operations declared | Raw shell and writes in a container, for every reviewer | Med-high |
| Fork 2 — where the review runs | **Host throwaway full clone**, never a pooled lane | Container with a mounted throwaway clone | Med |
...
Fork 2 depends on Fork 1: if Fork 1 is not (a), Fork 2 must be (a), because a model with a shell cannot be confined on the host.
...
- **(c) Raw shell and writes in a container, for every reviewer** (the filing's ask, raised to the common surface). The maximum-capability branch and legal under #3001 (writes inside an own clone are allowed). Loses because it needs a container that runs the Codex CLI, which does not exist, to make reads safe; its only audit trail is a transcript; and it buys power the mandate's mutation rule does not ask for (see the table). It remains the pre-declared escalation.
...
- **(a) Container, mounted throwaway clone.** VM boundary that also contains the PR's own test code. Loses as the *default* because nothing runs the Codex CLI in a container yet, it needs the auth bridge and a baked Linux dependency tree (measured costs in Context: about 3.6× on mount I/O, about 378 MB per idle guest), and under Fork 1 (a) the model cannot reach the host anyway. It is the required branch if Fork 1 is (c).
- **(b) Host throwaway full clone** — **recommended** under Fork 1 (a): a `git clone file://` of the PR head in the temp directory, history-stripped, with the Fork 1 server pinned to it and torn down after the run. "Full" means siblings too: the suite resolves `../frontierui` (`we:vitest.shared.ts`), so the clone's parent needs that sibling. The PR's test code still runs on the host, as it does for Claude today; the suite command can route through the existing container path (`we:scripts/readiness/heavy-admission.mjs run --container`) without changing this fork.
...
**Skeptic:** SURVIVES-WITH-AMENDMENT (self-run). The attack from the filing's own preference ("container, not bare-host"): under Fork 1 (a) the model has no shell or file tool, so container-versus-host only changes where a hostile PR's test code executes, and that risk exists today for Claude's juror. Amendment: (b) is the default *only with* Fork 1 (a), and the container path for suite execution is the named upgrade. Unverified: the native deny profile under a denied root that contains the lane (`~/workspace/.lanes/`); probe 14 used a temp-directory clone, which (b) also uses.
...
- **Sequencing:** only Fork 2 (a) depends on the capacity work in [#3611](/backlog/3611-hardware-usage-aware-heavy-command-capacity-control-a-staged/) and [#3610](/backlog/3610-how-far-to-take-heavy-command-admission-control-beyond-stage/); the recommended path does not.
...
[#3621](/backlog/3621-real-os-level-resource-isolation-per-dispatched-lane-is-appl/) (still an open decision; nothing on the recommended path depends on its ruling) measured, for a Linux guest on Apple's `container`: mount-bound I/O about 3.6× slower than the host, about 378 MB of host memory per idle guest, a Linux-built dependency tree needed per image, and subscription auth for Claude and Codex working from inside a container. Nothing yet runs the Codex CLI in one.
