...
- **Validator:** `validateTemplateA11y(layouts)` in [scripts/check-standards-rules.mjs](../scripts/check-standards-rules.mjs) (pure, unit-tested per #256), wired as §11 in [scripts/check-standards.mjs](../scripts/check-standards.mjs) reading `src/_layouts/*`.
...
- **Rules (mirroring the #636 warn→enforce shape):**
  ...
  - nav active-state wiring (#762 class) — a `<nav>` with a hardcoded `<a href=` list MUST wire `aria-current` → **warn** behind `NAV_ACTIVE_STATE_ENFORCED=false`. base.njk wires it; the dead/legacy `src/_layouts/base.html` (no `layout: base.html` refs — all 28 pages use base.njk) still has a 7-link nav with no aria-current, so enforcing now would red-gate a pre-existing miss.
- **Tests:** 4 cases in [scripts/__tests__/check-standards-rules.test.mjs](../scripts/__tests__/check-standards-rules.test.mjs) (pass; lang/title/main error path, #762 warn path, partial-template skip).
- **Verified:** `check:standards` → 0 errors; the new lint correctly warns on dead base.html. `vitest -t validateTemplateA11y` → 4/4 pass.
- **Carved:** delete dead base.html + flip `NAV_ACTIVE_STATE_ENFORCED` to enforce (#794).
