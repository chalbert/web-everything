...
- Spliced into `src/_data/blocks.json` via bracket-matched text insertion (no full re-stringify — keeps the diff localized to pure additions, per the mixed-escaping footgun). Verified via a scratch 11ty build that the panel renders on a backfilled block (`dialog`).
