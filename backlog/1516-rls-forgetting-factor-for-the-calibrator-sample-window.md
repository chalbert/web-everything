...
`vitest` 16/16 green; `check:standards` 0 errors; `backlog.mjs` parses. Pure-function change behind the existing calibrate CLI — no behavioural break for callers (the default is the ratified active forgetting; the only stored-schema additions are `forgetting` + `fit.nEff`).
