...
> **Superseded 2026-07-09 by #2238** — the pre-commit verdict-mtime gate + `we:scripts/check-visual-baseline-update.mjs`/`we:scripts/dev/visual-baseline-review.mjs` + `check:visual:guard`/`visual:review` npm scripts are RETIRED; baseline refresh now flows through the PR-based `we:.github/workflows/update-visual-baselines.yml`. See #2238.
