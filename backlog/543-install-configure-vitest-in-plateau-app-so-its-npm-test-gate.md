...
(`technical-configurator/compat.test.ts` + `nl-provider.test.ts`, `platform-manager/aggregator.test.ts`,
`compatibility-map/compute.test.ts`), but **vitest is not a dependency** — `npm test` fails with
...
1. Add `vitest` (+ `happy-dom` or `jsdom` for the DOM-touching suites) to plateau-app devDependencies and
   `npm install`.
2. Add a `vitest.config.ts` (or `test` block in `vite.config.mts`) with the DOM environment and the `@we/*`
