...
- **WE export delta** — `webcases/package.json` (`@webeverything/webcases`, exports
  `./requirementValidator`), mirroring the #814 scoped-package pattern (`name == specifier`, #239). FUI
  `tsconfig.json` paths + `vite.config.mts` alias map the specifier to the sibling source. The contract is
  imported **type-only** (erased at runtime; the record never leaves WE).
- **Adapter (impl → FUI per #463/#552)** — `frontierui/adapters/requirement-as-code/gherkinEars.ts`:
...
- **New FUI adapter home** — established `adapters/` as the impl home for #463/#552 interop adapters;
  added to FUI `vitest.config.ts` include + `tsconfig.json` include (typecheck-covered).
- **Tests** — `adapters/requirement-as-code/__tests__/gherkinEars.test.ts` (9): emit shape (Gherkin +
