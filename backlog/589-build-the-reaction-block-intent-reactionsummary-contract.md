...
- **Resolved 2026-06-14.** Authored the reaction standard per #370 Fork 5 — **block + intent**, the
  ...
  - **Reaction intent** (`src/_data/intents.json`, footgun-safe splice) — dimensions `reactionSet`
  ...
  - **Reaction block** (`src/_data/blocks.json` + `src/_includes/block-descriptions/reaction.njk`) —
  ...
  - **Verified:** both pages render (/intents/reaction/, /blocks/reaction/ — 11ty smoke 1541 pages);
    `check:standards` green; AGENTS.md inventory regen folds in the new block + intent counts.
