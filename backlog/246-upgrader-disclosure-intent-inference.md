...
rules in `we:blocks/renderers/upgrader/analyzers/legacyWebComponent.ts`. Its third worked example — a
...
Inferring an intent that doesn't resolve in `we:intents.json` would fail the upgrader's verify gate, so the
...
- Add a shared fixture case to `we:blocks/renderers/upgrader/__fixtures__/upgrader-cases.ts`
...
- **Status:** resolved (2026-06-10). #008 shipped `disclosure` in `we:intents.json`, so the deferred
  third rule was implemented:
  - **Rule** — `inferIntents()` in `we:blocks/renderers/upgrader/analyzers/legacyWebComponent.ts` now
  ...
  - **Fixture** — added shared case `disclosure-intent` (`expectIntents: ['disclosure']`) to
    `we:blocks/renderers/upgrader/__fixtures__/upgrader-cases.ts`, so the data-driven suite + the Code
    Upgrader playground exercise the same input.
  - **Demo** — `disclosure` added to `knownReferenceIntents` in `we:demos/code-upgrader-demo.ts`.
  - **Tests** — three dedicated cases in `we:blocks/__tests__/unit/renderers/upgrader.test.ts` (positive
