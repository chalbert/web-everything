...
- Unit test `RadioElements.test.ts` (2): the group renders a `role=radiogroup` of named native radios with the initial `value` selected; standalone `we-radio`s persist and a real `<form>` reads the checked one by shared name (FormData round-trip — the Fork-1-B form-participation proof). 16/16 radio unit tests green; FUI `check:standards` 0 errors.
