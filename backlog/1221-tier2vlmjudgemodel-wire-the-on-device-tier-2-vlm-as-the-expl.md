...
- **`playwrightTier2Analyzer(vlmPage, fallbackCapture)`** — the device-gated bridge: proxies a screenshot into `vlmPage.evaluate(() => window.__tier2AnalyzeRich(png))` (the #1142 in-browser WebGPU/Transformers.js Florence-2 path) and reads the envelope out. Node side holds no SDK/model (no-leakage #475). WebGPU + ~2 GB download → unrunnable in CI, so it's injected + NOT unit-covered; the mapping it feeds IS.
