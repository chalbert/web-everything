...
A `<component>` with no children is meaningful — it defines a custom element whose render produces nothing. The bidirectional transform desugars `<component name="x-foo"></component>` to the same lowered class it emits for the explicit-empty form. Current recommendation: keep the shorthand built into the transform until (and unless) a native declarative-custom-element standard lands that mandates an explicit `<template>` child. Alternative held open: require the explicit `<template>`, treating its omission as a transform error to stay 1:1 with whatever spec ships. Only the canonical (omitted) form is byte-identical on round-trip; the explicit-empty normalization is a sub-decision deferred to the first non-empty fixture.
