...
**The `deferred` bucket is the expensive one, and it is a third push-site the list above does not cover.**
`deferred` entries are minted inside `planLabelDrain` (`deferred.push({ num: c.num, item: c.item, waitOn, … })`),
where `c` IS the verdict — so the field is available, but roughly **19** existing assertions in
`we:scripts/__tests__/merge-ai-prs.test.mjs` pin that exact object shape with `toEqual`, and every one of
them has to gain the new key. Budget for that; it is most of the WE diff. (Raised by the independent review
below.) Note there is also a **second, unrelated** `deferred.push` shape — `{ id, reason }` for a couple half
still open — which carries no `num` at all; `parsePassResult` only reads entries where `Number.isInteger(it.num)`,
so that shape is already invisible to the journal and must be left alone.
...
**And name the population before picking `K`, because the drain re-heads tips itself.** This is the trap:
the sweep's own rebase-drop mints a new tip commit and force-pushes it to the `lane/*` ref every time it
rebuilds a candidate — that IS a head-SHA change, produced by the detector's own host, and #2460 documents
a single PR churned ~10 times that way. JIT renumbering at land moves heads too. So "head SHA changed N
times" is NOT synonymous with "the lane is thrashing", and a naive `K` would fire loudest on the PRs the
drain itself is busiest with. The detector has to either exclude drain-authored re-heads (the sweep already
tags them — `v.rebaseDrop === 'rebased'`) or set `K` above the drain's own rate. Say which.

**`K` cannot be calibrated before the emit lands** — that is exactly why #2487 deferred this — so the
detector half must be sized against real journal data collected after the WE emit is live, not guessed at
authoring time. Treat "collect a window of real passes, then pick `K`" as a step, not a formality.
...
- Every considered-PR entry in the sweep's emitted JSON carries the head SHA — `toMerge`, `merged`,
  `skipped`, `parked`, `deferred` and `failed` alike (the `{ id, reason }` couple-half defer shape excepted:
  it has no `num` and the journal never reads it). Cheap check on a `--dry-run --json` pass: no entry
  carrying a `num` lacks the field.
...
- The plateau lib's own suite gains three cases that fail before and pass after: `parsePassResult` persists
  the per-PR head SHA on the journal entry; the churn incident fires for a PR whose head SHA changes more
  than `K` times across the window while NOT firing for one that is merely stalled (distinct from the #2487
  no-merge-progress signal); and it does NOT fire for a PR whose head moved only because the drain rebuilt
  it. (`npm test` in the plateau checkout.)
- `K` is chosen from measured journal data collected after the WE emit landed — the PR records the observed
  head-SHA-change distribution it was set against, including the drain's own re-head rate.
...
- **premise** (addressed; strategy: verify by mutation or reversion, ahead of implementation) — prHeadSha = p.headRefOid already runs in production today as the cross-pass read-cache key (we:scripts/merge-ai-prs.mjs lines 2627, 2688, 2861), so the field's reliability is already verified by live use, not merely assumed.
