...
- Added [traitServePath.ts](../blocks/renderers/module-service/traitServePath.ts) — the framework-agnostic integration:
...
- Tests [traitServePath.test.ts](../blocks/__tests__/unit/renderers/traitServePath.test.ts) — 7/7 green (path build incl. pin/base-path, eager-inline vs lazy vs preload bucketing, immutable/floating cache, determinism).
