...
[we:blocks/adapters/report/ingestReport.ts](../blocks/adapters/report/ingestReport.ts) — the inverse of the
...
- **`fromCoverage`** — Istanbul/nyc `we:coverage-summary.json` → Report: per-file × per-metric `Score`s keyed
...
Tests: [we:blocks/__tests__/unit/adapters/reportIngest.test.ts](../blocks/__tests__/unit/adapters/reportIngest.test.ts)
