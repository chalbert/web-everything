...
- **Done (all 4 scope points):**
  - **Keyboard model** — `blocks/renderers/data-grid/renderDataGrid.ts`: `nextCellPosition` (arrows clamp, Home/End, Ctrl+Home/End, PageUp/Down by pageSize), `renderDataGrid` (`role=grid`/`row`/`columnheader`/`gridcell` + `aria-rowcount`/`colcount`/`rowindex`/`colindex`), `setActiveCell` (roving tabindex), `auditDataGrid`.
  ...
  - **Conformance demo + suite** — `blocks/renderers/data-grid/__fixtures__/data-grid-cases.ts` (8 cases) shared by `blocks/__tests__/unit/renderers/data-grid.test.ts` (22 tests) and `demos/data-grid-demo.{html,ts,css}` (8 static cards + 1 live keyboard-nav card). Registered: `blocks.json` entry, `block-descriptions/data-grid.njk`, `demos.json` playground.
  - **Green:** 22 data-grid tests + full suite 1418 pass; `check:standards` 0 errors; 11ty build wrote `/blocks/data-grid/` + `/demos/data-grid-demo.html`; playground e2e green on live :3000.
