...
2. **Live** — open the deployed /wip page (or the dev route) with a real in-flight PR and see, on that PR's
   card, both its review job (`role: review`, from #4198's `item-activity`) and its fixer session (`role:
   fix`), each with state, times, outcome, and a transcript link that opens to a real, non-empty transcript or
   log file — the exact case #4198's own live check proves, now visible on the page instead of only on the
   command line.
