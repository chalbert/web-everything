...
- **protocols.json** — added the `custom-elements-manifest` protocol (`ownedByProject: webdocs`, `anchor: protocol-custom-elements-manifest`, `status: draft`), with the contract body section in `we:src/_includes/project-webdocs.njk` (the gate probes the anchor). Renders in `/protocols/` and on the webdocs project page (build smoke confirmed).
- **Emit pipeline** — `we:scripts/gen-cem.mjs` + `npm run gen:cem` projects `fui:blocks.json` block-protocol fields (events/exports, with implementsIntent/traits/webStandards under a namespaced `x-webeverything` extension) into a CEM 2.1.0 `we:custom-elements.json` at the repo root (canonical location; discoverable via the new `we:package.json "customElements"` field). Deterministic (sorted by id, no timestamp) → no-op re-run diff. Generated for all 64 blocks (4 with events; 0 custom-element decls — see below).
