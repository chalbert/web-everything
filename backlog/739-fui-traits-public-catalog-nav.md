...
- Added `src/_data/traits.json` — registry of all 13 traits grouped by home: 4 reference capability traits (sortable/highlight/revealable/polling, with delivery badges) + 3 resource-loader strategy traits + 6 background-task-surface strategy traits. (No traits carry `@internal`, so none curated out; grouping conveys the per-block scope.)
