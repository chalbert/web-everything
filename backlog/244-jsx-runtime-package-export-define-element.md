...
(`blocks/renderers/jsx/index.ts`) now re-exports `defineElement` + the `AutoDefineStrategy` contract
from `blocks/renderers/auto-define/`.

**The gap:** the published **package copy** `frontierui/packages/jsx-runtime/src/index.ts` exports only
...
- Add `defineElement` (and re-export the `AutoDefineStrategy` / `RegistryScope` / `DefiningModule`
  contract types) from `frontierui/packages/jsx-runtime/src/index.ts`, mirroring the canonical
...
- **Done:**
  - Carried `frontierui/packages/jsx-runtime/src/auto-define.ts` (verbatim copy of canonical
    `blocks/renderers/auto-define/defineElement.ts` — self-contained, no imports).
  - `packages/jsx-runtime/src/index.ts` now re-exports `defineElement`, `explicitAutoDefine`, and the
    `AutoDefineStrategy`/`AutoDefineTrigger`/`RegistryScope`/`DefiningModule` types.
  - `packages/__tests__/extraction-smoke.test.ts` adds 3 cases: idempotent `defineElement`, the
