...
`we:reports/2026-06-16-backlog-split-analysis.md`) — investigation found **two of the original four
deliverables already landed**: #737 (resolved) filled the authored `fui:blocks.json` entries (23 entries),
and the render is already manifest-driven (`fui:frontierui/src/blocks.njk` iterates `fui:blocks.json`). What
...
- **#784** *(story·3, blocked-by #783)* — add the completeness gate to
  `fui:frontierui/scripts/check-standards.mjs` (`readdirSync(blocks/)` family check) + reconcile any residual
  entries.
