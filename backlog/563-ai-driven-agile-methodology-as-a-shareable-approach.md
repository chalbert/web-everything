...
- ✅ Artifact-shape fork **resolved** in [#569](/backlog/569-artifact-shape-for-the-ai-driven-agile-methodology-playbook-/) (2026-06-22, ratified): the living repo is the source; the canonical authored artifact is **(B) the extracted starter-kit**, **(A) prose** its narrative companion, **(C)/(D)** further surfaces. Scope = *canonicity, not content-freeze* — *what* ships is fixed near release (#143 caution).
- ⏸ Decomposition sliced (2026-06-23, [we:report](../reports/2026-06-23-563-slice-analysis.md)) into two **`maturityGated` parks** — the authoring is worse-if-built now (content-freeze + churn against the live source): **[#1678]** extract the starter-kit (B), **[#1679]** author the narrative companion (A, `blockedBy` #1678). Both un-park near release; (C)/(D) surfaces file on pickup. Resolves when the kit + companion land.
