...
relatedTo: ["3656", "3639", "3443", "3638", "3643"]
...
`#3656` ruled where a POC branch's work lands and when review happens, never how a branch keeps its OWN running
...
## Why this is not already answered by `#3656` or `#3639` — checked, not assumed

**`#3656` (resolved).** Read end to end. It rules five forks — transport (`A′`, the lock-serialized
...
| Delivery target | WHERE an item's work lands | `#3656` |
...
them (`#3656`'s "SUPERSEDED, kept for the record" blocks are the best example in the repo); and its doctrine
...
4. **It is a single append point.** `#3656` exists to allow N fast concurrent landings into one branch; one
...
This is `#3656`'s own test applied one axis over: skip the ceremony *inside* the branch, pay it in full at the
...
`main`'s behaviour for days before being promoted. (B) alone contradicts `#3656`'s ruling — per-increment
...
**The cost is small but real, and is not the same as adding a backlog field.** `#3656`'s survey found backlog
...
graduation. Given `#3656` ruled N concurrent POC branches, that price is paid N times, which is the strongest
...
- **Parent `#3383`**, deliberately, matching `#3656` and `#3639` rather than nesting under `#3656`. Reasons:
  `#3656` is `status: resolved` and already has its build child (`#3638`), so a new decision under it reads as
...
- **`#3656`** — ruled the delivery mode this card records the state of. Not superseded, not duplicated.
