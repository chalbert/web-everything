...
- **"FUI already holds its own copy" is only partial.** `frontierui/tools/trait-enforcer/` has just
  `vite-plugin.ts` + `virtual.d.ts` — **not** `traitManifestContract.ts`, the rollup/webpack/esbuild/parcel
  plugins, or `composedTraitSet.ts`. A move must ADD those to FUI, not delete a duplicate.
- **WE consumes the pieces marked for removal.** `blocks/renderers/module-service/traitServePath.ts` imports
  `traitManifestContract` **types** (the MaaS serve-path); `plugs/bootstrap.ts` + `tools/maas/vite-plugin.ts`
