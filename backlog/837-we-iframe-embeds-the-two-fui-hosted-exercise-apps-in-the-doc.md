...
- **demos.json** — the two existing entries (`loan-origination`, `auto-insurance`) had their stale `liveUrl` (`http://localhost:3000/demos/…` — the WE-local copy slated for deletion in #824) replaced with `fuiDemoFile` (`loan-origination.html` / `auto-insurance.html`, the #835/#836 flat FUI entries) + `fuiDemoHeight: 680`.
...
Resolving this frees the app-coupled deletion **#824** (the WE-local app copies + their demos.json/blocks.json refs can now be removed) and closes the #823 slice set.
