...
- `we:scripts/workflows/review-parked-prs.mjs` is launchable: `export const meta` is a pure object literal with
  ...
  re-verifying it as new work. (The literal PR #1031 did not land; a smaller carved-out fix did. The report's
  step 1 is satisfied by the *effect*, not by that PR number.)
- The report's critical-path **step 3 leaves no repo-state trace, so it must be asserted explicitly**: the
  converge loop's false-positive rate has been measured on a control PR and the number is written down. The
  report calls it decisive ("below ~10% findings get investigated; above ~30% they get triaged with suspicion;
  above 50% they are dismissed by default") and we do not know ours. The other bullets can all read true while
  this never happened — that is exactly how the one check worth keeping gets retired with the report.
...
**Finding applied after this review** (accepted): the report's critical-path **step 3** — measure the converge loop's false-positive rate on a control PR — leaves no repo-state trace, so all four original bullets could read true while it never happened. It is now its own criterion.
