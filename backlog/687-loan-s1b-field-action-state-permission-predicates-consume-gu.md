...
- **2026-06-15 — built + verified.** New `demos/loan-origination/domain/permissions.ts` maps the three
  scopes onto the shipping Guard protocol (`guard/provider.ts`, #178) as a `LoanGuardProvider`
  (`CustomGuardProvider`):
...
- **Wired** in `app.ts`: a `CustomGuardRegistry` with the provider registered as default, exposed as a
...
- **Verified:** `demos/loan-origination/domain/__tests__/permissions.test.ts` (18 tests) — the demoable
  across roles/states/fields/actions + the provider resolution + ownership + engine wall-off. Added a
  `demos/**/__tests__/**` include to `vitest.config.ts` so exercise-app domain logic runs in the suite.
