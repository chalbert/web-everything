...
- **Front B also reviews ADHERENCE to the deterministic-core rule** ([we:docs/agent/platform-decisions.md#deterministic-core-thin-judgment](docs/agent/platform-decisions.md#deterministic-core-thin-judgment), #2607): the watch surfaces judgment-work in the loop that has become script-decidable — a latency *and* drift source (model re-derivation is slower than a script and non-reproducible between ticks) — and files conversion items (move the rule into a tested `we:scripts` script; skills and UIs shell it).
