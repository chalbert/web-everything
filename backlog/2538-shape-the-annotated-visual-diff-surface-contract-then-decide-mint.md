...
**Research:** [/research/annotated-visual-diff-surface/](/research/annotated-visual-diff-surface/) (grounding in `we:reports/2026-07-18-annotated-visual-diff-surface.md`). Surveyed the shipped models — Percy (approve/reject, promotes baseline), Chromatic (accept/deny per story + region highlights), reg-suit/reg-cli (categorizes items new/passed/changed/deleted; 2-up/swipe/blend/diff-mask UI), Playwright `toHaveScreenshot` (pixel diff-mask + `maxDiffPixelRatio`), GitHub/GitLab PR diff (structural file+line hunks), Figma version-compare (structural layer/node-id) — all measured against `we:src/_data/intents/audit-timeline.json`.
...
- **(a — recommended) TWO orthogonal axes:** a structural `type` (`added | removed | changed`) × a review `disposition` (`unreviewed | accepted | rejected | expected`). Matches reg-suit's type + Chromatic/Percy's verdict, and first-classes the board's `expected`.
...
- **(a — recommended) A PER-REGION disposition** that promotes the built state into the baseline on `accepted` (Percy/Chromatic semantics), first-classes `expected` (park as known-pending, neither fail nor promote), and may be scoped by type. The whole-surface approve composes *above* it, never replaces it.
...
**Recommendation to the ratification turn:** ratify the column — **MINT a `visual-diff` intent** on the shaped contract (Forks B/C/D defaults), scoped to the intent with the differ protocol named as a follow-on (Fork E). Override any fork.
