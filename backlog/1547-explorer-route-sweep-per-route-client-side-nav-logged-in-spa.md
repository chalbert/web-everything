...
This was the last capability the bespoke `plateau-app:tools/explorer/plateau-audit.ts` had over the CLI, so that harness was **deleted** — the CLI reproduces it from an app-agnostic recipe (`--auth` steps + `routes` click-list + `--out` + `viewports`).
