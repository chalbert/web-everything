...
- **Authoring construct** [composedTraitSet.ts](../tools/trait-enforcer/composedTraitSet.ts) — first-class component-level trait-set declaration over the #716 `TraitMap`:
...
- **Production build-chunk assertion** [composedTraitSet.test.ts](../tools/trait-enforcer/__tests__/composedTraitSet.test.ts) — runs a **real Rollup production build** (not a dev server, not the per-usage Playwright check) over the Enforcer's generated manifest and proves: a *bound* trait emits a split chunk, an *unbound* trait emits **zero** chunk (it never appears in the manifest, so the bundler never sees its `import()`), and both ship when both are used. 7/7 vitest green; `check:standards` 0 errors.
