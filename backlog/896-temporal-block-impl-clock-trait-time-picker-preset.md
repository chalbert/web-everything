...
- **Trait** `frontierui/blocks/temporal/traits/Clock.ts` — standalone `CustomAttribute` mixin (same shape as
...
- **Catalog** — registered `clock` in `src/_data/traits.json` (temporal block traits group; the `temporal`
  family entry from #736 already covers blocks.json).
- **Tests** — `blocks/temporal/traits/__tests__/Clock.test.ts` (5 tests) green; FUI `check:standards` green.
