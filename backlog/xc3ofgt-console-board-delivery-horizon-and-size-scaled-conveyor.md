...
The scope-lease zones [#xzsx09z] are bands laid out *along* this axis (its **running** band is this slice's
live conveyor region), so [#xzsx09z] builds on the axis established here rather than defining a second one.
...
- Geometry + the conveyor renderer land in this slice's own module
  ...
  board view under `plateau-app:src/backlog-view/` and positioning the cells [#xq8fvck] renders. It also
  exports the lane vertical-axis layout the scope-lease zones [#xzsx09z] consume. Consumes the graduated scale/progress/zoom mints — [#2534] (scale-ruler),
...
- The cells themselves (glyph/color/motion) → [#xq8fvck]. Scope-lease zone stacking → [#xzsx09z] (it lays its
