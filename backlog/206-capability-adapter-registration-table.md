...
registered table** (the existing `adapters.json` pattern), not per-impl scattered adapters.
...
- **Central table** — one registered adapter row per impl, each declaring its **capability ID → tier**
  ...
  Follow the repo's existing `adapters.json` pattern (registered, hot-reloadable — see the
...
- **Source-of-truth ruling:** the adapter table **is** the matrix's row source — one file, no split.
  `capabilityMatrix.json` → `impls[]` *is* the registered capability adapter table; the build-matrix
...
- **Done:**
  - **First-class adapter type** — `capabilities/provider.ts`: `CapabilityAdapter` (the registered
  ...
  - **Registered table export** — `capabilities/index.ts`: `capabilityAdapters` = the shipped
  ...
  - **Tests** — `provider.test.ts`: `adapters()` accessor + the DoD **single-row-registration** test
  ...
  - **Discovery surface + docs** — `/capabilities/` section reframed to "The registered capability
    ...
    `capabilityMatrix.json` description updated; `docs/agent/design-first.md` authoring note for
...
- **Next:** strictness/cascade (#207), runtime/edge impls (#208) build on this. Follow-up filed: #216
  (per-registered-adapter detail pages, mirroring `adapters.json`'s `adapter-descriptions/{id}.njk`).
