...
([src/backlog.njk](src/backlog.njk), switched by [src/assets/js/backlog-burndown.js](src/assets/js/backlog-burndown.js)).
...
([src/_data/backlog.js](src/_data/backlog.js)) resolves them into a per-item `blockers` array. What's
missing is the **reverse** edge — for each item, *who depends on it* — and a ranking over that.

## Build

- **Loader** ([src/_data/backlog.js](src/_data/backlog.js)): after `blockers` are resolved, derive the
...
- **Index UI** ([src/backlog.njk](src/backlog.njk)): add a third tab `data-bd-tab="priority"` +
...
- **Tab JS** ([src/assets/js/backlog-burndown.js](src/assets/js/backlog-burndown.js)): extend the
...
- **Done:**
  - **Loader** ([src/_data/backlog.js](src/_data/backlog.js)): a reverse-edge pass after `blockers`/`tier`
  ...
  - **Index UI** ([src/backlog.njk](src/backlog.njk)): a third `data-bd-tab="priority"` tab + `#panel-priority`
  ...
  - **Tab JS** ([src/assets/js/backlog-burndown.js](src/assets/js/backlog-burndown.js)): added `priority` to
    the `panels` map — the existing `data-bd-tab` switch + keyboard nav + `TAB_KEY` persistence cover it with
    no new wiring.
  - **Tests** ([src/_data/__tests__/backlog-leverage.test.ts](src/_data/__tests__/backlog-leverage.test.ts)):
