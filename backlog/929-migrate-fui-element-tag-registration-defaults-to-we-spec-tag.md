...
**Tag-preserving consumer updates (identical registered tags — zero runtime change):** `fui:plugs/bootstrap.ts` (route-view/route-outlet/auto-heading overrides), the bg-task demos, loan-origination + auto-insurance (`data-table`/`page-nav`), `autocomplete-unplugged`, `fui:workbench/registry.ts`, and the AutoComplete unit test now register the pretty tag explicitly.

**Catalog:** removed the now-phantom literal names (`route-view`/`route-outlet`/`app-tasks`/`background-tasks`/`auto-complete`) from `fui:src/_data/blocks.json` `registeredNames` (they're param-defaults now, not literal defines).

**Gate:** `TAGNAME_GATE_ENFORCED=true` in `fui:scripts/check-standards.mjs`. `check:standards` green (0 errors; 18→6 warnings — the 12 tag-drift/hard-code warnings cleared; remaining 6 are #840 CEM member-surface drift, separate). Full vitest 1855 pass, `tsc` clean.

**Size note:** landed as effectively ~5–6 (16 files: 6 registrars + gate + fui:blocks.json + 8 consumers/test), larger than the size·3 estimate — a tag-preserving sweep, all mechanical, no fork.
