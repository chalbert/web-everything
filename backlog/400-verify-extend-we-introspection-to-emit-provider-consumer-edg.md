...
- 5 new unit tests (Injector.test.ts): off-by-default, records-when-on, dedups, records-at-call-site-when-resolved-up-the-chain, clears-on-dispose. Full webinjectors suite green (190 + 5); `tsc --noEmit -p tsconfig.json` shows 0 errors in the touched file.
