...
maturityTrigger: "adoptionSignal: zero @frontierui runtime imports remain under we:blocks/ + we:plugs/ (the #658/#1730/#1047 relocation drain fully landed)"
...
zero (the relocation has fully landed), build the two checks and flip them on. Until then this is a
`maturityGated` park (hold-model fix 2026-07-02 — the prose always said "Parked"; the frontmatter now
matches) and the front-B watch (#1852) covers these rules by reading. *(Trigger scoped to
...
- **Status:** parked (`maturityGated` + typed trigger as of 2026-07-02 — previously mis-encoded as
  `open` + `priority: low` while the prose said "Parked") — un-park trigger not yet met (trigger
  re-checked 2026-07-02: still non-zero).
