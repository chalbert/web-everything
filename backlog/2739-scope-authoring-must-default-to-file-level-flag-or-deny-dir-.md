...
Delivered as a `check:standards` lint (§6d-sexies in `we:scripts/check-standards.mjs`, mirrored + unit/corpus-tested in `we:scripts/__tests__/check-standards.test.mjs`).

- The lint FLAGS any repo-qualified `scope:` entry that is a bare directory prefix (ends in `/` with no filename) on a non-resolved item.
- Escape hatch: a non-empty `scopeRationale:` frontmatter note clears the flag — the deliberate, justified exception for a genuinely dir-spanning / cross-cutting item.
- Soundness: it is a WARNING, never an error. The remedy is always "narrow to the files you actually write (never fewer)" OR "add a scopeRationale" — it never pressures an author to UNDER-scope, and never reds the gate on the existing dir-scoped corpus (20 open/active items currently flagged; resolved items skipped as historical).
