...
The principled indirect path established while ratifying #747-Fork-4: a design system never names a trait directly; instead its intent defaults reach traits through a resolver that maps the active intent profile to trait build-time inclusion/delivery. The concept is already stated (block-pages.njk:85 'only the traits matching the active intent profile are bundled'; router.njk:896 'Loader Intent selects the loading UX trait') but no resolver exists as code. Keeps intents UX-only (no impl refs — ratified) and traits technical: the mapping lives in a Technical-Configurator-style resolver layer, not in the intent or the trait. Scope: build-time inclusion/delivery (which traits ship, eager/lazy); runtime activation gates (inert/visibility) stay DOM-driven.

## Progress (resolved 2026-06-16) — locus: webeverything

**Placement (set this turn):** built **WE-resident**, not in plateau-app. #747 carved this as "its own mechanic → #776" but left the home open; the loader inferred plateau-app from the `technical-configurator` tag. On merit the resolver is **standard logic**: a pure, deterministic function over the WE registries (the `intentDimension` keys traits already declare in `blocks.json`, + intents), the same shape as this batch's `webcases/requirementValidator.ts`. The Plateau Technical Configurator and the FUI build are *consumers* of it, not its home (standard logic → WE, lowest lock-in; "Technical-Configurator-**style**" describes the resolver pattern, not the repo). Set `locus: webeverything`.

- New [`webtraits/intentProfileResolver.ts`](../webtraits/intentProfileResolver.ts) (pure, dependency-free):
...
- Tests: [`webtraits/__tests__/intentProfileResolver.test.ts`](../webtraits/__tests__/intentProfileResolver.test.ts) (7/7) against the real `blocks.json` trait-key shape; added `webtraits/**` to `vitest.config.ts`. `tsc --noEmit` + `check:standards` clean.
