...
- **WE** — added a "Changed a rendered page, template, or layout" item to the Definition of Done in [AGENTS.md](../AGENTS.md): must pass the #770 rendered axe gate (`npx playwright test tests/a11y`) + the #772 static lint, plus a hand-reviewed structural checklist (aria-current #762, landmarks, heading order, focus order/`:focus-visible`, accessible names, no colour-only contrast cues).
- **FUI** — mirrored the same DoD item into `../frontierui/AGENTS.md`, pointing at the #771 mirrored gate (FUI site `:3001`).
