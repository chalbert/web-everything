...
- Unit test `fui:ProgressElement.test.ts` (4): self-replacement to `fui-progress` + native `<progress>`, indeterminate-when-value-omitted, `valuetext` → `aria-valuetext`, circular SVG `role=progressbar` ring. 9/9 progress unit tests green; FUI `check:standards` 0 errors.
