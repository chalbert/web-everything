...
- **Migrated 12 files** (6 impl + 6 unit tests) WE → `../frontierui/blocks/`, each **byte-identical** (`diff -q` clean): `fui:audit/AuditProvider.ts`, `fui:lifecycle/LifecycleProvider.ts`, `fui:master-detail/MasterDetailBehavior.ts`, `fui:selection/SelectionBehavior.ts`, `we:stepper/StepperBehavior.ts`, `fui:tree-select/TreeSelectBehavior.ts` + their `blocks/__tests__/unit/<family>/*.test.ts`. The 6 form a closed set — internal deps only (`audit → lifecycle`, `master-detail → selection`), no external block/plug imports — so the copies resolve in FUI with no edits.
- **`fui:blocks/package.json`** (S1 exports map) — added a bare entry per single-file family (`./audit` → `fui:./audit/AuditProvider.ts`, …) + a deep-import wildcard (`./audit/*`, …) for all 6.
