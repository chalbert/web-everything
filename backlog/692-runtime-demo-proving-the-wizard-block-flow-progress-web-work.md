...
- **2026-06-15 — built + verified.** New `demos/wizard-flow-demo.{html,ts,css}` mounts the `<wizard-flow>`
  ...
  Registered in `we:src/_data/demos.json` (id `wizard-flow-demo`, `path` + `liveUrl`, kind playground,
  projects webworkflows/webintents/webblocks, epic 651).
- **e2e:** `we:blocks/__tests__/e2e/wizard-flow-demo.spec.ts` drives the live page on :3000 — asserts the
...
- **Dev-server fallback:** not applicable — this is a single static-file demo (`we:/demos/wizard-flow-demo.html`)
...
- **Gate:** `npm run check:standards` 0 errors; `eleventy --dryrun` build smoke clean (the we:demos.json
