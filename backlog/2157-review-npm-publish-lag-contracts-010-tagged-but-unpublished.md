...
- `we:.github/workflows/publish-contracts.yml` still carries the `workflow_dispatch` trigger with a `dry-run`
  input (`:24-29`; the `default: true` line is `:29`) that defaults to a dry run, still runs `npm run check:standards` as a job step (`:49`), and
...
**The auto-lock race is HALF fixed already — do not re-diagnose it** (juror finding, 2026-08-21). The "To do
when unblocked" bullet above frames "make release-please tolerant" as open work; the tolerance half already
landed. `we:.github/workflows/release-please.yml` gates its publish job on
`always() && needs.release-please.outputs.releases_created == 'true'` (`:60`), and its own header (`:20-26`,
the *Publish resilience* note, #907 / #xjcbsad) records exactly why: a cosmetic post-release failure — such
as commenting on a conversation the bot just locked — must not skip a publish once a release was really cut.
So the **functional** consequence is closed. What is still open is only the **cosmetic** half: the release job
still reports failure. Scope the bullet to that; a picker who misses this will spend a round re-solving a
solved problem.
...
- The auto-lock-on-merge race gets a recorded outcome for its **remaining cosmetic half only** (the functional
  half is already closed by the `always()` gate at `we:.github/workflows/release-please.yml:60`): after the
  publish, check whether the release job still *reports* failure from a `github-actions[bot]` lock applied
  seconds after merge, and write "fixed" or "accepted, because …" in one line on this card. A named look, not
  a judgment call.
