...
- **The scorer** — new [scripts/design-refs/benchmark.mjs](../scripts/design-refs/benchmark.mjs): a pure
...
- **The runner + CLI** — `runBenchmark({provider, records, recipe, loadFrame})` drives the resolved provider
  over each held-out frame (base64 WebP), then `design-refs.mjs benchmark` prints agreement + per-class
...
- **Recipe floor finalized** — [distillation-recipe.json](../design-refs/distillation-recipe.json) v2:
...
- **Tests** — [benchmark.test.mjs](../scripts/design-refs/__tests__/benchmark.test.mjs) (13, green): split
