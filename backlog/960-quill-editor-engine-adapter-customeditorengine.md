...
**What was built (and works):** `engines/quill.ts` — `QuillEngine implements CustomEditorEngine`, mapping
...
Vite's pre-bundled `node_modules/.vite/deps/quill.js`. Single hoisted `parchment@3.0.0` (not a duplicate)
...
reinstate `engines/quill.ts` + the registration unit test (both reverted clean); (3) `npm i -D quill` +
re-add the optional `peerDependency`; (4) register `QuillEngine` in `fui:demos/rich-text-editor/rich-text-editor.ts`
+ extend the `engines()` assertion in `engine-switcher.spec.ts` to `['native','plain','quill']`; (5)
re-add `demos/rich-text-editor/__tests__/quill-engine.spec.ts` (type → keyboard select-all → bold →
