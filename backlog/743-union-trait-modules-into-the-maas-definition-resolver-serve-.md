...
- **`definitionRegistry.ts`** — `indexTraitModules(modules)` builds a `name → pre-built module source`
  registry (the trait-side resolver). A trait carries no `<component name=>`, so its name is supplied
  explicitly rather than scanned.
- **`tools/maas/vite-plugin.ts`** — the trait registry is unioned in as the component resolver's
...
- **`moduleService.ts`** — `serve()` now routes a resolved source that is **not** a `<component>`
...
Tests: `definitionRegistry.test.ts` (trait resolve + union + verbatim serve), `fetchHandler.test.ts`
