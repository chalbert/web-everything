...
#xmiuo0r because it is unimplementable until this story exposes the live SHA.
...
**older** tree over the newer one until the next push. The ancestry assertion added in #xmiuo0r does **not**
cover this — an older commit is still a legitimate ancestor of `main`, so it passes that gate.

Judged narrow, deliberately not fixed in #xmiuo0r: the `concurrency` group in `we:.github/workflows/ci.yml`
