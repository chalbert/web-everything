...
The **Native-first resolution** section of [we:src/capabilities.njk](../src/capabilities.njk) walks the
droplist worked example (native branch → `base-select`, fallback → `face`) as **hand-authored prose**.
The resolver itself is TypeScript ([we:capabilities/resolver.ts](../capabilities/resolver.ts)) and 11ty
...
- Emit the resolver's worked-example output into a **build-time JSON artifact** (a small script or an
  ...
  from `we:capabilityMatrix.json`), then render the page section from that artifact instead of prose.
...
- [we:capabilities/workedExample.ts](../capabilities/workedExample.ts) — `buildNativeFirstWorkedExample()`
...
- [we:src/_data/capabilityWorkedExample.json](../src/_data/capabilityWorkedExample.json) — the committed
  build-time artifact 11ty reads (11ty can't run the TS resolver, so the resolver computes it once and
  commits it, mirroring how the matrix table renders from `we:capabilityMatrix.json`).
- [we:src/capabilities.njk](../src/capabilities.njk) — the "Native branch" box now renders winner + each
  impl's eligibility/cost/blockers from that artifact instead of hand-authored prose.
- [we:capabilities/__tests__/worked-example-artifact.test.ts](../capabilities/__tests__/worked-example-artifact.test.ts)
...
already pinned by `we:resolver.test.ts`).
