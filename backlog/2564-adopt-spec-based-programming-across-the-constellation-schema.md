...
almost no power to detect. It is a tripwire, not an instrument.* The **permanent non-zero human sample itself
is not re-decided** — it is #2563 Fork 2's settled backstop, and autonomy graduation is the staged-autonomy
clause of `#agent-convergence-independent-validation` (`we:docs/agent/platform-decisions.md:2775-2776`); both
are **cited, not re-declared** (no third overlapping anchor).
...
- **(a) A purpose-built measurement instrument.** **Chosen.** To get a false-clear *rate* with power on the
  ...
  spec-violating change and confirm the independent reviewer catches it). This is the instrument that closes
  the open question the research flagged as unmeasured.
...
rate); seeded-defect / mutation-style audits as the standard way to *measure* a reviewer's catch rate (the
research refuted a specific 86–100% mutation-score claim, but the *method* of injecting known defects to
...
+ seeded-defect audit). No third anchor minted — graduation cites `:2775-2776`.
...
standard/spec, FUI/plateau = impl) and `#agent-convergence-independent-validation`. Research:
