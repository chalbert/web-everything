...
Core slice of the non-technical control plane (sliced 2026-06-16 after artefact contract #672 landed; siblings #B/#C/#D below). Build the persona-scoped `/control-plane` route + shell in plateau-app and the **jargon-translated gate dashboard**: render each gate (`GateType` advisory/validating/blocking/escalating, `src/profiles/schema.ts:75`) in plain founder/PM/compliance language, scoped by persona (`src/profiles/profiles.ts:14` `getProfile`, `src/profiles/roster.ts`). Reads the existing enforcement layer — `GateEnforcer.evaluate()→DeployVerdict` and `GateRecord` (`src/profiles/gate-enforcement.ts:201`/`:32`) — no new enforcement logic. New `src/control-plane/dashboard.ts` mounted on the proven `mountProfiles` pattern (`src/profiles/profiles-page.ts:158`, `index.html:135`, `src/main.ts:216`/`:351`). This shell is the landing surface the sibling panels (escalation inbox, audit view, trip planner) plug into. Constellation: plateau-app product layer (#091); standard+taxonomy stay in WE. Maps gate severities (#166) + autonomy levels (#672 L0–L4) onto persona-scoped views.

> Note: the body's "plain-language-gate requirement on the step-tree" is **absorbed here** (the dashboard IS the jargon translation); a dedicated step-tree-coupled slice is deferred until a plateau-app step-tree surface exists (#671 graduated to a webeverything `/research/` njk, not running code). See `reports/2026-06-16-backlog-split-analysis.md`.

## Progress (2026-06-16, batch-2026-06-16) — built

- **Route + shell:** `/control-plane` added to plateau-app — nav link + `<template route>` + `#control-plane-mount` in `index.html`, route-change case + `tryMountControlPlane` + the **INITIAL SETUP** direct-load/deep-link mount in `src/main.ts` (the mount path the sibling routes use; missing it was a real deep-link bug, caught by a browser probe).
- **Dashboard:** `src/control-plane/dashboard.ts` (`mountControlPlane`) + `control-plane.css`, on the proven `mountProfiles` pattern (persona tabs, full re-render, localStorage selection). Reads the **existing** enforcement layer — builds a `GateEnforcer` from each persona's review-area gates and renders its `DeployVerdict`; **no new enforcement logic** (#568 owns that).
