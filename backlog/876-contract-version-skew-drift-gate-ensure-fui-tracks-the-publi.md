...
#877 (CI publication pipeline) resolved, satisfying the old `blockedBy:[877]` edge — but it built the *pipeline*, not an actual publish. Verified: neither `frontierui/package.json` nor `plateau-app/package.json` declares a `@webeverything/contracts` dependency (they resolve via the #878 dev path-mapping), and the package is still `0.0.0` (unpublished). So **no consumer pins a version and the contract cannot skew** — this gate would have nothing to check. Re-blocked on the real precondition **[#907](/backlog/907-first-real-publish-of-webeverything-contracts-migrate-a-cons/)** (first publish + a consumer migrating to a pinned dep); cascade-frees when that lands.
