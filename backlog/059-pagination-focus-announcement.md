...
crossRef: { url: /backlog/gap-6-focus-announcements/, label: gap-6 focus & announcements }
---
...
This is the pagination-specific instance of the general concern in [gap-6 — Focus & Announcements intent](/backlog/gap-6-focus-announcements/); whatever gap-6 decides (centralized `focus` intent vs a11y protocol) is the mechanism this trait would re-point at. The verified part of the a11y story is settled: `<nav aria-label="pagination">` + `aria-current="page"` (W3C WAI-ARIA APG). The unsettled part is the *post-change* focus/announcement behavior.
