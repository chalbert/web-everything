...
requires each round to address root cause and escalates only on non-convergence; the operator's bar says a
finding blocks only on regression / new impact / weakened gate. The candidate reading — that the ruling
**narrows what "clean" means for #2851's exit condition** rather than replacing the loop — is exactly the
reading the item says must be confirmed with the operator. Write the anchor with that reading stated
explicitly and marked as *proposed* until confirmed; do not resolve on an unconfirmed reading.
...
3. **tier 2 — the anchor states its relationship to #2851 explicitly**, naming whether it narrows, bounds or
   ...
   same effective bar.
4. **tier 3 — the operator has confirmed the #2851 reading**, and the anchor records that confirmation
   (date + what was confirmed). Until then the anchor's reconciliation paragraph stays marked *proposed* and
   the item stays open. This is the criterion no command can decide, and it is the item's real gate.
