...
Done — FUI now consumes the contract halves from `@webeverything/contracts` (resolved via the #878 dev-time path/alias), retiring the byte copies. WE keeps each `we:contract.ts` as the source the package re-exports (the FUI→WE arrow); WE files untouched.

- **6 #694 families** (clean): each FUI runtime file (`fui:audit/AuditProvider.ts`, `fui:lifecycle/LifecycleProvider.ts`, `fui:master-detail/MasterDetailBehavior.ts`, `fui:selection/SelectionBehavior.ts`, `we:stepper/StepperBehavior.ts`, `fui:tree-select/TreeSelectBehavior.ts`) repointed `import type … from './contract'` + `export type * from './contract'` → `'@webeverything/contracts/<family>'`, and **deleted** its local `blocks/<family>we:/contract.ts` byte copy (6 files removed — the #879 FUI copies, now redundant).
- **guard**: FUI's `fui:blocks/guard/provider.ts` was the pre-#873 *monolithic* byte copy (#836, before WE split guard) — types inline, no `we:contract.ts`. Stripped the seven inline contract declarations (`GuardRegionKind`, `GuardRegion`, `GuardEvent`, `GuardContext`, `GuardDecision`, `GuardRevocationListener`, `CustomGuardProvider`) and replaced them with `import type … from '@webeverything/contracts/guard'` + `export type * from '@webeverything/contracts/guard'`. The guard runtime (`GuardDecisionError`, `assertGuardDecision`, `ALLOW`, `NativeGuardProvider`) stays in FUI; the siblings (`we:registry.ts`, `we:accessControl.ts`, `we:index.ts`) and tests resolve unchanged via `provider`'s re-export.
