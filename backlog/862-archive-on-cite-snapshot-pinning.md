...
- **Resolved 2026-06-17.** Built `we:scripts/pin-reference-snapshots.mjs` (`npm run pin:references`): for
  each #597-indexed URL it pins a snapshot into `we:src/_data/referenceSnapshots.json` — a
...
- **Tested:** `hashBody`, `parseArchiveResponse`, and the idempotent `pinReferences` merge are pure —
  10-case offline suite (`we:scripts/__tests__/pin-reference-snapshots.test.mjs`, green) with the most
  weight on idempotence (an existing pin must not move) + `--repin`/`--limit`/failure paths; the sweep's
  26-case suite still green after the probe change. Documented in `we:docs/agent/reference-retirement.md`
  (§ "Archive-on-cite").
