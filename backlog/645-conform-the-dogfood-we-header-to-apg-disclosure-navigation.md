...
- **reveal-nav.js** (new, wired in base.njk) — the progressive-enhancement layer: sets `.js-nav` on `<html>` (switching the keyboard reveal from `:focus-within` to aria-driven so **Esc can collapse while focus is still inside** the section), makes each head a click/Enter/Space **toggle** that closes siblings, **Esc** closes + returns focus to the head, outside click/focus dismisses, resize clears desktop state.
