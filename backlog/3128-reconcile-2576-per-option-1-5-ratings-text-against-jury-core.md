...
[#1034](/backlog/1034-vision-design-critique-service/), which already ratified a 1–5 scale here as *advisory,
...
| "jury-core is missing the rating instrument" | The evidence-correct form of move 3 **already ships**, on the finding axis: `IMPACT_LEVELS` (`we:scripts/lib/jury-core.mjs:190-195`) is a closed four-step ordinal grade, each step carrying a written descriptor in `IMPACT_GLOSS` (`:203-208`), tested against a configurable bar `PREVENTION_IMPACT_BAR` (`:257`) by `blocksAcceptance` (`:530-534`). Closed labelled grades + a bar is exactly what the prior art prescribes |
...
| "which shape does #2575's field follow?" | The verdict enum, as it already does (`:114-134`). The `jurorRatings` → `jurorVerdicts` rename is a mechanical entailment inside #2575's own build, not a ratification |
...
is ever told that. The reach is real — `we:AGENTS.md:61` (the Tier-0 router) sends any high-leverage
fork-ruling to this doc and names "per-option 1–5 ratings → weak-flag + search" in the router row itself;
`we:docs/agent/build-ui.md:133-134` and `:260` restate moves 3–4 as phase-4 procedure; and
`we:skills-src/design-committee/SKILL.md:9` defers its whole judging half to this doc. So three live
surfaces hand a downstream agent a different instruction depending on this call.
...
- **(a) Keep, re-pointed and status-free.** Move 3 is restated as *the instrument WE already ships* —
  a closed labelled ordinal grade with written level descriptors, tested against a bar — pointing at
  `IMPACT_LEVELS` / `IMPACT_GLOSS` / `PREVENTION_IMPACT_BAR`, with the `:51` average illustration struck.
  Move 4 (below-bar → search for new candidates, **keep the old**) stays as a **timeless method rule with no
  build-status prose**; its WE realization is filed as its own backlog item carrying the graduation trigger,
  and the doc links that item by id and says nothing else about what is built. ← **recommended**
...
fork-ruling here and `we:skills-src/design-committee/SKILL.md:9` defers its judging half here, so what the
doc instructs is a consumer-visible contract, and (a) re-points only at this repo's own agent tooling, never
at a Frontier UI implementation. Q2 clear **but the stated merit axis was rejected as a rationalization**:
...
engine), and composes with [#1034](/backlog/1034-vision-design-critique-service/) (resolved — the ratified
