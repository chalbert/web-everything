...
- **`we:src/_data/protocols.json`** — new `provider-consumer-graph` entry (`status: concept`, `ownedByProject: webregistries`).
...
- **`we:src/_data/semantics.json`** — two new terms: *Consumption Edge*, *Consumer-Driven Projection*.
