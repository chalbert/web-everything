...
The explorer finds issues two ways: **oracles** (Layer-1/2 — deterministic hand-coded rules: console errors, 5xx, contrast, broken layout; objective, gate-eligible) and the **judge** (Layer-3 — a probabilistic model that *looks* at a state and emits advisory candidates; subjective, never gates). Today the judge (`fui:tools/explorer/oracles/advisoryJudge.ts` + `fui:tools/explorer/oracles/tier2VlmJudgeModel.ts`, on-device Florence-2) is **inference-only and frozen** — it *describes* a state, it does not diagnose, and **nothing it sees ever updates it**. So every real improvement to the explorer's *perception* today means a human hand-codes another deterministic oracle. That doesn't scale to subjective/perceptual issues and is the explicit anti-goal ("not just adding scripts around the agent"). This epic makes the judge **trainable**: human feedback on real runs (the page-by-page review loop the `improve-explorer` skill drives) becomes labeled training signal, and the judge gets better at the subjective calls on its own — so next run catches what a human flagged without anyone writing a new rule.
