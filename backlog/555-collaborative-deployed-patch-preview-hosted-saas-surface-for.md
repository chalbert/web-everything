...
Once the #410 deployed live-patch overlay exists, a hosted SaaS turns the single-session preview into a shared, governed artifact — the paid open-core surface atop the open local primitive (the [monetization](docs/agent/platform-decisions.md#monetization) rule). This epic umbrellas the collaboration + approval features: (A) shareable live-fix preview links a stakeholder opens against the deployed app; stakeholder-initiated fix requests (annotate the live view → seed the loop); async comments anchored to an overlay; (B) designer/PM approve-to-PR sign-off on the rendered result; side-by-side variant preview (pick one → PR); a designer visual/token-only patch lane.
