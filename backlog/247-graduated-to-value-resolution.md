...
Today the graduation nudge in `scripts/check-standards.mjs` only checks that a resolved item *has* a
...
`graduatedTo: none` (sanctioned in `docs/agent/backlog-workflow.md` for resolved items that spawned no
...
- For any other value, parse the `kind:slug` form and verify it resolves to a real entity in the
  matching registry (`block:` → blocks.json, `intent:` → intents.json, `protocol:` → protocols.json,
  `adapter:` → adapters.json, `project:` → projects.json, etc.) — **error** on an unresolvable ref, the
...
## Progress
- **Status:** resolved (2026-06-09). `graduatedTo: none` — enhanced existing tooling (`check-standards.mjs`), no new entity.
...
- **Done (`scripts/check-standards.mjs`):**
  - Loads `adapters.json` + `demos.json`; builds `GRADUATED_KINDS` (block/intent/protocol/project/plug/
...
- **Gate / proof:** `check:standards` **0 errors** across all 239 real items (zero false-positives —
  ...
  → clean.
- **Scope note:** `check-standards.mjs` has **no unit-test harness** (no rule does — it's validated by its
