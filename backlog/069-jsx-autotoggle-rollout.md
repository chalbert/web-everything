...
- **Survey:** `autoToggle` live only on `we:for-each.njk`. Pages with `{% highlight "html" %}` examples (candidates): action-button, prefetch-behavior, interpolation-text-node, workflow, broadcast, component (manual), dropdown (manual/pseudo), resource-action, router.
- **Done:**
...
- **Notes:** Pages whose example is pseudo-HTML or reactive `{{ }}`/`[[ ]]` text (Axis 2) stay on the manual `sourceToggle` — that's correct, not missing coverage. Now-unblocked candidate (post-073): broadcast's "With static detail" example (`we:broadcast.njk:68-74`, the `broadcast-detail='{"theme":"dark"}'` JSON snippet) is still on a manual `{% highlight "html" %}` — convertible to `autoToggle` now that quote escaping works.

**Graduated to** `adapter:jsx-adapter` — we:adapters.json#jsx-adapter status set to implemented.
