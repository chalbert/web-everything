...
The epic's tracked prototype tip advanced from `600acc14f` to `6a2c8c1ab` (13 commits: `git log 600acc14f..origin/lane/mechanical-dispatcher`). Checked `git diff 600acc14f..6a2c8c1ab --stat` against every file in this card's scope (runner/supervisor/tick-once/tick-bookkeeping/shared-tick fixtures) — **no overlap**; none of this slice's files changed in that window. This card's existing merge notes, conflict counts, and the manual-tick check above remain valid as written; port from `6a2c8c1ab` when dispatched (identical content to `600acc14f` for every file this card owns).
