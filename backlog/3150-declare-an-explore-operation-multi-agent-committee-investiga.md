...
Parallel fan-out is a property of the EXECUTOR, so it is filed as #3192 rather than smuggled in behind a
...
the brief forbids writing anywhere but the one path given, and the structural close is filed as #3194.
...
**Leftovers filed rather than half-done:** #3192 (parallel dispatch fan-out in the executor), #3194 (the
residual blindness channels), #3193 (nothing reclaims the panelist scratch directory).
