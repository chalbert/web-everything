...
Parallel fan-out is a property of the EXECUTOR, so it is filed as #3189 rather than smuggled in behind a
...
the brief forbids writing anywhere but the one path given, and the structural close is filed as #3191.
...
**Leftovers filed rather than half-done:** #3189 (parallel dispatch fan-out in the executor), #3191 (the
residual blindness channels), #3190 (nothing reclaims the panelist scratch directory).
