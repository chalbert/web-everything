...
> `TreeSelectBehavior` (`fui:blocks/tree-select/TreeSelectBehavior.ts`, active, 6 unit tests) realizes the
...
> emitting `tree-change`. Registry entry + `we:block-descriptions/tree-select.njk` (WAI-ARIA APG Tree View
...
Author the tree-select block — a concrete hierarchical member of the droplist family. Thin trait selection over the substrate: composesIntents += hierarchy (#295), model=single|multiple, optional windowed; role=tree collection instead of role=listbox. No new behavior — every hard part is delegated to a composed intent per #064's ruling. Needs the block registry entry in fui:blocks.json, we:block-descriptions/tree-select.njk (overview, Web Standards Alignment table citing WAI-ARIA APG, Framework Research table, features, API), and demo. Native-first baseline degrades to single-level grouped `<select>`. Grounded in we:reports/2026-06-11-tree-select-hierarchical-selection.md.
