...
no fui:blocks.json entry):

- **`fui:reorderState.ts`** — the pure DOM-free reducer driving BOTH paths identically (keyboard parity
...
- **`fui:renderReorderableList.ts`** — APG structure (`role=list`, grab-handle buttons, roving tabindex,
...
- Exported from `fui:blocks/renderers/index.ts`. Manual order — the inverse of Collection Operations'
...
are the design surface, not built here — renderers family carries no fui:traits.json drift.)
