...
ruling (taxonomy ratified **B / C / B / A**). Add `we:design-refs/taxonomy.json` seeded with the
...
`we:targets.json` and run `design-refs collect` toward ≥3 shots per thin (productRegister × category)
...
1. **`we:design-refs/taxonomy.json`** — keyed open-growing registry (Fork 2 = C). Seed two vocabularies:
...
2. **Re-key (Fork 1 = B)** — migrate the 16 existing `designRegister` values in `we:targets.json` →
   `productRegister`; introduce `visualStyle` as a vision-pass-filled field (left empty at collect
   time). Mechanical, no recapture. Re-run `design-refs index` so sidecars/`we:index.json` carry the
   new keys.
3. **Scarcity grow-targets (Fork 3 = B)** — append `we:targets.json` entries for the thin cells
   (consumer/social, commerce/admin, finance/banking, productivity/collaboration,
...
4. **`report`** — point the aggregation at the keyed `we:taxonomy.json` vocab (by `productRegister` /
   `category`), validating new values against the registry (warn on unregistered).

## Acceptance

- `we:taxonomy.json` exists with both seeded vocabularies; `report` reads from it.
- All 16 existing targets carry `productRegister` (no `designRegister`); `visualStyle` present (may
  be null/empty).
- `we:targets.json` has scarcity-cell entries; a `collect` run is idempotent on re-run.
- `check:standards` green.

## Progress — resolved 2026-06-13

- **`we:design-refs/taxonomy.json`** (Fork 2 = C) — keyed open-growing registry: `productRegister`
...
- **Re-key (Fork 1 = B)** — a mechanical migration (keyed by url) re-keyed all **16 targets** and all
  **16 per-item `we:meta.json` sidecars**: `designRegister` → `productRegister`, fine categories folded
  onto the coarse vocab, `visualStyle: null` introduced. No recapture. `design-refs index` re-run so
  `we:index.json` carries the new keys.
...
- **Pipeline** — `collect`/`harvest` meta-writes emit `productRegister` + `visualStyle`; `report` now
  groups by `productRegister` × `category`, warns (never errors) on a value not in `we:taxonomy.json`, and
  prints a thin-cell (< 3 shots) scarcity view to drive the next grow pass.
- **Tests** — `we:scripts/design-refs/__tests__/taxonomy.test.mjs` (5) locks the vocab + re-key + grow
