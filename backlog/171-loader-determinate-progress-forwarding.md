...
- Add a progress signal to `ResourceLoader` — e.g. a `reportProgress(fraction)` method or a
  ...
  call).
- Have `ResourceLoaderHandle` (`we:blocks/resource-loader/backgroundHandoff.ts`) translate that into
...
- A demo card in the [handoff playground](/demos/loader-background-handoff-demo/) showing a
  determinate task filling its bar (extend `we:handoff-scenarios.ts`, keep the shared-fixture split).
...
- **Done:**
  - `ResourceLoader.reportProgress(loaded, total?)` (`we:blocks/resource-loader/ResourceLoader.ts`) emits a
    bubbling `resource-progress` event with a clamped 0..1 `fraction` (two-arg normalizes `loaded/total`;
    single-arg reports the fraction directly). New `ResourceProgressDetail` in `we:types.ts`.
  - `ResourceLoaderHandle` (`we:backgroundHandoff.ts`) listens for `resource-progress` and overlays the
  ...
  - Shared fixture scenario `determinate-progress` (`we:__fixtures__/handoff-scenarios.ts`) + a demo card in
    the handoff playground (`we:demos/loader-background-handoff-demo.ts`) — both consume the one fixture.
...
- **Verified:** resource-loader + background-task-surface suites green (89 tests); `check:standards` 0
  errors; `tsc` clean for touched files; playground 4/4 at `we:/demos/loader-background-handoff-demo.html`.
