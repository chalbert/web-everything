...
- **(A — recommended) Tier-1 self-run dev tool first; hosted broker deferred.** Maps directly onto the solo-founder operational ranking (self-run tool > single service > hosted) in [#089](089-monetization-product-ideas.md) — the open-core monetization strategy (defer live-serve). A solo founder should not take on hosted uptime/support at concept stage; the open self-run tool is what proves the schema.
