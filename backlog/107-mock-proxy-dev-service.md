...
- **(A — recommended) The mock/contract schema is the standard; the server is a provider.** Anchor the response *shape* on OpenAPI; add a WE **behavior layer** (latency / fault / conditional / `mock | proxy | record` response-kind) on top for what OpenAPI cannot express. The proxy/CORS-bypass/per-service-toggle are console+header+param overrides on that one schema, not a sibling standard. Matches "the protocol is the only lock" ([[feedback_minimize_lock_in_protocol_only_lock]]); borrows official vocabulary (OpenAPI) and adds the missing axis ([[feedback_intent_ux_only_technical_to_configurator]]). Cost: authoring the schema + a reference server provider.
...
- **(A — recommended) Tier-1 self-run dev tool first; hosted broker deferred.** Maps directly onto the solo-founder operational ranking (self-run tool > single service > hosted) in [#089](089-monetization-product-ideas.md) / [[project_monetization_strategy]]. A solo founder should not take on hosted uptime/support at concept stage; the open self-run tool is what proves the schema.
