...
The field splits cleanly. **Code-defined** mocks (MSW: handlers are `http.get(...)` → `we:HttpResponse.json()` in TypeScript) are best-in-class DX *because* they're code, and non-portable for the same reason — [mswjs.io/docs/comparison](https://mswjs.io/docs/comparison/). **Data-defined** mocks (Mockoon JSON, [WireMock stub mappings](https://wiremock.org/docs/stubbing/), Prism's OpenAPI) commit a JSON/spec another tool could consume — but they are *three incompatible dialects* of the same match-request→produce-response idea. OpenAPI is the closest shared substrate yet is provider-driven and describes the happy-path *shape*, not the dev-time *behaviors* (latency, forced 401, per-header conditional bodies) the essay demands.
