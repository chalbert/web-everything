...
`fui:blocks.json` entry. Today the demo is embedded **ad-hoc** in 8 of 69 per-block includes; this makes it a
...
- **Slot in the template.** Add a `section-card` to [`we:src/block-pages.njk`](../src/block-pages.njk) (it
...
- **Data field.** Add an optional `fuiDemo: { file, title, height }` to the relevant entries in
  [`fui:src/_data/blocks.json`](../src/_data/blocks.json) (start with the 8 that already have demos).
...
- The `fuiDemo` shortcode itself already exists ([`we:.eleventy.js:38`](../.eleventy.js), #701) — no
...
`we:reports/2026-06-16-backlog-split-analysis.md`), the props/token/a11y panels each bury a distinct
...
- **Props table → [#801](/backlog/801-per-component-api-data-sourcing-for-the-web-docs-props-table/).**
  ...
  `we:scripts/gen-cem.mjs`) and the props-table renderer
  ([#654](/backlog/654-mint-the-props-table-block-render-custom-elements-manifest-a/),
  `we:block-descriptions/props-table.njk`) both shipped. What's missing is *where* the structured
  per-component API data originates (WE-authored vs an FUI-side analyzer — a WE/FUI boundary call);
  `we:custom-elements.json` carries 0 members/attributes today.
- **Token table → [#802](/backlog/802-per-component-token-table-data-sourcing-for-the-web-docs-blo/).**
  A per-component token tier exists (`we:webtheme/defaultTokens.ts:90-104`, button/card) but covers ~2 of 69,
...
- Added the uniform **Live example** `section-card` to [`we:src/block-pages.njk`](../src/block-pages.njk) — renders `{% fuiDemo block.fuiDemo.file, block.fuiDemo.title, block.fuiDemo.height %}` only when the block carries a `fuiDemo` field (no empty slot otherwise).
- Added optional `fuiDemo: { file, title, height }` to the 8 entries in [`fui:src/_data/blocks.json`](../src/_data/blocks.json) that already had demos (`tooltip`, `nav-list`, `tabs`, `autocomplete`, `for-each`, `interpolation-text-node`, `component`, `view`) — surgical splice, JSON re-validated.
