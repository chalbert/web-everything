...
- `3304` criterion 1 asks that `#3238`'s Done-when 7 and `#3230`'s Done-when 6 each report **one** finding
  ...
  is broken.
- `3305` criterion 3 says `#3233` cites two facts correctly at `:274` and `:279`. At `ee6e5a98` it does.
...
This is the same failure `3298` already retracts once, in the same PR: *"`5289202` was an intermediate
...
`3301` — the card filed for *"corrected here, left standing there"* — is adjacent and does not cover it.
...
two lines above the paragraph it was editing — `3298` twice and the description once. `60acbe5f` is a
...
**The round that corrected that one missed a fourth, on the same sha, in the same file.** `3298`'s
...
produced four pairs across **nine** places: `5289202` as *head* in `3298` and the description (2),
`ee6e5a98` as *head* in `3305`, `3304` and the description (3), `60acbe5f` as *merge-base* in
`3298` twice and the description (3), and `60acbe5f` as *base* in `3298`'s fence (1).
...
`git show 87d5823d:backlog/3298-….md | grep -n '60acbe5f'` *returns lines* **60**, **66** *and* **110**:
...
(Anaphoric back-references — `3304`'s round-5 *"at the same head"*, twice — are not counted here: they
...
`3305` is adjacent too, and also does not cover it. That rule reads a cited **line** and looks for a quoted token;
here the citation is a **sha** and a role, the token is fine, and the file is not the thing being cited. A
different sha with the same shape passes `3305` untouched.
...
role word standing beside a sha in a result block, as `3298`'s `base 60acbe5f` did — is in scope, while a
...
wrong, and `3298` quotes *"#1556's head (`5289202`)"* for the same reason. The same negation `3299`
criterion 4, `3305` criterion 4 and `3304` criterion 4 all need.
...
1. **Executable** — `3304`'s round-5 text *"`#3233`'s Done-when 8, at PR #1556's head `ee6e5a98`"* against
...
4. **Executable** — **this card's own body reports none**, and so do `3298`'s corrected retractions.
...
5. **Executable** — a **second role word**, on real input, at the head this card was filed from:
   `3298`'s round-6 line 60, *"at PR #1556's **merge-base** `60acbe5f` and at its **merged head**
   ...
   claimed for the PR.
6. **Executable** — a **third role word, inside a fence**, on real input: `3298`'s round-7 marker-set
