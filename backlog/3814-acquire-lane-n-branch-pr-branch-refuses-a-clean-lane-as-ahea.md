...
1. **Executable** — `npx vitest run lane-pool-acquire-adopt-behind-main` passes. The new suite is we:scripts/__tests__/lane-pool-acquire-adopt-behind-main.test.mjs and follows the harness of we:scripts/__tests__/lane-pool-acquire-reverify-containment.test.mjs (temp bare origin, `--origin`, `--reference`, `--name`, `--no-install`, an integration branch that is not named `main`). Four cases: (a) a clean lane sitting on the integration tip acquires with `--adopt --branch=<an older pushed branch>` and exits 0; (b) a lane with one commit that exists on no remote is still refused with the "ahead of origin" message; (c) a lane with an uncommitted change is still refused; (d) **`refuses a commit whose only remote branch was deleted on origin, though a stale local tracking ref still holds it`**, built from this exact fixture: in the temp repo, create a commit `X` on a branch `lane/gone` and push it to the temp bare origin; in the lane, run `git fetch origin` so the local remote-tracking ref `refs/remotes/origin/lane/gone` exists and points at `X`; then delete the branch on origin (`git push origin --delete lane/gone`) and do NOT fetch with `--prune` and do NOT run `git remote prune` (set `fetch.prune=false` in the lane so nothing prunes it); put the lane's HEAD on `X` with a clean tree. The fixture asserts its own preconditions before the acquire: `git rev-parse refs/remotes/origin/lane/gone` in the lane still returns `X`, and `git ls-remote origin refs/heads/lane/gone` returns nothing. Then `acquire --lane=<n> --adopt --branch=<an older pushed branch>` must exit non-zero with the "ahead of origin" message, and the lane's HEAD must still be `X` afterwards (the commit is unpushed for real, and acquire's reset-to-origin step would discard it). **Case (d) fails if the implementation trusts local tracking refs** (for example `localRemoteShas` at we:scripts/lane-pool.mjs:686, or `git branch -r --contains`): the stale ref makes `X` look provably pushed, so acquire would succeed and (d) reddens. It passes only when the check uses the live `ls-remote` snapshot (`liveRemoteShas`). Case (a) fails today with the "commit(s) ahead" refusal; (b), (c) and (d) pass before and after, so they pin the safety the fix must keep, and (d) is the one that pins the live-snapshot requirement in "The fix".
