...
- **"Worktree" in the item title is guard-blocked → read as "clone."** `{#isolate-by-clone}` (#1153) has the
  guard *deny* `git worktree add` in a shared checkout; #1933 replaced worktree isolation with independent
  clones. So the substrate is a clone either way — the live question is *which* clone (per-test throwaway vs a
  pooled lane), answered below.
