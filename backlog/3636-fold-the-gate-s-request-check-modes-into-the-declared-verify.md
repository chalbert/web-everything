...
undelegated raw home, marked `@operation-home-ok: #3636` meanwhile. Fold `request`/`check` into `verify`'s
...
1. **Executable** — `we:scripts/operations/verify.mjs` declares a mode (or a sibling operation) covering
   ...
   `@operation-home-ok: #3636` markers are removed because the brief now calls the operation.
