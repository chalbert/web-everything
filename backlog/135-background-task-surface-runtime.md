...
The [Background Task Surface](/blocks/background-task-surface/) block (#128) is specified at `status: draft` — fui:blocks.json entry, design decisions, dimensions, events, and interface — but has **no runtime** (`sourcePath`). This item builds the actual custom element so the block can graduate `draft → active`.
