...
- **Home (Fork 1)** — does the naming policy get its own composable intent, or stay per-intent? The expressive-symbol intent ([#587](/backlog/587-author-the-expressive-symbol-rendering-intent-substrate/)) currently owns its *own* accessible-name dimension; `validation` ([we:intents.json:616](../src/_data/intents.json#L616)) is the precedent for an intent owning its aria surface (`aria-invalid`/`aria-describedby`) in-place. But `icon` ([we:intents.json:899](../src/_data/intents.json#L899)) has *no* naming axis (size/weight/style only), and naming recurs identically across icon/glyph/media/image/avatar — the separate-and-decouple case.
...
**Crux:** naming policy (meaningful↔decorative + label source/fallback) recurs unchanged across icon ([we:intents.json:899](../src/_data/intents.json#L899)), expressive-symbol (#587), media, image and avatar. There is no shared home for it today — each intent reinvents it, and ad-hoc `aria-label`/`aria-labelledby` wiring already scatters across the impl ([fui:frontierui/blocks/droplist/Clearable.ts:33](../../frontierui/blocks/droplist/Clearable.ts#L33), [fui:frontierui/blocks/tabs/TabGroupBehavior.ts:177](../../frontierui/blocks/tabs/TabGroupBehavior.ts#L177)).
...
**On resolution** (when ratified): if Fork 1 = A, graduate to an `accessible-name` entry in `we:intents.json` (the intent build) and a migration note on #587 to compose it. No Technical Configurator card — naming is a UX/a11y dimension, not an impl/technical setting.
