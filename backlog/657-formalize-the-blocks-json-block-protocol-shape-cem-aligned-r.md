...
Gate: `npm run check:standards` green (64 blocks, 0 errors). The physical delete of WE's vendored `blocks/` + migration of the 9 WE-only families is **#658** (out of scope here); emitting a real `we:custom-elements.json` is a deferred sub-build.
