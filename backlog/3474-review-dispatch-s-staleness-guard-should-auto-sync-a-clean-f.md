...
- 2026-09-14: **A complementary mitigation landed** (`we:scripts/conveyor/main-ref-sync.mjs`, epic #3383) — NOT a build of this item. Found live during a PR-queue automation audit: a real resident driver checkout's local `main` sat 161 commits / 33+ hours behind `origin/main` (nothing in its own operating loop ever touches `main` — its real work happens on `lane/mechanical-dispatcher`), so `assertMainNotStale` refused EVERY fix and review dispatch, every tick, silently (swallowed by the runner's best-effort `runQuiet` wrapper), for over a day, with a real capacity-available dispatch plan (`we:scripts/conveyor/reconcile-pass.mjs`) computing real work that never once fired. `we:scripts/conveyor/main-ref-sync.mjs` is a new, separate mechanical pass — a plain `git fetch origin main:main` (ref-only, never touches the working tree), wired into `we:skills-src/conveyor/runner.mjs`'s mechanical passes FIRST, before `we:scripts/conveyor/reconcile-fix-dispatch.mjs` and the review-dispatch reconcile step — that keeps local `main` from going stale in the first place, so `assertMainNotStale` rarely has anything to refuse on. **This does not build item #2/#3474's own fix** (making the guard itself auto-fast-forward on a clean staleness) and this item stays open. Worth reading before building it: `we:scripts/conveyor/main-ref-sync.mjs`'s own header notes a real gap in the design text above — `git pull --ff-only` / `git merge --ff-only origin/main` (as written in "The fix, precisely") assumes the DISPATCHING checkout has `main` itself checked out; on a checkout permanently parked on a different branch (this resident driver's own shape), either operation would fast-forward/merge the WRONG branch, not update `main`'s own ref. Any implementation of this item should read the current checked-out branch and choose the ref-only-fetch shape `we:scripts/conveyor/main-ref-sync.mjs` already uses whenever `main` is not what is checked out, rather than assuming a bare `--ff-only` pull is always correct.
