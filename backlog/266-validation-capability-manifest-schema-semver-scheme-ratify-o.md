...
[we:capability-manifest/provider.ts](../capability-manifest/provider.ts) (sibling to the validity-merge
...
current spec version — a `const` in the model, **not** an `we:intents.json` field. The capability manifest
is a conformance/protocol artifact and is kept out of the UX-only Validation Intent (per the
intent-UX-only rule), deviating from the older brief's "record it in we:intents.json" suggestion which
...
- **Done:** Authored the `capability-manifest/` standalone model (`we:provider.ts` — `CapabilityManifest`
  ...
  `assertCapabilityManifest` guards) + `we:index.ts` barrel + `we:__tests__/provider.test.ts`. Re-exported the
  vocabulary from `we:plugs/webvalidation/index.ts`. Ratified OP-18/19/20 above.
- **Next:** Downstream slices consume the model — #267 (build-time `check:validation-adherence`), #268
  ...
  now unblocked.
- **Notes:** No `we:intents.json` / `we:protocols.json` change — the manifest is intentionally a code-model
