...
- 2026-09-25: graduation-import-check added blockedBy #x0n0eiz — the moved-in `we:scripts/__tests__/check-standards-rules-backlog-integrity.test.mjs` also imports a module #x0n0eiz owns.
- 2026-09-25: graduation-import-check moved `we:scripts/__tests__/check-standards-rules-backlog-integrity.test.mjs` here from #x0n0eiz — it imports a module this card owns.
- 2026-09-25: graduation-import-check added blockedBy #x0n0eiz — `we:scripts/pr-land.mjs` imports a module #x0n0eiz owns.
- 2026-09-25: graduation-import-check added blockedBy #x0n0eiz — `we:scripts/operations/deliver-item-wrapper.mjs` imports a module #x0n0eiz owns.
- 2026-09-25: graduation-import-check moved `we:scripts/operations/__tests__/telemetry.test.mjs` to #xvlu8t1 — it imports a module #xvlu8t1 owns.
- 2026-09-25: graduation-import-check moved `we:scripts/operations/__tests__/host-process-sample.test.mjs` to #xvlu8t1 — it imports a module #xvlu8t1 owns.
- 2026-09-25: graduation-import-check added blockedBy #x0n0eiz — `we:scripts/lib/review-escalation.mjs` imports a module #x0n0eiz owns.
- 2026-09-25: graduation-import-check added blockedBy #x0n0eiz — `we:scripts/lib/gh-throttle.mjs` imports a module #x0n0eiz owns.
- 2026-09-25: graduation-import-check added blockedBy #x0n0eiz — the moved-in `we:scripts/lib/__tests__/gh-throttle.test.mjs` also imports a module #x0n0eiz owns.
- 2026-09-25: graduation-import-check added blockedBy #x0n0eiz — `we:scripts/conveyor/lease-reaper.mjs` imports a module #x0n0eiz owns.
...
- `we:scripts/operations/__tests__/host-process-sample.test.mjs`, `we:scripts/operations/__tests__/telemetry.test.mjs` → "later: `we:scripts/operations/telemetry.mjs`, owned by `#xvlu8t1`, fix → move to `#xvlu8t1`". Confirmed a pure snapshot-drift artifact: re-running the same check at `--snapshot=600acc14f` makes both disappear (`we:scripts/operations/telemetry.mjs`'s content differs between the two snapshots only via commit `e14f27bef`, `MAX_ATTRIBUTE_KEYS` 40→48, which `#xvlu8t1` — not this card — will port).
- `we:scripts/conveyor/lease-reaper.mjs`, `we:scripts/lib/gh-throttle.mjs` (+ its test), `we:scripts/lib/review-escalation.mjs`, `we:scripts/operations/deliver-item-wrapper.mjs`, `we:scripts/pr-land.mjs` → "later: `we:scripts/lib/lane-lease.mjs` / `we:scripts/readiness/heavy-admission.mjs` / `we:scripts/lane-pool.mjs` / `we:scripts/check-standards-rules.mjs` / `we:scripts/lib/lane-verify.mjs`, owned by `#x0n0eiz`, fix → move here / add blockedBy" — these are real static-import facts, not snapshot drift, but the proposed fix (move ownership into this card, or add a `blockedBy: x0n0eiz`) does not apply to an already-built PR: `#2636` was built against the pre-`x0n0eiz`-delta (resolved-card-baseline #3481/#3484/#3890/#3917) content of every one of those files, so this card genuinely needs neither the files themselves nor a landing-order dependency on `#x0n0eiz`. (An earlier pass DID add `blockedBy: x0n0eiz` here per the tool's suggestion, but that created a genuine `x0n0eiz → 3915 → x0n0eiz` cycle against `#x0n0eiz`'s own, real `blockedBy: 3915` — dropped in favor of the direction that is actually load-bearing: `#x0n0eiz` needs THIS card's `we:scripts/lib/review-core.mjs`, not the reverse.)
