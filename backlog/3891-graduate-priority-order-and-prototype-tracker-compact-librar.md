...
1. **Executable** — `npx vitest run we:scripts/__tests__/priority-order.test.mjs we:scripts/__tests__/prototype-tracker-render-real.test.mjs we:scripts/lib/__tests__/prototype-tracker-compact.test.mjs` passes on main's tree (all of this slice's tests; each fails before the port because its module is missing or differs).
