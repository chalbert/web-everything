...
| **Fork 3 · semantic-zoom / LOD** | **MINT a representational-zoom intent** (distinct from geometric `viewport-transform`) | keep app-custom (compose nav + `hierarchy` + `density`) | **yes** — shipped as a named control (MS `SemanticZoom`); Maps LOD; Figma/Miro; Photos Years→Days; Calendar views; Shneiderman's mantra | **Med-high** |
