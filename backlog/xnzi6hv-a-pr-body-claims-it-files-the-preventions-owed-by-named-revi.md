...
A PR body can say it files the preventions the reviews on named PRs marked `Prevention (OWED — file it)` while one of those markers has no card in the diff. The claim asserts the debt is discharged, so nobody goes looking. Both halves are greppable: the markers in the named PRs' review comments, and the `backlog/*.md` paths the diff adds. Prevention owed twice by CONFIRMED findings on PR #1563, whose body asserted closure over an owed set it had undercounted — first as two of three, then as three of four.

## Why this needs a check rather than more care

It has now failed twice on the same PR, with the author trying. The failure is not carelessness about the
claim — it is that the denominator is genuinely hard to see by eye. The markers are scattered across every
review comment on every named PR, several rounds deep, and the obvious reading ("the latest review on each")
silently drops earlier rounds. Round 1 of #1563's review counted the latest comment of each PR and got three;
round 2 counted every comment and got five. Both counts were carefully derived. Only one was the number the
body's own sentence claimed.
...
3. **Executable** — a body that names the shortfall as a deliberate deferral, with a tracking id, reports
   none. Disclosure is the fix the check is trying to force; it must not then fire on it.
4. **Executable** — a body that mentions a PR without claiming to file its preventions reports none.
5. **Mutation** — dropping the self-disclaim exclusion reddens case 1 by count (two shortfalls, not one);
   dropping the deferral negation reddens case 3 and nothing else.
