...
- **Playground lane** — `fui:demos/__tests__/playgrounds.spec.ts`: asserts each playground loads with
  `playgroundReady === true`, all badges green (`.summary.pass`, no `.badge.fail`, `playgroundPass > 0`),
  zero console/page errors. FUI ships no `we:demos.json` registry, so the playground set is an explicit list
...
- **Service-worker lane** — added a `chromium-sw` Playwright project (`serviceWorkers: 'allow'`) and gave
  ...
  `fui:demos/__tests__/durable-tier-verification.sw.spec.ts` (ported from WE) proves register → hard-reload →
