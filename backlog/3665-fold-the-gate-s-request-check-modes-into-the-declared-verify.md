...
undelegated raw home, marked `@operation-home-ok: #3665` meanwhile. Fold `request`/`check` into `verify`'s
...
1. **Executable** — `we:scripts/operations/verify.mjs` declares a mode (or a sibling operation) covering
   ...
   `@operation-home-ok: #3665` markers are removed because the brief now calls the operation.
