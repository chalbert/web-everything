...
The `autocomplete` entry in [src/_data/blocks.json](../src/_data/blocks.json) describes the
...
- *"…the controller param (**now real in plateau FocusDelegation.ts**)…"* — should point at Frontier
  UI's `blocks/droplist/FocusDelegation.ts`, not plateau.
- *"clearable, filter, live-status, and windowed are **spec-proposed trait surfaces (only
  ...
  `live-status` are now **built and tested** in `frontierui/blocks/droplist/` (`Clearable.ts`,
  `Filter.ts`, `LiveStatus.ts`). Only `windowed` remains genuinely unbuilt.
...
- **Done:** Updated the two stale phrases in [src/_data/blocks.json](../src/_data/blocks.json)
  autocomplete entry. (1) `controller param (now real in plateau FocusDelegation.ts)` →
  `Frontier UI's blocks/droplist/FocusDelegation.ts`. (2) The "spec-proposed trait surfaces (only
  focus-delegation, selection, and anchor are built today)" claim was *itself* more stale than the
  item knew — `windowed` is also built (`Windowed.ts`, 244 lines, tested in `behaviors.test.ts`).
