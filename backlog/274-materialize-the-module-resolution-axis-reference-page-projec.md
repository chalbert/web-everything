...
1. **Reference page** — [we:src/module-resolution.md](../src/module-resolution.md): the four native
...
2. **Config field / model** — [we:module-resolution/provider.ts](../module-resolution/provider.ts)
   ...
   [we:module-resolution/__tests__/provider.test.ts](../module-resolution/__tests__/provider.test.ts) (11);
   added the dir to `we:vitest.config.ts` includes.
3. **Exports-lock lint** — `validateModuleResolutionLock` + `isExportsSafeTarget` (pure, fixture-tested
   in we:check-standards-rules.test.mjs, 6 cases) wired into `we:check-standards.mjs` (§9b): gathers every
...
`we:demos/maas-consumer-demo.html` maps `@frontierui/jsx-runtime` → `we:/blocks/renderers/jsx/index.ts` (WE
