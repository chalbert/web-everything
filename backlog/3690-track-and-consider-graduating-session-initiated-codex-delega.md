...
padded. Total store records: **9** (this file's own first population on `main`).
...
1. **Executable** — `node we:scripts/conveyor/log-delegation-trial.mjs --help` exits 0 (fails today: the
   file does not exist before this lands), and reading `we:scripts/conveyor/run-scorecards.json` shows at
   least 9 rows with `dispatchKind: "session-delegation"` (fails today: zero such rows exist before this
   lands).
