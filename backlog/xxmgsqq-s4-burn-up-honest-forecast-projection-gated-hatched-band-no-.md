...
Burn-up capped at the unblocked ceiling, hatched gated-no-date band, dashed projection to the ceiling (not a date), now-marker, milestone chips. Consumes #2687. Explicit named surface + own baseline for M6 (no-forecast). Registers into the section registry; honest-forecast DOM guard passes.

## Deliverable
Burn-up capped at the unblocked ceiling, a hatched gated-no-date band above, a projection dashed to the ceiling (not a date), a "now" marker, milestone chips (a gated milestone reads `gated · no date`). Consumes #2687; a blocked feature projects only its unblocked remainder. Explicit named surface + own baseline for M6 (no-forecast: delivered curve only, no projection). Registers into the section registry.
