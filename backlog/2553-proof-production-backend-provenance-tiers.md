...
claimed. Blocked on the spec-schema fork F3 ([#2545]).
...
- **Confidence source** wiring — feed the launch gate's confidence (F2, [#2545]) from real signals, not a
  fixture.
...
gate that requires a higher tier. The board's proof rows ([#2550]) consume this.
