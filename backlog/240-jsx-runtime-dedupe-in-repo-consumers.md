...
(`frontierui/packages/jsx-runtime/src/JSXRenderer.ts`) to the WE canonical and add a drift guard so they
...
#125 created `@frontierui/jsx-runtime` (`frontierui/packages/jsx-runtime/src/JSXRenderer.ts`) by
...
#239 built out the package source (`packages/jsx-runtime/src` now has `index.ts` / `jsx-runtime.ts` /
`jsx-dev-runtime.ts` / `auto-define.ts` / `JSXRenderer.ts` — the half-built `dist` concern is gone) and
...
1. **The package copy is stale vs the WE canonical, not a clean mirror.** `frontierui/packages/jsx-runtime/src/JSXRenderer.ts`
   (293 lines) is missing two features WE's canonical `webeverything/blocks/renderers/jsx/JSXRenderer.ts`
...
2. **The browser resolution topology for the bare specifier is unsettled.** FUI serves jsx via a vite
   path-inject — `vite.config.mts` `jsxInject: import jsx from '/blocks/renderers/jsx'`, with `index.ts`
...
**This slice (#240) — the decision-free foundation:** (a) re-sync package `JSXRenderer.ts` from WE
...
`JSXRenderer.test.ts` green.** No consumer repointing, so no resolution decision is needed here — that's
why it batches now.
