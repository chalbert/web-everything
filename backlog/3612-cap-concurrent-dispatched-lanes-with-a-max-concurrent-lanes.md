...
## Finding (2026-09-21): the weights and the budget get a tracked home, card x5fkzgl

The finding above puts the weighted budget in the resolver of `we:scripts/lib/lane-concurrency.mjs` but leaves the numbers in the orchestrator's local notes. Card x5fkzgl gives the resolver a tracked `dispatch-budget` config (budget, weight per kind, ramp steps, knee thresholds, emergency floor, and a dated reason for each change) read by `dispatch-plan`, `tick-core`, `dispatch-lane` and `dispatch-task`, so a handoff loses nothing; it extends this card and folds in #3727. Two facts to keep straight: the repo's "8" here is a lane COUNT, while rule v4's "8" is 8 units of weighted work; and `we:scripts/operations/dispatch-lane.mjs` and `we:scripts/operations/dispatch-task.mjs` still read no ceiling. Whether one weighted budget is enough, or heavy and light workers need separate counts, is the open decision x3pvhaf (default: one budget plus the heavy pool).
