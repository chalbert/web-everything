...
skill (child `#xm6wpei`) would be a third. This item removes the duplication by extracting **one** review core.
...
allowed to clear what — that policy stays with each caller (the drain owns its leash; see `#xm6wpei`).
...
- **the drain auto-review** (`#xm6wpei`) — core over the PR diff in a fresh subagent → maps `verdict` → label.
- **`/review`** (`#xm6wpei`) — core over the PR diff → shows findings + reason → operator sets the label.
...
- The drain re-point + the duplicate prose removal ride the child `#xm6wpei`; this item lands the core + the
