...
provider at the CLI boundary in [scripts/propose-readiness.mjs](scripts/propose-readiness.mjs), never
...
- The pure engine (`scripts/readiness/proposer.mjs`) is unchanged — resilience lives at the CLI seam.

## Resolution (2026-06-10)

Extracted the BYO-key model provider out of the side-effectful CLI entry into a testable unit
`scripts/readiness/model-proposer.mjs`, and wrapped its Messages-API `fetch` in `fetchWithRetry` —
...
The pure engine (`proposer.mjs`) and the #250 deterministic core are untouched. Zero-dep stance kept
(hand-rolled ~20-line backoff rather than adding the Anthropic SDK for one call). Covered by
`scripts/readiness/__tests__/model-proposer.test.mjs` (13 tests: 429/529 retried, 400/401/403 not,
