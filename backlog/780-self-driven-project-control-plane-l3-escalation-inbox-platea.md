...
Sibling panel of the control-plane shell (#674). Plain-words 'X requests your decision' inbox over GateRecord.escalatedTo (plateau:src/profiles/gate-enforcement.ts:163 escalate, :32 GateRecord) + roster escalation strings (plateau:src/profiles/roster.ts); approve/refuse wired to GateEnforcer.signOff/refuse (:135/:145). New plateau:src/control-plane/escalation-inbox.ts on the mountProfiles pattern. plateau-app product layer (#091).
...
- `mountEscalationInbox()` renders the queue with approve/refuse buttons + a localStorage decision log; styles added to `plateau:control-plane.css`; wired into the `/control-plane` route (`we:index.html` + `plateau:main.ts`) below the dashboard.
- Tests: `plateau:escalation-inbox.test.ts` (5/5) pin the gate selection, the request text, and that approve/refuse round-trip through the enforcer (passed / failed+escalatedTo). Full plateau-app suite green (22 files, 177 tests); my files tsc-clean (pre-existing `@frontierui/*` resolution errors unrelated). Browser-verified on `:4000/control-plane`: 4 requests render with working buttons.
