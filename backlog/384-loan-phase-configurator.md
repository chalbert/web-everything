...
- **2026-06-15 — built (exercise app A).** New `/pricing` ("Product & Pricing") module:
  - `demos/loan-origination/configurator/productConfigurator.ts` — the **constraint graph**:
    ...
    cash-to-close. Pure functions over the existing `catalog.ts` pricing domain + `facts.ts`.
  - `demos/loan-origination/configurator/productConfiguratorView.ts` — eligible products as selectable
    cards, ineligible ones dimmed with the failing constraint(s); a lock selector re-prices; eligibility
    chips **reuse the active status-indicator block** (platform-first). Wired into `app.ts` (`/pricing`
    module + skeleton + mount on entry, pricing a representative book application) + styles in `app.css`.
