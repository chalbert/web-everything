...
- NEW `we:scripts/lib/buildReport.mjs` — a plain-object constructor matching the #431 report-model contract
  ([we:blocks/renderers/report/renderReport.ts](../blocks/renderers/report/renderReport.ts) — `sources` /
...
- Extend [we:scripts/check-standards.mjs](../scripts/check-standards.mjs) `--json` to emit a `Report`:
...
- **C** — `check:app-conformance` (+ the `--burndown` **series**, [we:check-app-conformance.mjs:170](../scripts/check-app-conformance.mjs#L170)) → model (coverage-matrix `section` + `series[]`).
- **D** — capability-manifest adherence ([we:capability-manifest/report.ts:27](../capability-manifest/report.ts#L27)) → model `section`.
...
- **Authored `we:scripts/lib/buildReport.mjs`** — the shared producer-side constructor: `buildReport()` +
  `source()`/`finding()`/`section()`/`score()`/`series()` factories matching the #431 contract
  (`we:blocks/renderers/report/renderReport.ts`). Pure JS, no TS import. Drops `undefined` optional keys and
  asserts the model's required fields (incl. a dangling-`source`-ref guard the export adapters rely on).
- **Migrated `check:standards --json`** ([we:scripts/check-standards.mjs](../scripts/check-standards.mjs)) to
...
- **Regression guard** `we:blocks/__tests__/unit/lib/buildReport.test.ts` proves the helper's output pipes
...
- **Demoable state met:** `node we:scripts/check-standards.mjs --json` emits a model-valid report
