...
4. **Cache-key reconciliation.** Fold the content hash into the existing `ServedArtifactCache` key in [we:definitionRegistry.ts](../blocks/renderers/module-service/definitionRegistry.ts) so the in-process cache and HTTP cache key on the same identity.
5. **Vite middleware becomes a caller.** Rewrite [we:tools/maas/vite-plugin.ts](../tools/maas/vite-plugin.ts) to delegate to the shared Fetch handler via a thin Fetch↔Node-stream adapter; keep the `X-MaaS-Lossy`/`X-MaaS-Diagnostic` diagnostics.
...
- **New [we:blocks/renderers/module-service/fetchHandler.ts](../blocks/renderers/module-service/fetchHandler.ts)** —
...
- **[we:definitionRegistry.ts](../blocks/renderers/module-service/definitionRegistry.ts)** — folded the
...
- **[we:tools/maas/vite-plugin.ts](../tools/maas/vite-plugin.ts)** — rewritten as **one caller** of the
...
- **Tests** — new
  [we:blocks/__tests__/unit/renderers/fetchHandler.test.ts](../blocks/__tests__/unit/renderers/fetchHandler.test.ts)
...
**Graduated to** `we:blocks/renderers/module-service/fetchHandler.ts` — MaaS distribution origin serve path (webadapters).
