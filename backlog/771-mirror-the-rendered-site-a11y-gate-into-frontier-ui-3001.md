...
- **Allowlist:** `frontierui/tests/a11y/route-allowlist.ts` — FUI's own curated route set (`/`, `/blocks/`, `/adapters/`, `/demos/`, `/plugs/`, `/traits/`, `/about/`) + the WCAG 2.0/2.1 A+AA tag set.
- **Gate:** `frontierui/tests/a11y/rendered-site-a11y.spec.ts` — same warn→enforce ratchet (`A11Y_ENFORCE=1` / per-route `enforce`), pinned to the FUI docs origin (`:8082`) so `/` gates the real site home not the Vite `:3001` shell. Wired into `frontierui/playwright.config.ts` `testMatch`.
