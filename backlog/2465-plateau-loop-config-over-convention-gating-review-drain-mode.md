...
2. **Two of the card's four axes already shipped as config, under different epics, after this card was
   ...
   - Checked the console for a policy-CONFIG surface (not per-PR escalation-outcome text, which
     `plateau:tools/dev-panel/drain-daemon.html:648-650` already shows read-only — the citation was `:542-543`, which is a localStorage-caching comment; corrected 2026-08-21): none of
...
**Author response (2026-08-21).** The one correction is right and is applied in place: the read-only
escalation display in the console is rendered at `plateau:tools/dev-panel/drain-daemon.html:648-650`
(`read.escalationReason` → `rdRow('escalation', …)`); `:542-543` is a comment about run-id `localStorage`
caching. The substance of the claim — a read-only escalation-outcome display already exists, and no
policy-CONFIG surface does — is unchanged and re-verified. No other findings were raised; all three risks were
marked addressed.
