...
POC + repeatable pattern for #604: add a one-line {% fuiDemo "fui:autocomplete-unplugged.html" %} embed to the autocomplete block description partial (the droplist-family demo FUI already hosts), mirroring we:component.njk:235; document the per-block demo→block mapping convention (demoFile in fui:blocks.json or the partial) so remaining blocks roll out as one-liners. Additive — static code sample retained.

## Progress (2026-06-15, batch-2026-06-15)

- **Embedded the demo:** added a `<h3 id="try-it-live">Try it live</h3>` section + the one-line
  `{% fuiDemo "fui:autocomplete-unplugged.html", "Autocomplete — editable, filterable droplist", 520 %}` to
  [we:src/_includes/block-descriptions/autocomplete.njk](../src/_includes/block-descriptions/autocomplete.njk),
  near the top after the intro (mirrors `we:component.njk:235`). Verified `fui:autocomplete-unplugged.html` exists
...
- **Documented the rollout convention:** new section in
  [we:docs/agent/demo-workflow.md](../docs/agent/demo-workflow.md) — "Embedding a Frontier UI demo on a block
  page — the `fuiDemo` convention (#701/#604)": the mapping is **declared in the block's `.njk` partial**
  (a single `fuiDemo` line under `Try it live`), **not** in `fui:blocks.json` (which stays a pure registry);
  demo file naming = `<concept>we:-unplugged.html` (the #606 public surface); additive, never a replacement.
