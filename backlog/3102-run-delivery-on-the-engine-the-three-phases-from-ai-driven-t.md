...
**[#2464], the agent-runner CLI backend** — the one open item that removes agent-spawning from the session.
Nothing downstream reaches zero-session without it. [#2612] is the interim main-session operator and is
explicitly a bridge, not the end state.
