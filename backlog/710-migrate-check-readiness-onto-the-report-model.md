...
Slice B of #435's reporter-migration fan-out: extend check-readiness.mjs --json/--select to emit a Report (ranked selection + batch pack as a section with scores[]), reusing buildReport() from #435. Terminal path stays bespoke.
...
- **Authored `buildReadinessReport(selection, batchPack, budget)`** in the pure readiness engine
  ([scripts/readiness/engine.mjs](../scripts/readiness/engine.mjs)) — re-expresses the ranked selection +
...
- **Wired it into `--json`** ([scripts/check-readiness.mjs](../scripts/check-readiness.mjs)) under a new
  ...
  `--json` view migrates.
- **Tests** ([scripts/readiness/__tests__/engine.test.mjs](../scripts/readiness/__tests__/engine.test.mjs)):
