...
**PHP is the Python case, not the .NET case** — an interpreted runtime with no compile step. So the closest
model is frontierui:plugs/webdirectives/ssr/python/: a `harness.php` implementing the #2354 grading protocol, a
language-native test suite, and a **vitest bridge** that shells `php` and self-skips when neither `php` nor the
sibling WE checkout is present (`it.skipIf(!canRun)`, exactly as the Python bridge does). Whether the bridge is
enough or a named `PHP SSR conformance harness` step + `shivammathur/setup-php` is also wanted in
frontierui:.github/workflows/ci.yml is the one CI call to make at build time — note ubuntu-latest already ships
a PHP CLI, and the item's `scope` already reserves the workflow file either way.
...
`render(source, data) -> str`. The PHP port needs the same public entry point (one `render(string $source, mixed
$data): string`) so `harness.php` is a thin loop, and the same `djb2` key-hash — that hash is wire-format, not an
...
- **Tier 2** — the harness reports honestly on the vectors this slice does NOT implement: the five `for-each` /
  ...
  silently absent from the output. One `--json` read of the harness output proves it.
