...
Conforms to `@webeverything/contracts/credential-management` (the FUI→WE arrow). Added the **WE-side distribution seam** (`we:contracts/credential-management.ts` type re-export of `we:identity/contract.ts` + the `./credential-management` subpath in `we:contracts/package.json` — slice A built only the contract module, not the entry) and the FUI `tsconfig.json` + `vite.config.mts` path-maps + the `webidentity` `fui:src/_data/plugs.json` catalog entry. Covered by `fui:plugs/webidentity/__tests__/webidentity.test.ts` (10 tests, mocked CredentialsContainer). FUI `check:standards` green.
