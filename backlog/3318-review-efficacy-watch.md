...
- **2026-08-26** — **the split result, re-counted.** Every figure below re-derived in this lane at `2026-08-26T17:39Z` by counting the structured verdict comments (`🔁 review — changes requested` / `✅ review — accepted`) with `gh pr view <n> --json comments`, and the file counts with `git diff --name-only <basis> | wc -l` over the exact revisions each verdict recorded as its net basis.
  ...
  **Both pre-split verdicts recorded a test finding too — and one of them is the same test finding.** The 13:15 round (`correctness` lens, three findings) carried a `coverage` finding on `we:scripts/review-corpus/mine-review-corpus.mjs:39` — *"has zero test coverage; no test file exists … at all"*, CONFIRMED, `degraded` — the same class as the post-split flagship. The 14:12 round (`claim-accuracy` lens, three findings) carried `we:scripts/lib/__tests__/review-core.test.mjs:710`, quoting the same `tampered = withCurrentPanel(FIXTURE) + ' AND ONE MORE THING. ' + MUTATION_PROBE_RULE` line and reaching the same conclusion as the post-split `:707` note. Same test, same construction; `:707` is its `it(` line and `:710` the assignment. The prose lens got there **two rounds earlier**.
