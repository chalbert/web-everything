...
- **2026-08-26** — **the split result, re-counted.** Every figure below re-derived in this lane at `2026-08-26T17:39Z` by counting the structured verdict comments (`🔁 review — changes requested` / `✅ review — accepted`) with `gh pr view <n> --json comments`, and the file counts with `git diff --name-only <basis> | wc -l` over the exact revisions each verdict recorded as its net basis.
  ...
  **What bounced the small version was about tests — but the prose lens found it first.** *Counting basis, which differs by side and must be stated: pre-split is the `### Findings` blocks of the two verdict comments only; post-split is the 16:40 verdict **plus** the independent reviewer-notes comment at `16:41:02Z`, because that is where two of the three post-split findings live.* The 16:40 verdict carries **one** finding — `test-coverage`, `we:scripts/lib/review-core.mjs:1074`: the headline addition (`LENS_HUNT_BRIEF` / `huntBriefForLens`) had zero test coverage anywhere in the repo, CONFIRMED, `broken`. It records how that was proved: deleting the two-line wiring from `buildPanelMandate` and re-running the four affected suites (592 tests), all of which stayed green. That figure is quoted from that verdict comment; it was not re-run here. The notes comment adds two more — a guard test at `we:scripts/lib/__tests__/review-core.test.mjs:707` that could not fail under the mutation it claimed to probe, and **a prose finding** (four figures the diff itself formally retracts were still standing in the description).

  **Both pre-split verdicts recorded a test finding too — and one of them is the same test finding.** The 13:15 round (`correctness` lens, three findings) carried a `coverage` finding on the corpus miner under `we:scripts/review-corpus/` — *"has zero test coverage; no test file exists … at all"*, CONFIRMED, `degraded` — the same class as the post-split flagship. (The miner's own file is deliberately not written as a locus: it lands with #1571, still open, so no path under that directory resolves in this checkout and citing one adds a gate-5 warning.) The 14:12 round (`claim-accuracy` lens, three findings) carried `we:scripts/lib/__tests__/review-core.test.mjs:710`, quoting the same `tampered = withCurrentPanel(FIXTURE) + ' AND ONE MORE THING. ' + MUTATION_PROBE_RULE` line and reaching the same conclusion as the post-split `:707` note. Same test, same construction; `:707` is its `it(` line and `:710` the assignment. The prose lens got there **two rounds earlier**.

  **The asymmetry is about evidence strength, not about which lens looks.** The `claim-accuracy` juror rated its own test finding `PLAUSIBLE` / `cosmetic`, and said why: *"I did not execute this in a live clone (analysis is from reading the string construction directly)."* Raising the identical defect to `CONFIRMED` took a reviewer who ran the mutation in the runner; rating the coverage class `broken` took a `correctness` run that did the same. **What belongs in every reviewer brief this programme writes is the mutation probe, not a ranking of lenses** — a finding reachable only by reading gets under-rated by whichever lens reads it, whatever that lens is hunting for.

  > **Retracted — the lesson this paragraph drew was backwards.** It read: *"**What bounced the small version is still the sharper finding.** Both post-split findings on #1569 were about tests, not prose … The two pre-split rounds recorded **four** `claim-accuracy` findings between them … and **no test finding at all**. Prose accuracy is cheap to find and rarely what matters; that asymmetry belongs in every reviewer brief this programme writes."*
  > **"no test finding at all"** — false on the entry's own counting basis. Both pre-split verdicts recorded one, and the 14:12 `:710` finding *is* the post-split `:707` finding. One of the four `claim-accuracy` findings was itself the test finding, so the two halves of the sentence contradict each other.
  > **"Both post-split findings … were about tests, not prose"** — the 16:40 verdict carries **one** finding, not two; the second test finding is in the `16:41:02Z` notes comment, which also carried a prose finding. Pre-split was being counted from verdicts and post-split from verdicts *plus* notes, with neither basis stated — and the post-split side was not prose-free either.
  > The **four** `claim-accuracy` figure itself stands (1 + 3, re-counted from those two verdict comments). Only the lesson drawn from it was wrong, and it is restated above at the strength the record supports.

  **Preventions filed, not just named.** Both rounds recorded a `Prevention (OWED — file it)` and neither reached the board until now; round 2 flagged round 1's as still missing, which is the same fix-only-where-quoted pattern one level up. Filed as children of this card: *"Check a quantitative review-log claim against the verdict record before writing it"* (this entry's own defect class) and *"Warn when a scope entry basename matches a tracked file at a different path"* (the `we:scripts/lib/__tests__/lane-verify.test.mjs` typo on #3321 — a path nothing validates today, since `we:scripts/check-standards-rules.mjs` checks that a `scope:` entry is repo-qualified but never that it resolves).

  > **Retracted earlier, in the same paragraph — it first said *"Two rounds of the `claim-accuracy` lens … found nine wrong figures."*** Two figures there are wrong. Only **one** of the two pre-split rounds ran the `claim-accuracy` lens (14:12; the 13:15 round ran `correctness` and merely categorised one finding under `claim-accuracy`). And the recorded finding count across both rounds is **four**, not nine — nine is not a count anything in the verdict record produces.
