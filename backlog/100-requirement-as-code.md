...
- **Meta-schema format + webcases relationship → tracked as decision [#714](/backlog/714-requirement-meta-schema-bdd-like-format-relationship-to-webc/) (parked).** The gating fork — the BDD-like intent/role/state *format* (standardize the format, not a fixed requirement list) and whether requirements *compile down to* webcases (recommendation: yes) — was previously an inline "recommended" note here. `/split 100` (2026-06-15) found that buried fork is why #100 can't split (you can't split away a fork), so it now lives in its own card. It un-parks when slice A is picked up.
...
- This is the gating dependency for the strong form of the evergreen vision; once #714 ratifies, sequence the authoring+validation slice first as a standalone win.
