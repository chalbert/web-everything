...
The earlier blocked-in-fact note's premise was wrong: #1071 did NOT build the runtime in FUI/plateau-app — it graduated to `we:process/provider.ts`, and `we:process/` now ships the full runtime (`we:process/contract.ts` + `we:process/provider.ts` + `we:process/registry.ts` + `we:process/driver.ts` + `we:process/index.ts`). Both `blockedBy` edges (#1070/#1071) are resolved. So a WE-side browser demo exercising the runtime is exactly the right slice C.
...
Verified live on the running dev server (Playwright on `:3000`, the `we:demos/webprocess-conformance-demo.html` page): **10/10 invariants hold**. (The 7 unrelated parser-module 500s in the console are a concurrent session's in-flight files — the known-good analytics conformance demo shows the identical set; not caused by this demo, which imports only `we:process/`.) `we:AGENTS.md` inventory regenerated.
