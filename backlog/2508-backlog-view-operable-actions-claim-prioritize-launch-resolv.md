...
**The design constraint that shapes this epic: item-mutations are lane-gated.** A backlog frontmatter splice (claim / resolve / etc.) can't run under a shared primary checkout — the guard denies it, and every mutation must ride a lane → PR. So the action path from the UI **routes through a lane**, not a direct write to the target repo. How the console requests, tracks, and reflects that lane-gated mutation is the central design point.

## Slices

Now an epic — the write path is carved into safe, independently-shippable actions. The **write seam + resolve** slice lands the lane-gated mutation pattern first (the foundation the others build on); claim/release, priority, and launch layer onto that proven seam.
