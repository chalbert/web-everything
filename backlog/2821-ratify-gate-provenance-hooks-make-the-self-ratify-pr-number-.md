...
### 8. Predicted-touch-set vs declared care level — a claim about the gate must be *derived from* the gate (candidate, surfaced by #957 round 4)

**The tell.** A `kind: decision` item whose **predicted touch-set** names a file that `we:scripts/lib/gate-config.mjs` registers as `tier: 'policy'` (the gate-self trust chain — `we:scripts/lib/review-escalation.mjs`, `we:scripts/lib/review-core.mjs`, the policy contract/loader), **while** the same item's gate-self note or pre-registered care level says the implementing diff "rides the normal independent review" / is agent-clearable / is only `elevated`. The two are a mechanical contradiction: a policy-tier touch → `humanRequired` → `deriveCareLevel` short-circuits to `high` → `review:human`, never agent-cleared, and `panelRigorForCareLevel('high')` = 3 rounds + 2 jurors/lens. The declared care level and gate-self note are therefore **derivable from `we:scripts/lib/gate-config.mjs`, not authorial**.

**Why it's script-decidable (belongs in this family).** The predicted touch-set is a list of `we:` paths in the item body; `isGateSelfPath` / the `TRUST_CHAIN` tiers in `we:scripts/lib/gate-config.mjs` are the authority. Cross-referencing the two — does any predicted path resolve to a `tier: 'policy'` member? — is a lookup, and the derived care level / gate-self clearance follow deterministically. A "claim about the gate should be *derived from* the gate," same root class as the rest of #2821 (a reference asserted without resolving it against the source). This is exactly what let this item's round-1..3 versions claim "rides the normal independent review" about the trust chain itself; round 4 caught it by hand.

**Real instance it must reproduce.** The **#957 item** `xgtiq7f`: predicted touch-set named `we:scripts/lib/review-core.mjs` (`tier: 'policy'`) while the gate-self note said "rides the normal independent review" and the pre-registered roster was `elevated` / 1-juror-per-lens. The corrected item (gate-self → `review:human`, care `high`, 3 rounds + 2 jurors/lens) must pass. **Status: candidate** — not yet a committed slice of this story; folded here rather than spawned as a new item because it is the same provenance family and clearly deterministic. Promote to a numbered gate when this story is sliced.
