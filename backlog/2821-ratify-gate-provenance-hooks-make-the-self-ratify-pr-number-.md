...
**This class is justified twice over inside #957 alone:** round 1's dead `2819` links and round 4's `#2828` leak are the same defect four rounds apart, both in files the rewrite never touches. The round-4 leak was a `#2828` in `we:src/_data/researchTopics/risk-based-care-scaled-review-gating.json` (which renders on the public `/research/` page) and a `we:backlog/2828-*.md` glob in the report's file table — fixed by naming the decision in prose / citing epic `#2804` instead of the hash. The cheap interim mitigation before this gate lands is a convention, not a hook: **never write a hash slug outside `backlog/` + `docs/agent/`** — cite the epic or name the thing in prose.
