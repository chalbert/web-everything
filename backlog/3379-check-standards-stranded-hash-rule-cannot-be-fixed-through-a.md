...
## Finding (2026-09-21): it recurred three times in one afternoon, and the deadlock is now the main cost

The same miss hit again after the drain merged PR 2400 (four hash-named cards, 19:24Z), then PR 2406 (six) and PR 2403 (two). Once the 180 s grace ended, `check:standards` on main reported the strays, every lane's `verify-lane` and `open-pr` went red, and three workers stopped. This card's Done-when 2 asks that a merge path number the cards of the PR it is merging; the 2026-09-21 cases add a second gap that item does not close: the numbering step in `we:scripts/merge-ai-prs.mjs` sits inside the `landedLocal` block, so a pass with no landable PR never numbers strays, and no PR is landable while verify is red. Filed as the story #xkww2n4 (the drain numbers strays on every pass, through the same transport). The manual repair that worked: `number-stranded` in a non-lane checkout, then `push-if-green` with `--assume-green`.
