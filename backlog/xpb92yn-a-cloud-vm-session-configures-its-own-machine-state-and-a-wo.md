...
The instruction layer travels to a cloud session — `AGENTS.md` is committed — but machine state does not:
...
4. On a durable host a default run leaves `$HOME/.claude/settings.json` byte-identical; on an ephemeral
...
- **`bootstrap-session.mjs`** — host-aware and idempotent. Deploys skills and commands, verifies memory,
  reports constellation siblings, and skips the lane pool and guard with the reason attached.
- **`sync-commands-deploy.mjs`** — slash commands had no deploy path at all; they were live only in a
...
- **`memory-resolve.mjs`** gains the in-repo fallback `check-memory.mjs` already had — the resolver
  hard-failed in any fresh clone while the checker passed, one corpus with two answers.
- **`constellation-repos.mjs`** gains `dirs` per repo plus `repoKeyForDir` / `siblingKeys`, so the
...
- **`planSteps` contract** — `{ id, title }` plus one of `skip` / `info` / `verify` / `argv` / `gitDir`.
...
`bootstrap-session.mjs` has **zero ES importers** outside its own test — every consumer is a subprocess
...
`formatPlans` is the one genuine ES-import consumer touched, from `sync-commands-deploy.mjs`.

## Delivery shape

One PR. The doc and the behaviour must land together — `vm-sessions.md` describes the consent model, and
...
| 2 | host detection rests on spoofable env vars | **dismissed** — no marginal escalation: a repo that can set env through `.claude/settings.json` already registers hook commands in that same file, so it has code execution by a shorter path |
| 2 | `AGENTS.md` row still promised the hook "runs it unasked" | fixed |
...
operations are unaffected (`review-pr` / `review-prep` set `JUDGE_BUDGET_USD = 1.5`); only the converge
