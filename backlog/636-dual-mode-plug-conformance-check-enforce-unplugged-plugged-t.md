...
- Added the pure rule `validatePlugDualMode` to [we:scripts/check-standards-rules.mjs](/scripts/check-standards-rules.mjs) and wired the fs collection (per plug domain under `plugs/web*`: does it have an unplugged-mode test, a plugged-mode test) into [we:scripts/check-standards.mjs](/scripts/check-standards.mjs) as a new section.
...
- Fixture tests added to [we:scripts/__tests__/check-standards-rules.test.mjs](/scripts/__tests__/check-standards-rules.test.mjs) (4 cases: dual-mode pass, plugged-missing error, unplugged-missing backfill warn, non-plug-dir skip). 109 rule tests green.
- `npm run check:standards` green (0 errors, 9 backfill warns).

**Graduated to** `we:scripts/check-standards-rules.mjs` — validatePlugDualMode rule (+ we:check-standards.mjs) — #606 dual-mode plug conformance: ERROR on missing plugged-mode test, WARN→ERROR (PLUG_UNPLUGGED_TEST_ENFORCED) after #649 backfill.
