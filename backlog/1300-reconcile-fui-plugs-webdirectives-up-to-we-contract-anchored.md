...
- **Ported 5 WE-only files** (FUI lacked the comment-directive family entirely):
  `fui:plugs/webdirectives/CustomComment.ts`, `CustomCommentParser.ts`, `CustomCommentParserRegistry.ts`,
  `CustomCommentRegistry.ts`, `multiTemplate.ts` (byte-identical to WE; imports resolve to FUI's
  `../core/*`). Updated `index.ts` to WE's full export surface. Ported their 5 unit tests +
  `webdirectives.unplugged.test.ts`.
- **Reconciled `CustomTemplateDirective.ts`** to WE's #1174 fix: the base `connectedCallback` is now a
  **prototype method** (was an instance-property assignment in the constructor). A custom element's
  ...
  call `super.connectedCallback()`. Ported WE's updated `CustomTemplateDirective.test.ts`.
