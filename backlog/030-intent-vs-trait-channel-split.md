...
- Sibling [#027](027-droplist-trait-language.md) graduated the trait-language into [block:droplist]. The live Droplist block page already carries the **"Two channels: ambient intent vs. trait selection"** table, the resolution rule `explicit (local) ⊕ ambient intent (behavioral only) ⊕ trait default` (structural dims stop at `explicit ⊕ default`), and the narrowed behavioral-only `customContexts:droplistIntent`. Report items A1–A7, B8, B10 are reflected there.
