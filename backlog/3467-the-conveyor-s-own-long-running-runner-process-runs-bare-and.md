...
## Session update (2026-09-07) — a fourth occurrence, same manual cycle, plus one concrete new finding: `we:skills-src/conveyor/runner.mjs` has no signal handler at all today

A fourth real instance of exactly this gap, independent of this item's own filing: `we:skills-src/conveyor/runner.mjs` (live pid, `origin/lane/mechanical-dispatcher` checkout `wev-scratch-dispatcher-4`) had to be restarted by hand again to pick up `8ada649d` ("runner: run mechanicalPasses before dispatchPass -- fixes mechanical-pass starvation", card `xpshzms`) — the exact monitoring-starvation bug this same card's own commit message documents (`#1939` sat with a real merge conflict `we:scripts/conveyor/parked-pr-conflict-watch.mjs` never caught because a large dispatch backlog starved the mechanical passes of a timely turn). The live process was still running the pre-fix code, so it was still live-exhibiting the bug during this restart's own safety check: the runner-singleton lease's `heartbeatAt` (only written after both `mechanicalPasses` and `dispatchPass` finish) sat unchanged for 6+ minutes while `dispatchPass` worked through a ~15-item backlog, one blocking `execFileSync` spawn at a time — directly confirming this card's own "checked via `ps`/log-watching, not just the log" caution, since `runner.log`'s on-disk path had also rotated to a new inode underneath the still-running process's open fd (`lsof -p <pid>` showed a different inode than `stat` on the path), so the path-tail alone under-reported how current the process's own output was.

**New, more specific finding for Done-when #3 ("mechanized and self-verifying restart")**: `we:skills-src/conveyor/runner.mjs` registers **zero** `process.on('SIGTERM'|'SIGINT', …)` handler today (confirmed by direct grep — the only signal wiring in this file's own directory is in `we:skills-src/conveyor/supervisor.mjs`, which forwards `SIGTERM` to its child and its own comment claims that gives "`we:skills-src/conveyor/runner.mjs` a chance to run its own `driveConveyor` `finally` — release the singleton lease" — but `we:skills-src/conveyor/runner.mjs` has no handler to catch that signal and run the `finally`, so a bare `kill <pid>` (or `we:skills-src/conveyor/supervisor.mjs`'s own graceful-SIGTERM-then-SIGKILL sequence) never runs the lease-release path; the lease is left stamped with the dead pid until the 15-minute TTL reclaim. Tonight's restart worked around this exactly the way the prior three did: `rm -rf ~/.claude/conveyor-runner-locks/<hash>` by hand, immediately after confirming (via `ps`) the old pid was actually gone, before relaunching. Whatever mechanism this item builds for #3 needs to either (a) add the missing `process.on(SIGTERM/SIGINT)` handler to `we:skills-src/conveyor/runner.mjs` itself so its own `finally`-based lease release actually fires (making `we:skills-src/conveyor/supervisor.mjs`'s existing comment true), or (b) have the restart mechanism explicitly clear the stale lease itself rather than waiting on the TTL — either is buildable, but it must be a stated choice like Done-when #4 asks for the bookkeeping-loss question, not an unconsidered gap the way it was tonight.

No refile: this is the same interim, bare-`main`-process gap this card already scopes end-to-end (Done-when #1-#6 already cover detect/signal, safe-point, mechanized+self-verifying restart, durable bookkeeping floor, and a regression test) — tonight is additional live evidence for #2 (a real in-flight dispatch backlog to wait out) and a sharper, code-grounded requirement folded into #3 above, not a new fork or a narrower separate item.
