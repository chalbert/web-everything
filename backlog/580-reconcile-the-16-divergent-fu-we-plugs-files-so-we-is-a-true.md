...
ahead in some** (`we:webcontexts/CustomContext.ts`: 24 FU-only lines). Aliasing+deleting would **lose FU runtime
...
Expected WE-ahead (per #449's own list — adopt WE): `we:webinjectors/Injector.ts`, `we:webinjectors/index.ts`,
`we:webregistries/CustomElementRegistry.ts`.

**Unexpected divergences — direction must be verified per file before any deletion** (FU may be canonical):
`we:bootstrap.ts`, `we:core/cloneUtils.ts`, `we:webbehaviors/CustomAttributeRegistry.ts`, `we:webcomponents/cloneHandlers.ts`,
`we:webcontexts/CustomContext.ts` (FU ahead, proven), `we:webcontexts/Node.contexts.patch.ts`,
`we:webexpressions/CustomTextNodeParser.ts`, `we:webexpressions/CustomTextNodeRegistry.ts`,
`we:webexpressions/UndeterminedTextNode.ts`, plus 4 divergent test files
(`we:__tests__/e2e/webcomponents.spec.ts`, `we:core/__tests__/pathInsertionMethods.extended.test.ts`,
`we:webcomponents/__tests__/unit/Element.insertion.patch.test.ts`, `we:webinjectors/__tests__/unit/Injector.test.ts`).
...
adopting it; where **FU is ahead** (e.g. `we:CustomContext.ts`), port FU's additions up into WE first (WE is
...
- `we:webinjectors/Injector.ts`, `we:webinjectors/index.ts`, `we:webinjectors/__tests__/unit/Injector.test.ts` — WE-only lines, FU+0.
- `we:bootstrap.ts` — WE carries webvalidation (#215/#224), webguards (#289), data-grid, type-ahead registrations FU entirely lacks; FU's "+14" is only older comment wording + import order.
- `we:core/cloneUtils.ts`, `we:webcomponents/cloneHandlers.ts` — WE has the #454 `hasOwnProperty('options')` fix (native `<select>`/`<datalist>` clone); FU has the old `'options' in node`.
- `we:webbehaviors/CustomAttributeRegistry.ts` — WE has the #320/#321 `createViewportPresenceObserver` refactor; FU has the older inline `new IntersectionObserver`.

**Style/representation divergence — pick WE, no runtime loss:**
- `we:webexpressions/CustomTextNodeParser.ts` + `we:UndeterminedTextNode.ts` — `parserName: string | null` (WE) vs `parserName?: string` (FU). Equivalent; keep WE for consistency.
...
- `we:webcontexts/CustomContext.ts` — **the crux.** WE: `Key`-parameterized `Registry<ContextValue, keyof ContextValue>`. FU: `string`-keyed `Registry` with method-level `get<Key>`, **plus genuinely FU-ahead runtime** WE lacks — `values()`, `entries()`, and a `delete(_key): boolean` signature (WE has `delete(): void`). Not a superset either direction; the converged tree must choose ONE public form.
- `we:webexpressions/CustomTextNodeRegistry.ts` — FU made `ImplementedTextNode` generic (`<any>`) + imports `RootNode`. Same "which generic form" question.
- `we:webregistries/CustomElementRegistry.ts` — both have `upgrade()`; FU types it with a concrete signature + an `ImplementedElement` type, WE uses `Parameters<typeof …upgrade>`. Same type-form family.
- `we:webcontexts/Node.contexts.patch.ts` — FU-ahead robustness (a null-check throwing on an unregistered context + `HTMLInjectorTarget` casts). One-directional-portable, but entangled with CustomContext's resolution.

**Test files (FU-ahead lines, reconcile after the code lands):** `we:__tests__/e2e/webcomponents.spec.ts` (FU+148/WE+162 — both moved substantially), `we:core/__tests__/pathInsertionMethods.extended.test.ts`, `we:webcomponents/__tests__/unit/Element.insertion.patch.test.ts`.
...
- **Resolved 2026-06-14.** Converged `webeverything/plugs/` one-directionally onto the #582 A/A ruling so
  ...
  - **Converged (adopt FU typings, keep WE runtime):** `we:CustomContext.ts` + `we:Node.contexts.patch.ts` +
    `we:CustomTextNodeRegistry.ts` now byte-identical to FU (method-level `get<Key>`, string-keyed Map-shaped
    ...
    `Node.contexts` null-check + `HTMLInjectorTarget` casts). `we:CustomElementRegistry.ts` took FU's typings
