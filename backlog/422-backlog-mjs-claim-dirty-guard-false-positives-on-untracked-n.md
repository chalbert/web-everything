...
# we:backlog.mjs claim dirty-guard false-positives on untracked-new items
...
- [we:scripts/backlog.mjs:71-77](scripts/backlog.mjs#L71-L77) — `isDirty(relPath)` returns `true` for any non-empty `git status --short` output (untracked `??` included).
- [we:scripts/backlog.mjs:85](scripts/backlog.mjs#L85) — `if (v === 'claim' && isDirty(rel)) die(...)` — the comment on line 71 says "Warn (don't block)" but the call `die()`s.
