...
- **Done:**
  - Harness scaffold pre-existed (seeded by #162: `plateau/playwright.config.ts`, `e2e/`, `test:e2e`,
    dedicated port 5180, `reuseExistingServer`).
  - **Spec 1 — autonomous connect (#160 guard):** `e2e/autonomous-connect.spec.ts` + demo
  ...
  - **Spec 2 — autocomplete demo boots green:** `e2e/auto-complete-demo.spec.ts` drives
    `/auto-complete-demo.html` — async card "par → ArrowDown → Enter" commits "Paris" and dismisses
