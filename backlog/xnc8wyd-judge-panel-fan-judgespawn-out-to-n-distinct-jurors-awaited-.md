...
- [ ] `judgePanel({ jurors, runId, maxTotalBudgetUsd, depth, maxDepth, … })` spawns one `judgeSpawn` per entry in
...
- [ ] **Pairwise-distinct sibling ids.** A test asserts that for a roster of N ≥ 3 seats the returned
...
- [ ] **Depth fails closed.** Refused when `depth >= maxDepth`, and refused when `depth` or `maxDepth` is
      `undefined`/`null`/non-numeric. A test covers the unknown-depth refusal explicitly, not just the
      over-cap one.
- [ ] **Aggregate budget is enforced before the first spawn.** With `sum(perJurorBudget) > maxTotalBudgetUsd`
...
- [ ] **No background escape.** The exported surface has no detach/`unref`/fire-and-forget path; a test asserts
...
- [ ] `assertNoForbiddenArgv` still fires per child (a `--bare` juror is refused inside a panel exactly as it is
      alone).
- [ ] Unit tests spawn no process (injected `spawnFn`). Any live test is **opt-in** behind an environment flag, as
...
- **Declaring this as an engine operation.** The engine and its four step kinds are #3032, and `review-pr` is the
  first operation onto it (#3035). This slice is the fan-out layer a `judge` step will call, which is why it is
  **blocked on nothing** — #3028 has landed and `jury-core` is already pure.
- The hosted-tier backend, per #3028 and statute clause 4.

## Neighbours

- **#3028** (resolved) — the single spawn this fans out. Does not fan out.
- **#3032** (open) — the registry + run engine. The consumer, not the producer.
