...
(session report `reports/2026-06-16-parcel-trait-enforcer-config-delivery.md`). **The survey first reshaped
...
- **B — JS-arg via local wrapper (documented default).** `options.traitMap` provided → use it. The project
  authors a tiny local `parcel-resolver-trait-enforcer.js` (`export default traitEnforcerParcel({ traitMap })`)
...
- **A — declarative `loadConfig` (first-class alternative, not rejected).** No `options.traitMap` → the
  ...
  (default export = `traitEnforcerParcel()`, no options) and supplies a `.trait-enforcerrc` / `package.json`
...
**Graduates to build slice [#787](/backlog/787/):** implement `tools/trait-enforcer/parcel-plugin.ts` with
...
`buildTraitManifestSource(options)` (`tools/trait-enforcer/vite-plugin.ts:274-282`) is reused verbatim
across all four shipped adapters (Vite `vite-plugin.ts:289`, Rollup `rollup-plugin.ts:21`, esbuild
`esbuild-plugin.ts:23`, webpack `webpack-plugin.ts:26`); the only Parcel-specific question is **how the
...
- **Dep scope:** a real Parcel build needs the full `@parcel/config-default` stack **+ `@parcel/plugin`**,
  not just `@parcel/core` as #744 assumed. None are installed today (`package.json` carries only `webpack`
...
a constructor argument the way `traitEnforcerVite({ traitMap })` is wired into `vite.config.ts`. How does
...
- **A — idiomatic declarative config.** Publish `@frontierui/parcel-resolver-trait-enforcer`; the project
  ...
  `package.json` `"traitEnforcer"` key, read via the resolver's `loadConfig` →
...
- **B — local wrapper preserving the factory (v2.9.0 relative-path). ← recommended.** Ship
  ...
  `parcel-resolver-trait-enforcer.js` that imports it and passes the Map as a JS arg captured in a closure,
  referenced in `.parcelrc` by **relative path** (allowed since Parcel v2.9.0 — no `package.json` needed):
...
in `tools/trait-enforcer/parcel-plugin.ts` (mirroring the four siblings), add the dev-deps
...
(`tools/trait-enforcer/__tests__/cross-bundler-conformance.test.ts` — Part A manifest byte-identity after
...
mirroring the webpack case in `tools/trait-enforcer/__tests__/multi-bundler.test.ts:101-146`. The test
