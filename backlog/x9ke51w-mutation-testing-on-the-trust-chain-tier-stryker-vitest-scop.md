...
- Add Stryker + the vitest runner, **scoped to a named file set** (start with the trust-chain / engine files the tier predicate names in slice 4, e.g. the `disposition-judge` / review-core cluster under [we:scripts/lib/](scripts/lib/)) — **never repo-wide**.
...
Not repo-wide mutation (cost). Not a correctness claim — the red probes (slice 2) and the human scenario-coverage judgment remain the backstop.
