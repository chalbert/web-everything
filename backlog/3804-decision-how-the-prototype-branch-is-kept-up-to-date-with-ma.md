...
- **Every line carries exactly one next step** and a link to the evidence, so the operator never has to go find the report.
- **It clears itself.** The line disappears on the pass after the conflict clears (a clean merge, or a promotion). The record gets a "cleared" commit, so the history shows it happened.
- **It fails visible.** An unreadable record shows the `unknown` line, never silence.

**Attack on the ops-branch record:** (1) *A hidden branch is invisible.* Only if the surface fails, and the `unknown` line covers that. (2) *A new write path with its own failure modes (auth, a race with another writer).* One file per branch removes the race; a failed push falls back to a local copy and the `unknown` line says the record was not saved. Holds.
...
## Ratified so far (forks 1 and 2 with its sub-fork; this card resolves only when all four forks are ruled)
...
- **Not done before ratifying:** an independent `judgePanel` skeptic on Fork 2. The operator ratified on this session's inline red-team, recorded in Fork 2's update note.
