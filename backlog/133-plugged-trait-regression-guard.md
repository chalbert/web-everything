...
#116 wired Frontier UI's global `we:plugs/bootstrap.ts` to consume the Enforcer's
...
(`fui:demos/lazy-traits.html`) builds its **own** `CustomAttributeRegistry` and calls
...
- Either an **e2e test** (Playwright, alongside `we:plugs/__tests__/e2e/declarative-spa.spec.ts`) that
...
- **Done (all in `/Users/nicolasgilbert/workspace/frontierui`):**
  - `fui:demos/lazy-traits-plugged.html` + `fui:demos/lazy-traits-plugged.ts` — a plugged demo whose
    ...
    registry (contrast `fui:demos/lazy-traits.ts`), so it exercises the real app-wide lazy path.
  - `fui:plugs/__tests__/e2e/plugged-traits.spec.ts` — asserts `data-sortable-ready` /
    `data-sort-direction` appear and the list sorts. An empty/disconnected manifest fails it.
  - `we:playwright.config.ts` — there was **none** in frontierui (e2e specs were carried in by the
...
- **Notes / gotcha:** Vite `bootstrapPatches()` skips injecting bootstrap into any demo HTML that
  already contains the literal string `we:/plugs/bootstrap.ts` — so don't write that exact path in a
  plugged demo's prose/comments (it silently disables the trait path). Documented inline in the demo.

**Graduated to** `fui:frontierui/plugs/__tests__/e2e/plugged-traits.spec.ts` — + demos/lazy-traits-plugged.* + we:playwright.config.ts — standing app-wide lazy-trait regression guard.
