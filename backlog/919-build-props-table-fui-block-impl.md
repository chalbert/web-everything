...
- **`cemToRows.ts`** — pure projection of a CEM `custom-element` declaration into uniform rows across
...
- **`PropsTableElement.ts`** — the `<props-table>` element + `registerPropsTable(tag='props-table')`
...
- **FUI `src/_data/blocks.json`** — new `props-table` entry (sourcePath `blocks/props-table`,
