...
- Added `fui:src/_data/demos.js` — a **scan-based** registry that reads repo-root `demos/*.html`, pulls each `<title>` (stripping the " | Web Everything" suffix), and emits `{slug, name, href}`. New demos appear automatically. Internal dev tooling (`dev-panel` Spec Explorer) is curated out via a `CURATED_OUT` set.
- Added `we:src/demos.njk` (`/demos/`) gallery grid (22 demos after curation) + a "Demos" nav entry in `we:src/_layouts/base.njk`.
