...
and `componentEsbuildPlugin` (`frontierui/compiler/src/component-transform/plugins.ts`), both thin
...
Each is "the same few lines over `compile()`" (per the #231 module doc, `STRATEGY-AXES.md`). This is
...
- **Done:**
  - **Rollup** — `componentRollupPlugin` (`frontierui/compiler/src/component-transform/plugins.ts`), the
  ...
  - **Babel** — documented pre-step path in `STRATEGY-AXES.md` ("Babel — a documented pre-step, not a
  ...
  - Exports wired through `index.ts`; `STRATEGY-AXES.md` Axis-2 section updated; `rollup` added to the
