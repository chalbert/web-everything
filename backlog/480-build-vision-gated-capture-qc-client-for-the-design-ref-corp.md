...
a thin, swappable vision-client seam in `we:scripts/design-refs.mjs` that classifies each captured frame
...
- **Done:**
  - `we:scripts/design-refs/vision.mjs` — the vendor-free seam: 6-verdict taxonomy + `ungated` sentinel,
  ...
  - `we:scripts/design-refs.mjs` — wired the gate into `collect()`: selector fast-path (`confirmed` skips
    ...
    `contentHash` (`we:verdicts.json`), `meta.visionVerdict`. Default `manual` provider ⇒ behaviour
    unchanged unless opted in (offline/CI-safe).
  - `we:scripts/design-refs/__tests__/vision.test.mjs` — 16 tests: taxonomy, gate mapping, registry/seam,
