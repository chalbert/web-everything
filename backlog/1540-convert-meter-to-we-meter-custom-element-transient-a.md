...
- Unit test `MeterElement.test.ts` (4): self-replacement to `fui-meter` + native `<meter>` (config attrs not copied literally), native vocabulary (min/max/low/high/optimum), `valuetext` → `aria-valuetext`, radial SVG `role=meter` gauge + semantic zone. 9/9 meter unit tests green; FUI `check:standards` 0 errors.
