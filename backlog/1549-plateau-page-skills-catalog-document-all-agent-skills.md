...
repo's per-skill frontmatter (the skill `name` + `description`, triggers split out) →
...
SSR primitives (dogfoods FUI #1254), mounted on the `/skills` route (nav link + `<template>` + `plateau:src/main.ts` wiring,
