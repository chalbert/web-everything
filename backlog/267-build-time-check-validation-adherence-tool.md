...
- **Done:**
  ...
  - **`capability-manifest/__tests__/check.test.ts`** (8 tests) — drives `runAdherenceCheck` over `CAPABILITY_FIXTURES` (in-capability → no finding; out-of-capability → finding matching `expectedOutOfCapability`) and `INVALID_MANIFEST_FIXTURES` (each → `malformed-manifest`, incl. the L1-missing-core OP-18 case), plus `formatCheckResult` and the build gate; prints the verdict via `console.log` for build visibility.
  - **`check:validation-adherence`** npm script. Toolchain note: Node 22.1 cannot execute a `.ts` CLI and there is no tsx/ts-node, so the command runs the check **via vitest** (`vitest run capability-manifest/__tests__/check.test.ts`) — the only TS-executing tool — rather than duplicating the TS model in a `.mjs` (which would fork the contract, against single-source). A red gate fails the build as required.
  - Barrel: re-exported `check.js` from `capability-manifest/index.ts`.
- **Gate:** `check:validation-adherence` green (8/8, prints verdict); full `capability-manifest` suite 41/41; `check.ts` `tsc --noEmit` clean; `check:standards` 0 errors.
