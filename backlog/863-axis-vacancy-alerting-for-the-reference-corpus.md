...
- **Resolved 2026-06-17.** Built `scripts/check-axis-vacancy.mjs` (`npm run check:axis-vacancy`):
  treats each `benchmarkCorpus.json` `category` as a coverage **axis** and counts live vs `retired`
...
- **Tested + verified:** `computeAxisVacancy()` is pure — 6-case offline suite
  (`scripts/__tests__/check-axis-vacancy.test.mjs`, green) covers retirement-driven and sweep-driven
  ...
  Documented in `docs/agent/reference-retirement.md` (§ "Coverage floor").
