...
- **Precursor (WE `blocks/background-task-surface/reloadDurabilityAdapter.ts`)** — cast `scope.ServiceWorkerRegistration` to `(Function & { prototype?: object }) | undefined` so the `typeof reg === 'function'` guard narrows to a usable type instead of `never` under FUI's stricter TS. Pure type annotation — no runtime change (WE bg-task tests 30/30 unchanged; WE `check:standards` 0 errors).
