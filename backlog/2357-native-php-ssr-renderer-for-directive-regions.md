...
**Sliced (2026-07-27):** mirrors the JVM (#2355) / .NET (#2360) shape — one foundational slice (greenfield PHP build subtree + parse/dispatch scaffold + shared helpers + the PHP-side conformance harness runner + the two branch-select directives if/switch to prove the pipeline), then two per-directive slices fanning out behind it (for-each keyed+empty+count/key-hash state tokens; resource:loader + defer). Each slice passes its own vector subset byte-for-byte (fixture-driven demo). DAG: foundation → {for-each, resource:loader+defer}.
