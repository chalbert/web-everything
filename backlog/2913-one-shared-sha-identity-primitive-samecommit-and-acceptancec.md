...
`we:scripts/fetch-parked.mjs` can import, and where the strict `REVIEWED_SHA_RE` bound already lives),
export it, and have `sameCommit` become a thin re-export/wrapper so its existing call sites and tests are
untouched. Signature:
...
1. **Executable** — run, from the WE checkout root:
   ...
   ```
   npx vitest run scripts/__tests__/fetch-parked.test.mjs scripts/__tests__/review-escalation.test.mjs
   ```

   It passes with a new case asserting that
...
2. **Executable** — a test pins the preserved posture: `acceptanceCoversHead({})`,
   ...
   into a fail-closed.
3. **Observable** — one cheap grep over `we:scripts/fetch-parked.mjs` shows at most one surviving
   `[0-9a-f]{7,64}` literal: the identity comparison no longer carries its own copy of the bound. (Line 311's
   `headRefOid` shape check may stay — it validates an input, it does not compare two ids.)

   ```
   grep -c '\[0-9a-f\]{7,64}' scripts/fetch-parked.mjs   # → 1 or 0
   ```
