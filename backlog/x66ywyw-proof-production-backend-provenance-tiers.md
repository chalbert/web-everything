...
claimed. Blocked on the spec-schema fork F3 ([#xs62gnh]).
...
- **Confidence source** wiring — feed the launch gate's confidence (F2, [#xs62gnh]) from real signals, not a
  fixture.
...
gate that requires a higher tier. The board's proof rows ([#xaz4dcn]) consume this.
