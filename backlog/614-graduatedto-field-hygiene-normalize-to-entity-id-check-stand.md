...
- **`scripts/normalize-graduated.mjs`** (new; `npm run normalize:graduated`) — classifies every value
...
- **check:standards rule tightened** — `isCanonicalGraduated` (in check-standards-rules.mjs, unit-tested)
  requires the value to LEAD with a resolvable ref (`none` / resolving `<kind>:<id>` / repo-path / bare
  registry id; a trailing annotation is tolerated). check-standards.mjs emits **one aggregated warning**
...
- **Docs** — canonical grammar recorded in `docs/agent/backlog-workflow.md` close-out step. **Tests** —
