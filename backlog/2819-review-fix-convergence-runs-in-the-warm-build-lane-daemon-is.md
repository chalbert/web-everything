...
- **Build-brief discipline (this item's remaining scope):** added a `#build-brief-discipline` statute to
  `we:docs/agent/platform-decisions.md`; added four deterministic gap detectors (`edge-cases`,
  `integration-tests`, `overclaim-scope` for an unbacked "closes X" claim in the body, and `overclaim-title`
  for a slice title that claims full closure on its own) to `we:scripts/readiness/proposer.mjs` (+ tests in
  ...
  `we:skills-src/conveyor/delivery-agent-brief.md` (both for scaffolding a new leftover-work item and for the
  agent's own current build), `we:skills-src/conveyor/fix-agent-brief.md`, and
  `we:skills-src/conveyor/fix-agent-ci-brief.md` so the executing agent applies the same checks to its own
  interpretation of an already-dispatched spec.
