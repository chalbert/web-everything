...
- **2026-06-15 — built + verified (reproducible green, both lanes).** The publishable shell:
  - `bridge-server.ts` — a dependency-free `node:http` localhost server that pipes each JSON `BridgeRequest`
  ...
  - `activate.ts` — the host-only shell: builds the live host from the real `vscode` API, starts the bridge
  ...
  - `protocol.ts` gains `jumpRequestFromUriQuery(path, query)` (the deep-link → jump-request parser, on the
    shared wire contract).
  - `extension/` — the publishable manifest (`engines.vscode`, `main`, `onStartupFinished`, the command) +
    `tsconfig.json` compiling the shell to CJS (`out/vscode-extension/activate.js`).
- **Verified two ways:**
  - **Fast lane (locus gate, `npm test`):** `bridge-server.test.ts` — the real server over a REAL loopback
  ...
  - **Host lane (`@vscode/test-electron`, #685):** extended `__host-tests__/runner.mjs` with a second pass
    that compiles `extension/` and launches the Extension Development Host against it; new
    `real-suite/real-shell.test.cjs` asserts `activate()` stands up the bridge + returns `{activated, port}`,
...
- This closes the host-only surface #134-style note in `host.ts`/`host-adapter.ts` deferred to the
