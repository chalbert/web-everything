...
> fork to decide but a **validation gate whose verdict is: resolve as graduated to the successor epic `2410`.** The survey and a
...
> single successor epic `2410` (superseding the earlier "file under #2285" framing).

## Digest + verdict

**Verdict: settled by precedent on the A/B fork — but the merit review (below) makes the actionable outcome a
BUILD, not two inert stories. Resolve #2398 as graduated to `2410` (successor to #2285) and build the red-CI convergence residue now
...
**Resolve #2398 as graduated to `2410` (successor to #2285) and build the unified convergence loop** — approach-handshake-first, with
...
folded in above. Verdict adjusted from "close as duplicate" to "resolve as graduated to `2410` + build one
...
settled-by-precedent ruling graduated to a single unified build (epic `2410`), not as a merit fork with a pick.

---

*Complements the overlap-stack coordination theme (stacking kills *conflict* round-trips; #2285 already killed
*review* round-trips). Ruling this graduates it to `2410` (the successor epic to #2285); it does not itself build anything.*
