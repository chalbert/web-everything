...
- The drag machinery, the review `<dialog>`, and the hover-verb layer land in this slice's own module
  ...
  extending the board view under `plateau-app:src/backlog-view/` and acting on the cells [#xq8fvck] renders. The queue / merge / bounce /
...
- Opening a decision from a lane, the new-work composer, and the new-spec→constitution loop → [#xprj9ov]
  (the create/decide write affordances). Cell rendering → [#xq8fvck].
