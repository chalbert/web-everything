...
- `src/_data/projects.json` — registered `webtheme` (category `standard`, status `concept`), describing the concrete-value layer, 3-tier taxonomy with the semantic tier owned by the intents, DTCG-adopted-not-coined, native-CSS runtime, complete default set + `extends` override.
- `src/_data/protocols.json` — registered the **Design Tokens (DTCG ↔ CSS)** protocol (`design-tokens`, `ownedByProject: webtheme`, anchor `protocol-design-tokens`): DTCG 2025.10 authoring/interchange + native CSS compile (custom properties, `@property`, `light-dark()`, relative-color/`color-mix()`, WCAG/APCA contrast gate).
- `src/_includes/project-webtheme.njk` — project page body: Mission, Feature Surface (3-tier + format/runtime + theming-axis tables), the Design Tokens protocol section with DTCG + compiled-CSS examples, Composition, Status + open questions.
