...
- `BuildRunStore` (`plateau:src/build-runner/build-action.ts:60-107`) is the long-lived per-run record. It
  carries a single `note` string — the row's ⟳ cell — and **no event history**.
...
1. **There is no plan-todo event.** `RunnerEvent` has no todo/plan variant, and the demuxer throws the payload
   away: `fromAssistant` (`plateau:src/build-runner/events.ts:44-57`) emits `{ type: 'tool', name, id }` and
...
streams events arriving after connect shows a blank pane on every reload. So `BuildRunStore` needs a bounded
per-run event buffer — appended from the existing `onEvent` hook — that the endpoint flushes before switching
to live push. Bound it (a ring, or a cap with a "…truncated" marker); an unbounded buffer in a long-lived dev
server is a leak.
...
3. **Executable — replay before live.** A test over the run store asserts that events recorded during a build
   are readable afterwards in order, and that the buffer is bounded (feeding more than the cap keeps the most
   recent N and marks the truncation). Fails on `main` — `BuildRun` stores no events.
4. **Observable — the endpoint exists and streams.** With the dev server running and a build in flight, a
...
5. **Observable — the existing consumer is untouched.** `plateau:src/build-runner/build-action.ts` still owns
