...
1. **Executable** — `npx vitest run we:scripts/operations/__tests__/claude-otel-collector.test.mjs we:scripts/usage-report/__tests__/usage-report.test.mjs we:scripts/usage-report/__tests__/usage-ledger.test.mjs` passes on main's tree (all of this slice's tests; each fails before the port because its module is missing or differs).
