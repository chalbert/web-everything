...
- **`validation-generation/provider.ts`** — the neutral intent vocabulary `ValidationIntentId` (13 ids:
...
- **`validation-generation/registry.ts`** — the `CustomValidationAdapterRegistry` standalone table
  (`localName` + `define(asDefault?)` / `get` / `has` / `keys` / `resolve` / `defaultKey`, mirroring the
  standalone `validity-merge/registry.ts`) with `adaptersFor(intent)` for the #085 "which adapters comply
...
- Re-exported the whole vocabulary from **`plugs/webvalidation/index.ts`**; registered a vitest glob for
  `validation-generation/**/__tests__/**`.

**Design rulings.**

- **Code model, not a UX intent.** The generation vocabulary is deliberately *not* an `intents.json`
  ...
  Validation Intent (`src/_data/intents.json`, which owns context/level/execution/visibility).
