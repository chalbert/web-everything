 ...
 * @returns {{reviewsOwed:number, dispatched:Array<{prNumber:number, agentId:string|null}>, failed:Array<{prNumber:number, error:string}>, refusals:number, reconcileError:string|null, holdReconcile:Array<object>, holdReconcileError:string|null}}
 */
export function runReviewTick({
  ...
  return {
    reviewsOwed: reviews.length, dispatched, failed, refusals: (plan.refusals ?? []).length, reconcileError: null,
    holdReconcile: holdReconcileResults, holdReconcileError,
  };
}
 ...
 *   holdReconcile:Array<{num:number, remove:string[], repo:string}>, holdReconcileFailed:Array<{repo:string, error:string}>}}
 */
export function runReviewTickAllRepos({ repos = REVIEW_DAEMON_REPOS, tick = runReviewTick, ...tickOpts } = {}) {
  ...
  return {
    repos: perRepo, reviewsOwed, dispatched, failed, refusals, reconcileFailed,
