...
3. **Loop, per item — the full arc, one command per transition:** `claim <NNN>` (wins the race + flips +
   ...
   **gate in the item's own locus** (look up `LOCI[item.locus]` in `check-standards-rules.mjs`: run
   ...
   <!-- @operation-home-ok: #x6ry8mf — this instruction passes no `--body-file`, which `open-pr` requires; the home's own #2332 guard would refuse it too, so the line needs a body before it can be rewired. -->
