...
> decorated strings (`gate-self` / `gate-derivation` / `statute` / `codification` / blast-radius / size /
> dismissed-findings / cross-repo), and a negotiation that hits the round cap or a mandate conflict (below) adds
> `non-convergence` / `mandate-conflict`. Pass them through verbatim.
