...
it reads every `we:.conveyor/learnings/*.jsonl`, re-applies the write-seam scrub (defence in depth),
...
Sonnet is fine — it's bounded. A candidate must clear all four:
...
- **Memory** — write the file + its index pointer line, per the memory-management policy.
- Both land through the **normal lane → PR** transport (`we:scripts/lane-pool.mjs` →
  `we:scripts/pr-land.mjs`), never a direct write to the primary tree.
...
actually routed and the PR is open. Files move to `we:.conveyor/learnings/harvested/<stamp>/`, so a re-run
never re-processes them and the trail stays inspectable.
...
**Survived red-team:** <name each, with sessions×count — or "none (the common outcome)">
**Routed:** <backlog #NNN… / memory <slugs> — via PR #NNN, or "nothing to route">
**Archived:** <stamp, or "not archived — candidates deliberately deferred">
```
...
- **The pool is untracked and machine-local by design.** A cheap in-the-moment append cannot afford a
  lane→PR; the durable artifacts this skill lands are what reaches git.
