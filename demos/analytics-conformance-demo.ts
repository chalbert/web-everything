/**
 * Analytics conformance demo (#1014; slice 3 of the #1294 relocation cascade, #1921).
 *
 * The webanalytics runtime — the `CustomTrackerRegistry` swap registry, the native-first `NoopTracker`
 * default, and the reference vendor adapters — has been relocated to FUI (#449/#1918); its conformance now
 * runs through the **plateau-hosted conformance iframe** (#1788 (b) / #1790) — the plateau-origin runner
 * driving the WE analytics vector suite (#1918: predicate matchers over the recorded-call log — routing /
 * swap-reroute / arg-order / absence / count, #1816/#1847) against FUI's binding
 * (`AnalyticsConformanceBindingFactory`). This page no longer wires in-demo recording-stub backends or
 * hand-rolls in-page asserts (WE holds zero executable, #1282); it embeds the plateau iframe cross-origin
 * and reflects the conformance result it posts back. The visible pass/fail is rendered at plateau-origin
 * inside the iframe; the parent shows the summary and flips the `setPlaygroundReady` flag the e2e smoke reads.
 */
...
summary.textContent = 'Running analytics conformance via the plateau runner…';

const frame = document.createElement('iframe');
frame.className = 'conformance-iframe';
frame.title = 'analytics conformance (plateau-hosted runner)';
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  summary.textContent = `${data.passed}/${data.total} analytics conformance vectors passed (plateau-hosted runner, FUI engine)`;
