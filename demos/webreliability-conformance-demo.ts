/**
 * Web Reliability conformance demo (#1053; W3 of the #1294 relocation cascade, #1922).
 *
 * The reliability (error-recovery) runtime has been relocated to FUI (#1916: the `customRecovery`
 * `CustomRecoveryHandlerRegistry` walking handlers first-accept-wins) and its conformance now runs through
 * the **plateau-hosted conformance iframe** (#1788 (b) / #1790) — the plateau-origin runner driving the WE
 * reliability vector suite (#1919) against FUI's binding (`ReliabilityConformanceBindingFactory`). This page
 * no longer imports the WE reliability contract or hand-rolls in-page asserts (WE holds zero executable,
 * #1282); it embeds the plateau iframe cross-origin and reflects the conformance result it posts back. The
 * visible pass/fail is rendered at plateau-origin inside the iframe; the parent shows the summary and flips
 * the `setPlaygroundReady` flag the e2e smoke reads.
 */
...
summary.textContent = 'Running reliability conformance via the plateau runner…';

const frame = document.createElement('iframe');
frame.className = 'conformance-iframe';
frame.title = 'reliability conformance (plateau-hosted runner)';
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  summary.textContent = `${data.passed}/${data.total} reliability conformance vectors passed (plateau-hosted runner, FUI engine)`;
