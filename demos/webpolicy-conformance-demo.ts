/**
 * Web Policy conformance demo (#1079; W3 of the #1294 relocation cascade, #1801).
 *
 * The webpolicy runtime has been relocated to FUI (#1799: PDP/PEP + ProofChain) and its conformance now
 * runs through the **plateau-hosted conformance iframe** (#1788 (b) / #1790) — the plateau-origin runner
 * driving the WE webpolicy vector suite (#1800) against FUI's binding. This page no longer imports a WE
 * runtime or hand-rolls in-page asserts (WE holds zero executable, #1282); it embeds the plateau iframe
 * cross-origin and reflects the conformance result it posts back. The visible pass/fail is rendered at
 * plateau-origin inside the iframe; the parent shows the summary and flips the `setPlaygroundReady` flag
 * the e2e smoke reads.
 */
...
summary.textContent = 'Running webpolicy conformance via the plateau runner…';

const frame = document.createElement('iframe');
frame.className = 'conformance-iframe';
frame.title = 'webpolicy conformance (plateau-hosted runner)';
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  summary.textContent = `${data.passed}/${data.total} webpolicy conformance vectors passed (plateau-hosted runner, FUI engine)`;
