/**
 * Web Compliance conformance demo (C4 of the #1294 relocation cascade, #1810).
 *
 * The webcompliance runtime — the #437 gate runner, #438 waiver machinery, and #439 audit mapping — is
 * relocated to FUI (#1814; WE holds zero executable, #1282) and its conformance runs through the
 * **plateau-hosted conformance iframe** (#1788 (b) / #1790): the plateau-origin runner driving the WE
 * webcompliance vector suite (#1809) against FUI's binding. This page embeds the plateau iframe cross-origin
...
summary.textContent = 'Running webcompliance conformance via the plateau runner…';

const frame = document.createElement('iframe');
frame.className = 'conformance-iframe';
frame.title = 'webcompliance conformance (plateau-hosted runner)';
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  summary.textContent = `${data.passed}/${data.total} webcompliance conformance vectors passed (plateau-hosted runner, FUI engine)`;
