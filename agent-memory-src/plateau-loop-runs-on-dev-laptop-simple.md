...
description: "Short-term the resident drain daemon runs from the dev LAPTOP in dev mode (launchd, primary plateau-app checkout) — priority is keeping it SIMPLE and running, NOT self-update/self-hosting. Do NOT ratify/build epic #2468; decision #2501 is PREPARED+SHELVED until the daemon leaves the laptop."
metadata:
  node_type: memory
  type: project
...
**How to apply — do NOT build the supervisor self-update chain yet.** Epic **#2468** (slices L1→L2→L3: a dedicated daemon clone, `refreshDaemonSource`, reload) stays **unbuilt**. The self-hosting-boundary decision **#2501** — forks: *dedicated clone* / *`exit(0)` + launchd `KeepAlive` reload* / *self-source exclusion* — is **PREPARED and SHELVED**, not a call to ratify now. Ratify + build it **only when the daemon moves OFF the dev laptop** (onto a real host / the constellation); until then the self-update work is deliberately deferred, not forgotten.
