...
- [Story preparation checklist](story-preparation-checklist.md) — scoping is NOT preparation. A card needs `scope:` INCLUDING consumers (ES importers AND subprocess callers — in this repo the latter is the bigger set), Fibonacci size with its basis (`>8` = slice, not a number), TESTABLE `Done when`, a DECIDED design (a real fork gets NAMED, never picked silently), interfaces/protocol at every seam (the omission costing the most review rounds), tasks, and delivery shape (incremental vs one piece). Operator correction 2026-08-13, measured on four items that burned 2–6 rounds each on card-level omissions
