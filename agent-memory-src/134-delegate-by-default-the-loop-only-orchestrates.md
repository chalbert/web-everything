...
**Judgment-shaped work still stays inline.** Selection, the decision arc ([[feedback_decisions_are_workitems_not_plan_mode]]), slicing/splitting, preparing a fork: the call and the work under it both stay on the loop, unchanged by this reframe. What the inversion changed is *execution* — the mechanical middle of an item whose call is already made. Splitting judgment into a delegated *work* half and an inline *call* half is a live question, not a rule: see the item filed off this reframe.
