...
- [Never write scratch files under .git/](feedback_avoid_bash_heredoc_file_writes.md) — `.git` is a Claude Code hardcoded protected path, never auto-approved by an allow rule; use a plain path inside the repo root instead, not os.tmpdir(); confirmed live 2026-08-17
