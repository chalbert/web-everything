...
- [Normalize timezone before comparing timestamps](normalize-timezone-before-comparing-timestamps.md) — never compare timestamps from different tools unless all are pinned to the same zone (prefer UTC); macOS `stat -f "%Sm"` prints LOCAL time with no offset marker and looks exactly like UTC — use `TZ=UTC stat -f "%Sm" -t "%Y-%m-%dT%H:%M:%SZ" <file>` instead. Diagnosing the live conveyor runner as "stalled 4 hours" off a `date -u` vs. unmarked-local `stat` mismatch; the real gap was ~3 minutes, EDT is UTC-4
