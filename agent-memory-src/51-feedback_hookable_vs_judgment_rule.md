...
**Trigger — a repeated human correction of a *process* is the detection signal.** When the operator corrects you on *how a process runs* (when to continue vs stop a loop, which step is next, when to escalate) — especially more than once — that decision was mis-classified as JUDGMENT when it is mechanical. The fix is to move it into a deterministic controller, **not** to comply more carefully next time (which just drifts again). Often the controller already exists and the gap is *wiring it in*, not new logic — e.g. the jury's `deriveNegotiationOutcome` + `NEGOTIATION_ROUND_CAP` (continue-vs-escalate) was built for the drain's review loop but never wired into the subject-jury harness, so the loop got hand-driven and mis-judged (filed #xvwmwkx, 2026-07-26). Prefer the mechanical solution over an AI/judgment one wherever a script can decide.
