 ...
 *
 * Tests both PLUGGED mode (with bootstrap.ts) and UNPLUGGED mode (with explicit imports)
 */
 ...
 * Errors are collected and can be checked at the end of each test.
 */
...
// Test both plugged and unplugged modes
const modes = [
  { name: 'plugged', url: '/demos/declarative-spa.html' },
  { name: 'unplugged', url: '/demos/declarative-spa-unplugged.html' },
];

for (const mode of modes) {
  test.describe(`Declarative SPA Demo (${mode.name})`, () => {
    let errorCollector: ErrorCollector;

    test.beforeEach(async ({ page }) => {
      // Create fresh error collector for each test
    ...
    test.describe('Form View', () => {
      ...
      test('should update preview in real-time as user types', async ({ page }) => {
        ...
        await nameInput.pressSequentially('lice');
    ...
    test.describe('Mode Verification', () => {
      test(`should be running in ${mode.name} mode`, async ({ page }) => {
        const demoMode = await page.evaluate(() => (window as any).demoMode);
        expect(demoMode).toBe(mode.name);
      });
    });

    test.describe('Error Detection', () => {
      test('should load without JavaScript errors', async ({ page }) => {
        // This test specifically validates no errors during page load
        // The afterEach hook will fail if any errors were captured
        // We add a small wait to catch any async errors
        await page.waitForTimeout(500);

        // Explicitly check for errors in this test
        expect(errorCollector.hasErrors()).toBe(false);
      });

      test('should handle user interactions without errors', async ({ page }) => {
        // Perform various interactions to ensure no runtime errors

        // Counter interactions
        await page.locator('button:has-text("Increment")').click();
        await page.locator('button:has-text("Decrement")').click();
        await page.locator('button:has-text("Reset")').click();

        // Navigate to todos
        await page.locator('nav button[data-view="todos"]').click();
        ...
        // Navigate to form
        await page.locator('nav button[data-view="form"]').click();
        ...
        // Navigate back through all views
        await page.locator('nav button[data-view="counter"]').click();
        await page.locator('nav button[data-view="todos"]').click();
        await page.locator('nav button[data-view="form"]').click();
