...
/** Computed background colour of a chip — the SELECTED TINT under test (#2280). The fixture loads the
...
// ── #2280: the SELECTED TINT must track real pressed state, not the SSR `selected` attribute ──
...
test('at load, a pressed chip carries the accent tint and an unpressed one does not (#2280)', async ({ page }) => {
...
test('toggling a chip OFF strips its tint even though the SSR `selected` attribute survives (#2280 core)', async ({ page }) => {
