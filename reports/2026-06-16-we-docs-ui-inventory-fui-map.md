...
Surveyed `we:src/_layouts/base.njk` (header/nav/footer/shell + inline scripts), `we:src/css/style.css` (1711 lines — the component class families), `we:src/assets/js/reveal-nav.js`, the `src/*.njk` catalog/index pages, and `src/_includes/**` partials. Mapped against the FUI block catalog (`fui:frontierui/src/_data/blocks.json`, 24 blocks): `simple-store, view, tabs, router, handler-expression-parser, event-behaviors, transient-component, audit-trail, background-task-surface, data-grid, droplist, for-each, lifecycle, master-detail, nav-list, resource-loader, selection, stepper, tree-select, type-ahead` + 3 parsers.

## Chrome surfaces (the page frame — we:base.njk)

| # | Surface | Source | FUI component | Status |
|---|---------|--------|---------------|--------|
| C1 | App shell / sticky header bar | `we:base.njk:19-96` `.site-header` `.header-controls` | *(layout primitive — app-shell/header)* | **FUI gap** — no shell/header-layout block |
| C2 | Brand lockup (logo + title) | `we:base.njk:21-24` `.site-title` | *(plain anchor+img; theme-tokened)* | No block needed (theme only) |
| C3 | Tagline | `we:base.njk:25` `.site-tagline` | *(text; theme-tokened)* | No block needed |
| C4 | Disclosure nav — link lists | `we:base.njk:32-88` `.nav-menu` `.nav-group-links` (the #645 APG disclosure) | **`nav-list`** (the #645 conform-to ref) | FUI exists — covers the link lists |
| C5 | Nav section heads (expand/collapse panels) | `we:base.njk:43/60/75` `.nav-menu-head` `aria-expanded` | *(disclosure button + panel)* | **FUI gap** — no `disclosure` block; nav-list is flat, not sectioned |
| C6 | Mobile nav toggle (hamburger) | `we:base.njk:28-31` `.nav-toggle` + inline script `:112-146` | *(disclosure/icon button)* | **FUI gap** — no `button`/`disclosure` block |
| C7 | Header icon buttons (GitHub, Spec Explorer) | `we:base.njk:89-94` `.header-icon` | *(icon button)* | **FUI gap** — no `button` block |
| C8 | Footer | `we:base.njk:102-109` `.site-footer` | *(layout; theme-tokened)* | No block needed |
| C9 | Disclosure-nav behavior (click/Esc/outside-click) | `we:reveal-nav.js` + `we:base.njk:112-146` | **`nav-list`** + **`event-behaviors`** | FUI exists — behavior ships with nav-list |
| C10 | Spec Explorer side panel (resizable iframe) | `we:base.njk:148-399` `.claude-panel` | *(dev-only tool)* | **Excluded** — dev tooling, not dogfood chrome |
...
| P2 | Sortable backlog + prioritisation tables | `we:backlog.njk`, `we:backlog-table-sort.js` | **`data-grid`** (sortable) | FUI exists |
| P3 | Display toolbar (search + filter chips + list/grid view) | `we:display-toolbar.njk`, `.home-display-toolbar` `.home-search` `.home-filter` `.backlog-filter-row` | **`type-ahead`** (search) + *(filter-chip group)* | Partial — type-ahead exists; **gap**: filter-chip/segmented control |
...
| P6 | Code sample frame + copy | `.code-block-dark`, we:prism-theme.css, `we:copy-code.js`, `we:component-source-toggle.njk` | *(code-block / syntax view)* | **FUI gap** — no `code-block` block |
| P7 | Mode / format / source / strategy toggles | `we:mode-selector.njk`, `we:format-selector.njk`, `we:source-toggle.njk`, `we:strategy-toggle.njk` | **`tabs`** / **`selection`** / **`droplist`** | FUI exists — map per toggle semantics |
| P8 | Burndown / coverage meters | `.meter-track` `.meter-fill` | *(meter / progress)* | **FUI gap** — no `meter` block |
| P9 | Project-page layout (sidebar TOC + body) | `we:project-pages.njk` `.project-layout` `.project-sidebar` + tocbot | **`master-detail`** + *(TOC nav)* | Partial — master-detail exists; **gap**: `toc` nav |
