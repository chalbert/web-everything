...
not an `we:intents.json` entry), **#504/#465** (the CEL pivot is the canonical representation; the runtime
is a swappable impl detail), **#464** (the field-error shape is a data contract).

## The real import closure (traced 2026-06-16)

`we:plugs/webvalidation/index.ts` is the single re-export hub. Its cross-subsystem imports:

| we:index.ts line | imports | from |
|---|---|---|
| [:82](../plugs/webvalidation/index.ts#L82) | capability vocabulary | `we:capability-manifest/provider.js` |
| [:169](../plugs/webvalidation/index.ts#L169) | `guardCapability`/`guardCapabilities` | `we:capability-manifest/guard.js` |
| [:107](../plugs/webvalidation/index.ts#L107) | intent vocabulary + adapter contract | `we:validation-generation/provider.js` |
| [:122](../plugs/webvalidation/index.ts#L122) | CEL pivot | `we:validation-generation/cel.js` |
| [:128](../plugs/webvalidation/index.ts#L128) | cross-field layer | `we:validation-generation/crossField.js` |
| [:133](../plugs/webvalidation/index.ts#L133) | adapter registry | `we:validation-generation/registry.js` |
| [:146](../plugs/webvalidation/index.ts#L146) | field-error shape | `we:validation-generation/fieldError.js` |
| [:154](../plugs/webvalidation/index.ts#L154) | shipped adapters | `we:validation-generation/adapters/index.js` |
| [:163](../plugs/webvalidation/index.ts#L163) | Mode-2 service | `we:validation-generation/service.js` |
...
Self-described as "a standalone, dependency-free model of the contract" ([we:provider.ts:1-26](../capability-manifest/provider.ts#L1)),
...
| file | LOC | role | layer |
|---|---|---|---|
| `we:provider.ts` | 240 | #266 manifest model + feature vocabulary + semver | **WE** |
| `we:guard.ts` | 103 | #268 dev-only runtime capability warner (stripped in prod) | **WE** |
| `we:check.ts` | 130 | #267 build-time adherence gate | **WE** |
| `we:report.ts` | 173 | #269 adherence report format | **WE** |
| `we:fixtures.ts` | 242 | #270 conformance fixtures + shared `outOfCapability` diff | **WE** |
| `we:index.ts` | 19 | barrel | **WE** |

Splitting `we:guard.ts` (the only "runtime" file) off to FUI would fork the #266 plane and break the
...
**Caveat (type-only, non-blocking):** `we:report.ts` type-imports `we:../blocks/renderers/report/renderReport.js`
([we:report.ts:23](../capability-manifest/report.ts#L23)) for the `adherenceToReport` mapper (#712). It is
...
|---|---|---|---|---|
| `we:provider.ts` | 249 | #304 neutral intent vocabulary + `CustomValidationAdapter` contract (SoT) | **WE** | high |
| `we:registry.ts` | 91 | adapter-resolution SoT ("one home … cannot drift") | **WE** | high |
| `we:fieldError.ts` | 104 | #464 RFC 9457 field-error **data contract** | **WE** | high |
| `we:cel.ts` | 241 | #504/#465 portable CEL pivot — the canonical representation | **WE** | med-high |
| `we:service.ts` | 161 | #309 Mode-2 wire contract + pure reference handler | **WE** | **med — Fork C** |
| `we:crossField.ts` | 110 | Mode-1 forward (`emitCrossFieldOrFallback`) + ingest (`jsonLogicToCel`) adapter layer | **FUI** | med |
| `adapters/{zod,pydantic,jsonSchema,nativeHtml,index}.ts` | 510 | per-language forward/generation emitters (#305–#308) | **FUI** | high |

- **provider/registry/fieldError → WE:** textbook neutral-contract SoT (#463). we:provider.ts explicitly
  contrasts itself with impl ("A code model, not a UX intent", [we:provider.ts:14](../validation-generation/provider.ts#L14)).
- **cel.ts → WE:** #465 ruled CEL is "the single canonical representation a WE-compliant component
  ...
  contract clause" ([we:cel.ts:1-13](../validation-generation/cel.ts#L1)). The pivot AST + parser is the
...
- **crossField.ts → FUI:** the Mode-1 forward+ingest **adapter layer** (orchestrates `adapter.emitCrossField`;
  ...
  (we:cel.ts) + contract (we:provider.ts) → a clean FUI→WE edge. Alternative: keep with we:cel.ts in WE as the
...
### Fork C carve — `we:service.ts` (Mode-2 service)

The one genuine cross-cutting tension. we:service.ts is the Mode-2 generation **service** (#309): a thin,
...
([we:service.ts:1-22](../validation-generation/service.ts#L1)). It splits conceptually:
...
- The **handler** (`handleValidationRequest`/`serveValidation`) is a pure reference dispatcher. Per #091
  ...
  future plateau-app/FUI piece. **Default: we:service.ts whole → WE.** Alternative: port the handler to FUI
  as the impl host (foreclosing the reference-in-standard symmetry with we:check.ts/MaaS). Medium confidence
...
- **→ FUI (impl, ports with the plug):** `we:validation-generation/crossField.ts` + `adapters/*` (6 files).
