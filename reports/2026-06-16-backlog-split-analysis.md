...
| Slice | workItem · size | Home (new + reads) | Blocked by |
|---|---|---|---|
| **A** — *decision*: Plateau↔WE **embed/seed transport contract** (iframe + query-param seed? shared component? deep-link-only?) — analogous to #700/#701 and to sibling decision #747 | `type:decision` · — | `we:plateau-app/.../configurator.ts:581` (`mountTechnicalConfigurator` — no seed today), the #701 `fuiDemo` iframe precedent | — (root) |
| **B** — add the **four WE technical dimensions as Configurator domains**: render-strategy (#079), transport (#455), trait-lazy-load (#448), chunk-split (#719/#720) — 4× `seed-*.ts` + `we:provider.ts` entries + `plateau:presets.ts` blocks | `story` · 5 | new `we:plateau-app/.../seed-render-strategy.ts` (+3 more), `we:provider.ts:19-29` (`DOMAINS[]`), `plateau:presets.ts` | — (root) |
