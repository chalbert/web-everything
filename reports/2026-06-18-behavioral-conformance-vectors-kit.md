...
How does an implementer prove a component conforms to a WE standard when the contract is build-agnostic (only the final rendered component counts)? A static manifest check (`we:capability-manifest/check.ts:1-12`, the build-time `check:validation-adherence` #267) validates *declared-vs-used features* but cannot capture **behavioral/temporal** conformance — e.g. that `validator-resolution`'s `versioning` default drops a stale async generation (`we:validator-resolution/provider.ts:84-88`). The leaning direction: WE publishes a conformance **KIT** (declarative JSON vector corpus + vector schema + dependency-free reference verifier + binding interface); implementers supply a binding + their own browser driver and run in their own CI (the escapable floor); plateau hosts the zero-setup in-browser exerciser (controllable clock, dashboards) as the paid product. Two stated forks: (1) runner home plateau vs FUI; (2) reference-verifier thinness — pure assertion semantics vs also a small reference driver.
...
- **Forced invariant A — the layer split (per #817's file-seam test).** WE owns the vector corpus (JSON) + vector schema + **assertion-semantics** reference verifier (it *is* a WE conformance gate, like `we:check.ts`, so #817 keeps it WE) + the binding **interface** (including a clock *verb contract* `now/tick/tickAsync/next/runAll`). The binding **implementation** (mount/dispatch/read) and the clock **implementation** (Sinon-style embedded / Playwright `page.clock`-style) are runtime → FUI. The broken branch — a runtime clock/DOM driver *inside `@webeverything`* — is exactly what #817 excluded from WE. The verifier consumes **time as data** (a timestamped observed trace), never a live clock, so no WE gate consumes the clock impl → it ports out.
...
| **ARIA-AT** | Invariant **assertions** authored once (priority `1/2/3` = MUST/SHOULD/MAY); per-impl **commands** vary (`we:commands.json`). aria-at-app = hosted iframe runner collecting interop results — the open-corpus + paid-hosted shape again. |
...
**In-repo prior art (not greenfield):** WE already ships a conformance-vector + reference-target + golden-output pattern at `blocks/renderers/module-service/conformance/` (`we:vectors.ts:17-29` `VectorInput`, `we:golden.json`, `we:runner.ts`, `we:referenceTarget.ts`, `we:dotnetTarget.ts`). #899 generalizes that *request→golden* shape to **behavioral/temporal** vectors with **DOM/ARIA observation** and a **controllable clock**.
...
| File | Action |
|---|---|
| `we:reports/2026-06-18-behavioral-conformance-vectors-kit.md` | Created (this report) |
| `we:src/_data/researchTopics.json` | Added `behavioral-conformance-vectors-kit` topic |
| `we:src/_includes/research-descriptions/behavioral-conformance-vectors-kit.njk` | Created write-up |
| `we:backlog/899-behavioral-conformance-vectors-in-browser-implementer-valida.md` | Rewritten to prepared-fork shape; `preparedDate` set; `relatedReport`/`relatedProject` linked |
