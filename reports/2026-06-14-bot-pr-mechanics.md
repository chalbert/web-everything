...
- **Invariant (ratify, not weigh): generated code is NOT privileged.** A generated fix runs the *identical*
  ...
  (`we:scripts/autofix/engine.mjs:229-247`) is *necessary but not sufficient*; the org's CI + review still gate
...
- **Fork 2 — PR evidence payload: a standard-owned conformance-evidence manifest vs. Plateau-formatted
  markdown.** The autofix gate already produces the failure + verify before/after (`we:engine.mjs`); #410 Fork
  ...
  self-description / trace substrate (reusing the capability-manifest vocabulary, `we:capabilityMatrix.json:1`),
...
3. **Forge abstraction is a solved provider-interface pattern.** Renovate's `Platform`
   (`we:lib/modules/platform/types.ts`) abstracts GitHub/GitLab/Bitbucket/Azure/Gitea/Forgejo/Gerrit/CodeCommit;
...
| File | Action |
|---|---|
| `we:reports/2026-06-14-bot-pr-mechanics.md` | created (this report) |
| `we:src/_data/researchTopics.json` | added `bot-pr-mechanics` entry |
| `we:src/_includes/research-descriptions/bot-pr-mechanics.njk` | created (write-up) |
