...
**Real idioms it leans on:** `type-ahead` behavior (`fui:blocks/type-ahead/TypeAheadBehavior.ts`), Plateau's `composite-widget` attribute (`plateau:plateau/src/blocks/attributes/CompositeWidget.ts`), the injector chain (`customContexts:*`), and the `with*` traits in `blocks/resource-loader/traits/`.
