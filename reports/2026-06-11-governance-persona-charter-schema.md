...
- **`Profile`** (`plateau-app/src/profiles/profiles.ts:85`) — `id`, `name`, `title`, `accent`, `glyph`, `mission`, `signals[]`, `reviewAreas[]`, `artifactsOwned[]`, `escalation`.
- **`ReviewArea`** (`profiles.ts:17`) — `title`, `platformArea` (the join key, `profiles.ts:22`), `why`, `reviews[]`, `gates[]`, `manages[]`.
- **`Gate`** (`profiles.ts:34`) — `trigger`, `blocksDeployment: boolean` (`profiles.ts:39`), `sla`.
- **`Signal`** (`profiles.ts:45`) — `label`, `value`, `status: 'ok' | 'watch' | 'risk'`.
- The roster itself is a hand-authored array, `export const profiles: Profile[]` (`profiles.ts:941`), with all seven personas inline.

`platformArea` is typed as a closed union (`PlatformArea`, `profiles.ts:54`) pinning each review area to a real Plateau domain (`apps`, `dependencies`, `intents`, `deployment`, …), so a profile is a *lens over the platform*, not a parallel structure. This is the shape the forks must either keep, share, or enforce — they do not redesign it.

Two facts from `profiles.ts` are load-bearing for the forks:

1. **`blocksDeployment` is already a boolean on every gate** (`profiles.ts:39`, set `true`/`false` across ~30 gate definitions) — but it is purely *descriptive* today; nothing reads it to actually block anything. Fork 4 is about whether that boolean ever becomes an enforced signal.
2. **The roster is a static TypeScript array** (`profiles.ts:941`) compiled into plateau-app. Fork 3 is about whether it stays there or graduates to a shared data home both lenses read.
...
- **Gate types (the key find):** the literature names exactly four —
  ...
  The current schema collapses all of this into a single `blocksDeployment: boolean` (`profiles.ts:39`) — i.e. only Advisory (`false`) vs. Blocking (`true`). This four-way enum is the grounded vocabulary for Fork 4's enforcement axis, and `escalation` (already a `Profile` field, `profiles.ts:104`) is the Escalating route.
