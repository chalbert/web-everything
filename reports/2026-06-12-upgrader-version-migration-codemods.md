...
1. **Both prerequisites #191 named already shipped.** (a) The "machine-readable change/migration
   descriptor per release" is the **`changelog-manifest` protocol** ([we:protocols.json:86](../src/_data/protocols.json#L86),
   ...
   [we:upgraderEngine.ts:142](../blocks/renderers/upgrader/upgraderEngine.ts#L142), `upgrade()` orchestrator
   :216, `CustomAnalyzerRegistry` :72). #266 added the semver/availability axis (`compareSpecVersions`
   [we:capability-manifest/provider.ts:182](../capability-manifest/provider.ts#L182), `featureAvailableIn`
...
2. **Angular `ng update` is the canonical version-gated migration runner.** A library ships a
   `we:migrations.json` (registered under `we:package.json` `ng-update.migrations`); each entry is `{version,
   ...
   versions**. This is exactly the shape #191 needs: `changelog-manifest` = `we:migrations.json` (per-release,
...
| File | Action |
|---|---|
| `we:reports/2026-06-12-upgrader-version-migration-codemods.md` | created (this report) |
| `we:src/_data/researchTopics.json` | added `version-migration-codemods` entry |
| `we:src/_includes/research-descriptions/version-migration-codemods.njk` | created (write-up) |
| `we:backlog/191-upgrader-version-migration-codemods.md` | rewritten to prepared-fork shape; `preparedDate` set |
