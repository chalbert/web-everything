...
owns the only template; a hand-written page silently drops the required fields, is overwritten by the next
run, and fails `node scripts/progress-board.mjs --verify=<path>` — which is how you check a file you did not
just generate.
