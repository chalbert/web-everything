...
// (#1952/#2149 Fork 1): FLAT developer-unique-keyed config + line-structured singletons (optimistic-merge bucket) —
// incl. package.json, whose duplicate-key clean-but-wrong class is caught by a merge-gate lint, not this list.
// HERE though (#2149 Fork 2): .eleventy.js, a registration monolith whose same-name/order-sensitive registrations
// clean-merge silently and are NOT lintable. frontierui = its monolithic
...
const RESERVED_MERGE_RISK_BY_REPO = {
  we: [
    ...
    '.eleventy.js', // #2149 Fork 2: registration monolith — same-name/order-sensitive registrations clean-merge silently and are NOT lintable (unlike we:package.json, Fork 1). Delist via ① iff a split proves order-insensitivity.
  ],
  frontierui: [
...
const PROBE_SCHEMA = {
  ...
  properties: {
    ...
    touchesMonolith: {
      type: 'array', items: { type: 'string' },
      description: 'genuinely-monolithic shared files it must edit (the single-doc registries src/_data/{traits,docs,capabilityMatrix}.json, webhandlers/webportals.json, the curated-sweep artifacts workbench*/benchmark*.json, the hand-authored PROSE body of AGENTS.md — its AUTO-GENERATED inventory block is a derived artifact, see derivedArtifacts — and the Eleventy registration monolith .eleventy.js: #2149 Fork 2 lists it because two lanes registering the same addFilter/addShortcode name, or order-sensitive registrations, clean-merge silently and are NOT lintable) — list them explicitly. These are the merge-risk (blacklist) files: an item serializes against ANOTHER item only when BOTH touch the same one (a clean git merge of two edits to one of these can be silently wrong); a lone toucher still runs concurrent. NOT monolithic: package.json (a keyed manifest — #2149 Fork 1 keeps it optimistic behind a duplicate-key merge-gate lint, not this list) and per-entry registry files (src/_data/<reg>/<id>.json, INCLUDING src/_data/adapters/<id>.json since #1938) — never list them.',
    },
    confident: { type: 'boolean', description: 'TRUE when every predicted file (in WE AND every extraRepo) is one this item OWNS (its own impl/code, its own demo page, its own backlog/NNN.md, its own per-entry registry entries, its own test file). FALSE *only* when work plausibly spills into a SHARED surface another item could also touch: a still-monolithic registry, shared runtime (plugs/bootstrap.ts), a shared *.njk include, shared test specs, or a broad cross-file refactor — in ANY affected repo. FLAT build config (tsconfig/vite/package.json/vitest.config) does NOT lower confidence (#1952/#2149 Fork 1): it is line-/key-structured and merges optimistically (package.json's only clean-but-wrong class, duplicate keys, is caught by a merge-gate lint, not serialization), so such an edit is owned, not shared-risk. The ONE build-adjacent exception is .eleventy.js — a registration monolith, listed in touchesMonolith (#2149 Fork 2), so treat a touch on it as merge-risk, not owned. Routine uncertainty about the exact file COUNT is NOT a reason for false; only genuine shared-surface risk is. Per-entry registry writes do NOT lower confidence. NOTE (#1950): confident:false NO LONGER forces the serial lane on its own — it only tightens the pairwise check (a low-confidence item serializes against ANOTHER item it overlaps on any file). A confident:false item that is file-disjoint from every other still runs concurrent under the optimistic floor.' },
  },
};
